#!/usr/bin/env python3
"""Build the external, canonical blind-v2 frozen-design release asset.

The command is deliberately not a general JSON editor.  It reads the exact
tracked draft from the reviewed checkout and permits only the lifecycle and
artifact-binding changes accepted by :func:`v2.runner.validate_frozen_design`.
...
from v2.runner import RunnerError, validate_frozen_design  # noqa: E402
...
DRAFT_RELATIVE_PATH = "v2/design-registration.draft.json"
...
class FrozenDesignBuildError(ValueError):
    """The reviewed checkout or a freeze binding is incomplete or mutable."""
...
def _ensure_external_output(output: Path, lab_root: Path) -> None:
    ...
    try:
        ...
        return
    raise FrozenDesignBuildError(
        "frozen design output must remain outside the reviewed implementation tree"
    )


def verify_reviewed_checkout(
    lab_root: Path,
    *,
    ...
    if require_running_checkout and root != PROJECT_ROOT.resolve(strict=True):
        raise FrozenDesignBuildError(
            "builder must execute from the exact reviewed lab checkout"
        )
    ...
    if observed_commit != expected_commit:
        raise FrozenDesignBuildError("reviewed checkout commit differs")
    if observed_tree != expected_tree:
        raise FrozenDesignBuildError("reviewed checkout tree differs")
    origin = _git(root, ["remote", "get-url", "origin"]).strip().decode()
    if _repository_slug(origin) != _repository_slug(expected_repository):
        raise FrozenDesignBuildError("reviewed checkout origin differs")
    if _git(root, ["status", "--porcelain=v1", "--untracked-files=all"]):
        raise FrozenDesignBuildError("reviewed checkout is not clean")
    ...
    if draft_raw != blob_raw:
        raise FrozenDesignBuildError("working draft bytes differ from reviewed Git blob")
    try:
...
def construct_frozen_design(
    draft: Mapping[str, Any], manifest: Mapping[str, Any], manifest_raw: bytes
) -> dict[str, Any]:
...
def build_frozen_design(
    *,
    ...
    draft, draft_raw = verify_reviewed_checkout(
        lab_root,
    ...
    _draft_again, draft_raw_again = verify_reviewed_checkout(
        lab_root,
        expected_repository=implementation["repository"],
        expected_commit=expected_lab_commit,
        expected_tree=expected_lab_tree,
    ...
    if draft_raw_again != draft_raw:
        raise FrozenDesignBuildError("reviewed draft changed during freeze construction")

    frozen = construct_frozen_design(draft, manifest, manifest_raw)
