...
from v2.protocol import load_json_strict_bytes
...
from v2.github_release_attestation import (
...
from v2.reproducibility import canonical_json_bytes, write_new_bytes
from v2.source_archive import SourceArchiveError, verify_source_archive
...
CI_PAYLOADS = {
    "linux-ci-artifact": {
        ...
        "machine": "x86_64",
        "files": {
            "v2-preflight-linux.json",
            "v2-runtime-linux.json",
            "v2-zero-skip-linux.log",
            "v2-design-check-linux.json",
            "v2-release-attestation-known-answer-linux.json",
        },
        "suffix": "linux",
    ...
    "macos-arm64-ci-artifact": {
        ...
        "machine": "arm64",
        "files": {
            "v2-preflight-macos.json",
            "v2-runtime-macos.json",
            "v2-zero-skip-macos.log",
            "v2-design-check-macos.json",
            "v2-release-attestation-known-answer-macos.json",
        },
        "suffix": "macos",
...
def _github_gate_summary(
    ...
    *,
    ...
    review = gate.get("reviewGate")
    ci = gate.get("ciGate")
    if not isinstance(repository, dict) or not isinstance(review, dict) or not isinstance(ci, dict):
    ...
    try:
        verified = verify_github_gate_receipt(
            ...
            expected_reviewer_login=review.get("reviewerLogin"),
        ...
        raise ZenodoArchiveError("GitHub gate receipt failed independent verification") from error
    artifacts = dict(verified.artifact_sha256)
    linux = [name for name in artifacts if name.startswith("blind-v2-linux-development-")]
    macos = [name for name in artifacts if name.startswith("blind-v2-macos-development-")]
...
def _verify_ci_payload(
    ...
    *,
    ...
    _verify_runtime_manifest(
        payloads[f"v2-runtime-{suffix}.json"],
    ...
    preflight = _json_object(
        payloads[f"v2-preflight-{suffix}.json"], label=f"{role} preflight"
    )
    platform = preflight.get("platformSafety")
    codec = preflight.get("codec")
    if (
        preflight.get("schemaVersion") != "corelm-crossmodel-livewiki-v2-preflight-v1"
    ...
    check = _json_object(
        payloads[f"v2-design-check-{suffix}.json"], label=f"{role} design check"
    )
    if (
        check.get("schemaVersion") != "corelm-crossmodel-livewiki-v2-design-check-v1"
    ...
    known_answer = _json_object(
        payloads[f"v2-release-attestation-known-answer-{suffix}.json"],
    ...
    try:
        log = payloads[f"v2-zero-skip-{suffix}.log"].decode("utf-8", "strict")
