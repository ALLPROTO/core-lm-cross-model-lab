#!/usr/bin/env python3
"""Fail-closed supervisor for the registered blind-v2 one-shot.
...
from v2.git_source import (  # noqa: E402
    GitSourceSeal,
...
from v2.mediawiki_snapshot import (  # noqa: E402
...
from v2.package_snapshot_release import (  # noqa: E402
    SnapshotReleaseError,
    verify_snapshot_release,
)
from v2.preflight import (  # noqa: E402
    platform_safety,
...
PRIVATE_SCHEMA = "corelm-crossmodel-livewiki-v2-private-snapshot-manifest-v1"
FREEZE_SCHEMA = "corelm-crossmodel-livewiki-v2-freeze-manifest-v1"
RESULT_SCHEMA = "corelm-crossmodel-livewiki-v2-result-v1"
ONE_SHOT_NOT_BEFORE = datetime(2026, 8, 28, 18, tzinfo=timezone.utc)
HARD_DEADLINE = datetime(2026, 8, 29, 18, tzinfo=timezone.utc)
...
SCIENTIFIC_HASH_INPUT = "corelm-crossmodel-livewiki-v2"
SCIENTIFIC_HASH_KNOWN_ANSWER = -7302361945815449509
...
def _validate_schema(instance: Any, filename: str) -> None:
    ...
    schema = load_json_strict(V2_ROOT / "schemas" / filename)
...
def validate_frozen_design(design: Any) -> None:
    ...
    reference = load_json_strict(V2_ROOT / "design-registration.draft.json")
    ...
    # Every scientific choice must equal the tracked, reviewed draft.  Only the
...
def validate_freeze_manifest(
    ...
    *,
    ...
    if manifest.get("status") != "IMPLEMENTATION_FREEZE_READY_FOR_DESIGN_BINDING":
        raise RunnerError("freeze manifest is not reviewed/frozen")
    ...
    if (
        ...
        or approved.get("commitSHA") != design["labSource"]["commit"]
        or approved.get("reviewerLogin", "").lower() == "allproto"
    ):
        raise RunnerError("freeze manifest lacks an independent approved review")
    if (
...
def _require_design_publication_source(
    ...
    """Require the design tag to target the one reviewed implementation tree."""
...
def verify_runtime_live(runtime_manifest: Mapping[str, Any], runtime_root: Path) -> None:
    ...
    if runtime_manifest.get("schemaVersion") != "corelm-crossmodel-livewiki-v2-runtime-manifest-v1":
    ...
    try:
        while True:
            ...
            if return_code is not None:
                ...
                return {
                    "schemaVersion": "corelm-crossmodel-livewiki-v2-supervisor-receipt-v1",
                ...
                supervisor_receipt = _supervise_worker(
                    _networkless_macos_command(
                        _scientific_python_command(
                            sys.executable,
                            str(private_root / "lab" / "v2" / "model_worker.py"),
                            "--job",
                            str(job_path),
                            "--snapshot-root",
                            str(private_root),
...
def execute_private_one_shot(private_root: Path, result_root: Path) -> str:
    ...
    expected_script = (private_root / "lab" / "v2" / "runner.py").resolve(strict=True)
            ...
            try:
                verifier_supervisor_receipt = _supervise_worker(
                    _networkless_macos_command(
                        _scientific_python_command(
                            sys.executable,
                            str(private_root / "lab" / "v2" / "verify_evidence.py"),
                            "--evidence-root",
                            str(result_root),
                            "--codec-root",
                            str(private_root / "codec"),
...
def reexec_private_one_shot(
    *, private_root: Path, result_root: Path
    ...
    script = _private_file(private_root, "lab/v2/runner.py")
...
def main() -> int:
    ...
    try:
        ...
        print(f"BLIND-V2 RUNNER FAIL: {error}", file=sys.stderr)
        return 1
