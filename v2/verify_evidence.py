#!/usr/bin/env python3
"""Recompute the complete blind-v2 verdict from frozen raw evidence."""
...
from v2.evidence import (  # noqa: E402
    EvidenceError,
...
from v2.git_source import GitSourceError, verify_copied_source  # noqa: E402
from v2.github_gate_receipt import (  # noqa: E402
    GitHubGateReceiptError,
    verify_github_gate_receipt,
)
from v2.independent_verifier_core import (  # noqa: E402
...
from v2.independent_model_replay import (  # noqa: E402
...
from v2.package_design_release import (  # noqa: E402
    DesignReleaseError,
    verify_design_release_package,
)
from v2.publication import (  # noqa: E402
    PublicationError,
...
def validate_frozen_design(design: Any) -> None:
    ...
    if design.get("suiteId") != "corelm-voidtoken-crossmodel-livewiki-v2":
        raise EvidenceError("design suiteId differs")
    if design.get("schemaVersion") != "corelm-crossmodel-livewiki-v2-design-v1":
...
def verify_private_snapshot_manifest(
    ...
    if (
        value["schemaVersion"]
        != "corelm-crossmodel-livewiki-v2-private-snapshot-manifest-v1"
...
def verify_github_gate_binding(
    ...
    *,
    ...
    """Replay archived review/CI bytes and bind them to the frozen implementation."""
    ...
    if not isinstance(freeze, dict) or set(freeze) != freeze_fields:
        raise EvidenceError("freeze manifest fields differ")
    if (
        freeze["schemaVersion"]
        != "corelm-crossmodel-livewiki-v2-freeze-manifest-v1"
        or freeze["status"]
        != "IMPLEMENTATION_FREEZE_READY_FOR_DESIGN_BINDING"
        or freeze["suiteId"] != design["suiteId"]
        or freeze["countsTowardScientificVerdict"] is not False
    ):
        raise EvidenceError("freeze manifest identity/state differs")
    if freeze["freezeProcedure"] != {
        ...
        "designBindingDigest": (
            "sha256(canonical-json-with-contentSHA256-plus-terminal-LF)"
        ),
        "implementationMutationAfterManifest": "FORBIDDEN",
        "manifestContainsOwnFileSHA256": False,
    }:
        raise EvidenceError("freeze procedure differs")
    unsigned_freeze = dict(freeze)
    content_digest = unsigned_freeze.pop("contentSHA256")
    if (
        not isinstance(content_digest, str)
        or re.fullmatch(r"[0-9a-f]{64}", content_digest) is None
        or hashlib.sha256(canonical_json_bytes(unsigned_freeze)).hexdigest()
        != content_digest
    ):
        raise EvidenceError("freeze manifest content digest differs")
    ...
    if freeze["artifacts"] != expected_artifacts:
        raise EvidenceError("freeze artifact bindings differ")
    review = freeze["review"]
    ci = freeze["continuousIntegration"]
    approved = review.get("approvedReview") if isinstance(review, dict) else None
    if not isinstance(approved, dict) or not isinstance(ci, dict):
        raise EvidenceError("freeze review/CI statements are absent")
    try:
        verified = verify_github_gate_receipt(
            ...
            expected_pull_request_number=review["pullRequestNumber"],
            expected_implementation_commit=expected_implementation["commit"],
            expected_reviewer_login=approved["reviewerLogin"],
        ...
        raise EvidenceError(
            f"archived GitHub review/CI gate failed offline replay: {error}"
        ) from error
    repository_base = expected_implementation["repository"]
    if repository_base.endswith(".git"):
        repository_base = repository_base[:-4]
    expected_review = {
        "pullRequestURL": (
            f"{repository_base}/pull/{verified.pull_request_number}"
        ),
    ...
    expected_ci = {
        "runURL": f"{repository_base}/actions/runs/{verified.workflow_run_id}",
    ...
    if review != expected_review or ci != expected_ci:
        raise EvidenceError("freeze review/CI prose differs from archived GitHub bytes")
    created_at = freeze["createdAt"]
    if (
        not isinstance(created_at, str)
        or re.fullmatch(
            r"[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}Z",
        ...
        is None
        or created_at < verified.review_submitted_at
        or created_at < verified.last_server_date
    ):
        raise EvidenceError("freeze manifest predates the verified GitHub gate")
...
def verify_runtime_asset_sbom_bindings(
    ...
    *,
    ...
    if (
        ...
        != "corelm-crossmodel-livewiki-v2-runtime-manifest-v1"
    ...
    if (
        ...
        != "corelm-crossmodel-livewiki-v2-asset-receipt-v1"
...
def validate_selection(selection: Any, design: dict[str, Any], snapshot_digest: str):
    ...
    if selection.get("schemaVersion") != "corelm-crossmodel-livewiki-v2-selection-v1":
    ...
    *,
    ...
    for model_key in model_order:
        ...
        if (
            ...
            != "corelm-crossmodel-livewiki-v2-supervisor-receipt-v1"
        ...
        expected_identity = {
            "schemaVersion": "corelm-crossmodel-livewiki-v2-worker-summary-v1",
            "suiteId": marker["suiteId"],
...
def canonical_scientific_result(
    ...
    *,
    ...
    return {
        "schemaVersion": "corelm-crossmodel-livewiki-v2-result-v1",
