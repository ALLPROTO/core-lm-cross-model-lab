...
No v2 scientific result exists. The selector's committed known-answer vector
...
```sh
/path/to/locked/python -I -B v2/run_zero_skip_tests.py
/path/to/locked/python -I -B v2/verify_design.py
/path/to/locked/python -I -B v2/preflight.py \
...
`v2/.assets/ud-english-pud-r2.18/en_pud-ud-test.conllu` (SHA-256
...
receipt before `2026-08-09T00:00:00Z`. Local files, an unsigned tag, or an
...
```sh
/path/to/locked/python v2/collect_snapshot.py \
  --phase crawl-1 \
  --asset-manifest v2/model-assets.draft.json \
  ...
  --output-root /new/path/livewiki-v2-snapshot
```
...
runtime, asset, source, review, CI, CA, and NIST-trust inputs; it cannot waive a
missing independent review or publication receipt. Public release creation,
...
clean-clone macOS arm64 and Linux zero-skip CI on that exact commit, independent
human approval of the same commit using the exact registered declaration,
