# Canonical signed GitHub release receipts (v2)
...
For every v2 development-control, design, snapshot, evidence, and closeout
release, the expected commit and tree are exactly the reviewed implementation
...
The collector additionally requires this exact regular executable, not a
PATH-resolved `gh`: `/Users/ivan/.cache/corelm/tools/gh-2.97.0-macos-arm64/bin/gh`.
It must be the preregistered GitHub CLI 2.97.0 macOS-arm64 binary
...
```sh
python3 -I -B \
  v2/tests/integration_release_attestation_crypto_known_answer.py \
...
python3 -I -B v2/collect_release_receipt.py \
...
```sh
python3 -I -B v2/package_design_release.py package \
...
`corelm-crossmodel-livewiki-v2-development-control`; its GitHub immutable-release
