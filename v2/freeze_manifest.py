#!/usr/bin/env python3
"""Create and verify the non-self-referential blind-v2 freeze manifest.

The reviewed implementation commit is stage one.  This canonical manifest is
...
from v2.mediawiki_snapshot import PROJECTS, PinnedHTTPSClient  # noqa: E402
from v2.development_artifact_verifier import (  # noqa: E402
...
from v2.nist_beacon import (  # noqa: E402
...
from v2.release_receipt import (  # noqa: E402
...
SCHEMA_VERSION = "corelm-crossmodel-livewiki-v2-freeze-manifest-v1"
STATUS = "IMPLEMENTATION_FREEZE_READY_FOR_DESIGN_BINDING"
SUITE_ID = "corelm-voidtoken-crossmodel-livewiki-v2"
RUNTIME_SCHEMA = "corelm-crossmodel-livewiki-v2-runtime-manifest-v1"
ASSET_RECEIPT_SCHEMA = "corelm-crossmodel-livewiki-v2-asset-receipt-v1"
TRUST_SCHEMA = "corelm-crossmodel-livewiki-v2-nist-trust-bundle-v1"
PULSE_TIME = datetime(2026, 8, 27, 18, 0, 0, tzinfo=timezone.utc)
...
DEVELOPMENT_ARCHIVE_MANIFEST_SCHEMA = (
    "corelm-crossmodel-v2-real-e2e-development-archive-manifest-v1"
)
DEVELOPMENT_ARCHIVE_TAG = (
    "corelm-crossmodel-livewiki-v2-development-control"
)
...
DEVELOPMENT_PLAN_SCHEMA = "corelm-crossmodel-v2-real-e2e-development-plan-v1"
DEVELOPMENT_REPLAY_SCHEMA = (
    "corelm-crossmodel-v2-real-e2e-development-model-replay-v1"
)
...
DEVELOPMENT_SUITE_ID = "corelm-voidtoken-crossmodel-v2-development-e2e"
...
DEVELOPMENT_ARCHIVED_INPUTS = {
    "designRegistration": (
        ...
        "v2/design-registration.draft.json",
    ),
    "modelAssetManifest": (
        "inputs/model-assets.draft.json",
        "v2/model-assets.draft.json",
    ),
    "fullAssetReceipt": (
        "inputs/model-assets.full-rehash.json",
        "v2/manifests/model-assets.full-rehash.json",
    ),
    "developmentCorpusManifest": (
        "inputs/development-corpus.draft.json",
        "v2/development-corpus.draft.json",
    ),
...
def _validate_development_rights_evidence(
    ...
    try:
        ...
        tracked_manifest_raw = read_regular_bytes(
            V2_ROOT / "development-corpus.draft.json",
...
def _validate_development_plan(
    ...
    *,
    ...
    tracked_design_raw = read_regular_bytes(
        V2_ROOT / "design-registration.draft.json",
        maximum_bytes=MAX_DESIGN_BYTES,
    )
    try:
...
def verify_development_control_report(
    ...
    *,
    ...
    if (
        ...
        != "corelm-crossmodel-v2-real-e2e-development-start-v1"
...
def _gate_manifest_sections(
    ...
    *,
    ...
    return review, continuous_integration
...
def _verify_github_gate_input(
    ...
    *,
    ...
    try:
        review_gate = value["reviewGate"]
        ci_gate = value["ciGate"]
        if not isinstance(review_gate, dict) or not isinstance(ci_gate, dict):
        ...
        verified = verify_github_gate_receipt(
            ...
            expected_reviewer_login=review_gate["reviewerLogin"],
        ...
        raise FreezeManifestError(
            "GitHub gate receipt failed independent offline verification"
        ) from error
    return verified, raw
...
def validate_freeze_manifest(value: Any) -> None:
    ...
    review = _required_fields(
        manifest["review"],
        {"pullRequestURL", "pullRequestNumber", "approvedReview"},
        label="review",
    )
    pr_number = review["pullRequestNumber"]
    if type(pr_number) is not int or pr_number < 1:
        raise FreezeManifestError("pull request number must be positive")
    implementation_base = _github_repository_base(
        implementation["repository"], label="implementation repository"
    )
    if review["pullRequestURL"] != f"{implementation_base}/pull/{pr_number}":
        raise FreezeManifestError("pull request URL does not match implementation repository")
    approved = _required_fields(
    ...
    if type(approved["id"]) is not int or approved["id"] < 1:
        raise FreezeManifestError("approved review id must be positive")
    if (
        not isinstance(approved["reviewerLogin"], str)
        or GITHUB_LOGIN.fullmatch(approved["reviewerLogin"]) is None
    ):
        raise FreezeManifestError("approved reviewer login is invalid")
    repository_owner = urlsplit(implementation_base).path.strip("/").split("/", 1)[0]
    if approved["reviewerLogin"].casefold() == repository_owner.casefold():
        raise FreezeManifestError("approved review is not independent of repository owner")
    if approved["state"] != "APPROVED":
        raise FreezeManifestError("review is not approved")
    if approved["commitSHA"] != implementation["commit"]:
        raise FreezeManifestError("approved review does not bind implementation commit")
    submitted_at = _utc_second(
        approved["submittedAt"], label="approved review submittedAt"
    )
    ...
    if gate_last > DECISION_CHECKPOINT:
        raise FreezeManifestError("GitHub review/CI gate missed the decision checkpoint")
    ...
    if (
        ...
        raise FreezeManifestError(
            "freeze manifest predates review, CI, or development-control completion"
        )
...
def verify_artifact_inputs(
    ...
    *,
    ...
    expected_review, expected_ci = _gate_manifest_sections(
        verified_gate,
        implementation_repository=manifest["implementation"]["repository"],
    )
    if manifest["review"] != expected_review:
        raise FreezeManifestError("review fields differ from verified GitHub gate receipt")
...
def build_freeze_manifest(
    *,
    ...
    review, continuous_integration = _gate_manifest_sections(
        verified_gate,
...
def verify_design_binding(
    ...
    if (
        design.get("schemaVersion") != "corelm-crossmodel-livewiki-v2-design-v1"
