# Blind-v2 clean-clone reproducibility
...
the archived review and green CI. There is no separate, later publication
...
git -C lab fetch origin tag corelm-crossmodel-livewiki-v2-design
test "$(git -C lab rev-list -n 1 corelm-crossmodel-livewiki-v2-design)" = \
...
```sh
cd lab
RUNTIME_ROOT="$PWD/v2/.runtime/linux"
./v2/bootstrap_runtime.sh \
  --platform linux \
  --codec-root ../codec-source \
  --runtime "$RUNTIME_ROOT" \
  --python "$(command -v python3.12)"
"$RUNTIME_ROOT/bin/python" -I -B v2/run_zero_skip_tests.py
```
...
```sh
cd lab
RUNTIME_ROOT="$PWD/v2/.runtime/macos"
./v2/bootstrap_runtime.sh \
  --platform macos \
  --codec-root ../codec-source \
  --runtime "$RUNTIME_ROOT" \
...
mkdir -p v2/.working
python3 v2/create_asset_receipt.py \
  --asset-root v2/.assets \
  --output v2/.working/asset-receipt.json
```

The same standard-library downloader writes the byte-pinned real-data input to
`v2/.assets/ud-english-pud-r2.18/en_pud-ud-test.conllu`. It verifies commit
...
`v2/manifests/model-assets.full-rehash.json` must compare byte-for-byte equal to
...
```sh
"$RUNTIME_ROOT/bin/python" -I -B v2/create_runtime_manifest.py \
  ...
  --codec-root ../codec-source \
  --output v2/.working/runtime-manifest.json
```
...
```sh
"$RUNTIME_ROOT/bin/python" -I -B v2/create_sbom.py \
  --runtime-manifest v2/.working/runtime-manifest.json \
  --asset-receipt v2/.working/asset-receipt.json \
  --output v2/.working/cyclonedx-sbom.json
```

All three outputs remain ignored development artifacts until a reviewed freeze
...
```sh
"$RUNTIME_ROOT/bin/python" -B -m v2.create_zenodo_deposit_manifest \
...
"$RUNTIME_ROOT/bin/python" -B -m v2.verify_zenodo_receipt \
...
1. Review and obtain green zero-skip CI for one exact implementation commit.
...
2. Generate `freeze-manifest.json` outside the implementation tree, then put
   ...
   a new commit, review, CI run, runtime manifest, and freeze manifest.

The generator requires the recorded approved GitHub review and successful
Actions run to name the same exact implementation commit. Those claims come
only from a canonical direct-TLS GitHub gate observation produced as described
in [`GITHUB_GATE_RECEIPTS.md`](GITHUB_GATE_RECEIPTS.md), never from
operator-entered review/CI strings. Its required `evidenceBoundary` makes clear
...
```sh
"$RUNTIME_ROOT/bin/python" -I -B v2/freeze_manifest.py create \
  --runtime-manifest v2/.working/runtime-manifest.json \
  --asset-receipt v2/.working/asset-receipt.json \
  --transport-ca-bundle v2/trust/transport-ca.pem \
  --offline-trust-manifest v2/trust/nist/manifest.json \
  ...
  --output v2/.working/freeze-manifest.json
```
...
Do not edit a copy of the draft by hand. From the same clean, detached reviewed
...
clean runtime and asset receipts, GitHub review/CI gate, CA and NIST trust
...
"$RUNTIME_ROOT/bin/python" -I -B v2/build_frozen_design.py \
  ...
  --signing-public-key v2/signing/corelm-crossmodel-v2-signing.pub \
  --cosign /absolute/path/to/pinned-cosign-v3.0.6 \
...
After creating the frozen design, independently re-open every input and verify
the stage-two binding:

```sh
"$RUNTIME_ROOT/bin/python" -I -B v2/freeze_manifest.py verify \
  --manifest v2/.working/freeze-manifest.json \
  --runtime-manifest v2/.working/runtime-manifest.json \
  --asset-receipt v2/.working/asset-receipt.json \
  --transport-ca-bundle v2/trust/transport-ca.pem \
  --offline-trust-manifest v2/trust/nist/manifest.json \
...
```sh
"$RUNTIME_ROOT/bin/python" -I -B v2/package_design_release.py package \
  --frozen-design /path/to/frozen-design.json \
  --development-control-report /path/to/development-control-report.json \
  --development-control-archive-receipt /path/to/development-control-archive-receipt.json \
  --freeze-manifest v2/.working/freeze-manifest.json \
  --github-gate-receipt /path/to/github-gate-receipt.json \
  --linux-ci-artifact /path/to/downloaded-linux-artifact.zip \
  --macos-arm64-ci-artifact /path/to/downloaded-macos-artifact.zip \
  --asset-source-manifest v2/model-assets.draft.json \
  --full-asset-receipt v2/.working/asset-receipt.json \
  --runtime-manifest v2/.working/runtime-manifest.json \
  --sbom v2/.working/cyclonedx-sbom.json \
  --signing-public-key v2/signing/corelm-crossmodel-v2-signing.pub \
  --output-root /new/path/design-release-assets

"$RUNTIME_ROOT/bin/python" -I -B v2/package_design_release.py verify \
...
```sh
"$RUNTIME_ROOT/bin/python" -I -B v2/verify_design.py
"$RUNTIME_ROOT/bin/python" -I -B v2/preflight.py \
  --codec-root ../codec-source \
  --asset-root v2/.assets \
  --asset-receipt v2/.working/asset-receipt.json \
  --require-assets
...
RUNTIME_ROOT="$PWD/v2/.runtime/macos"
RUNTIME_MANIFEST="$PWD/v2/.working/runtime-manifest.json"
DATASET="$PWD/v2/.assets/ud-english-pud-r2.18/en_pud-ud-test.conllu"
...
```sh
PYTHONHASHSEED=0 "$RUNTIME_ROOT/bin/python" -P -s -B \
  v2/package_development_control_release.py package \
...
Before `2026-08-09T00:00:00Z`, publish those exact three files in an immutable
GitHub release named by the signed annotated tag
`corelm-crossmodel-livewiki-v2-development-control`. The tag must directly
...
`v2/trust/github/trusted_root.json`. The release ID is an explicit input:

```sh
PYTHONHASHSEED=0 "$RUNTIME_ROOT/bin/python" -P -s -B \
  v2/collect_release_receipt.py \
  --repository ALLPROTO/core-lm-cross-model-lab \
  --kind development-control \
  --tag corelm-crossmodel-livewiki-v2-development-control \
  --commit "$IMPLEMENTATION_COMMIT" \
  --tree "$IMPLEMENTATION_TREE" \
  --deadline 2026-08-09T00:00:00Z \
  --signature-type SSH \
  --key-fingerprint 'SHA256:8A4y/GkoFglweSfg3rP21BtWWqIBOeQAUoAJDQM8sMM' \
  --public-key v2/signing/corelm-crossmodel-v2-signing.pub \
  --repo-path "$PWD" \
