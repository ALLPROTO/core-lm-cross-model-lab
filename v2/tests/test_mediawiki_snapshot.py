...
from v2 import mediawiki_snapshot as subject


SERVER_DATE = "Tue, 25 Aug 2026 06:00:01 GMT"
...
class MediaWikiSnapshotTests(unittest.TestCase):
    ...
    @staticmethod
    def clock() -> datetime:
        return datetime(2026, 8, 25, 6, 0, 2, tzinfo=timezone.utc)
    ...
    def collect_two_stages(
        ...
        subject.collect_crawl_stage(
            ...
            crawl_index=0,
            transport=transport,
            clock=lambda: datetime(2026, 8, 24, 6, 0, 2, tzinfo=timezone.utc),
        )
        subject.collect_crawl_stage(
            root=self.root,
            crawl_index=1,
    ...
    def test_two_durable_crawl_stages_can_run_on_separate_days(self) -> None:
        ...
        first_day = lambda: datetime(
            2026, 8, 24, 6, 0, 2, tzinfo=timezone.utc
        )
        subject.collect_crawl_stage(
            root=self.root,
            crawl_index=0,
