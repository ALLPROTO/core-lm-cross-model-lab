...
from v2.create_sbom import build_sbom
from v2.protocol import canonical_json_bytes
from v2.reproducibility import with_content_digest
from v2.state_machine import RESERVATION_FILENAME, create_attempt_marker
...
def frozen_design_fixture() -> dict[str, object]:
    ...
    design = json.loads((project_root / "v2/design-registration.draft.json").read_text())
    design["schemaVersion"] = "corelm-crossmodel-livewiki-v2-design-v1"
...
class IndependentVerifierContractTests(unittest.TestCase):
    def test_publication_inputs_share_one_explicit_cryptographic_verifier(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch(
                "v2.verify_evidence.verify_publication",
                side_effect=publications,
            ) as publication, mock.patch(
                "v2.verify_evidence.require_frozen_lab_publication_source"
            ), mock.patch(
                "v2.verify_evidence.verify_design_release_package",
    ...
    def test_private_snapshot_manifest_is_bound_to_attempt_marker_bytes(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            manifest = {
                "schemaVersion": "corelm-crossmodel-livewiki-v2-private-snapshot-manifest-v1",
                "suiteId": marker["suiteId"],
                "status": "SEALED_BEFORE_ATTEMPT",
                "createdAt": "2026-08-28T17:59:59Z",
                "countsTowardScientificVerdict": False,
    ...
    def test_independent_reservation_binding_rejects_rewritten_commitment(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            design = {
                ...
                "beacon": {"targetTimestamp": "2026-08-27T18:00:00.000Z"},
            }
            marker = create_attempt_marker(
                root,
    ...
    def test_worker_jobs_summaries_and_jsonl_are_exactly_bound(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            suite = "corelm-voidtoken-crossmodel-livewiki-v2"
            ...
            for corpus in corpora:
                for page_index, selected in enumerate(selected_pages[corpus]):
                    ...
                    for prediction_index in range(128):
                        raw_records.append(
                            {
                                "schemaVersion": "corelm-crossmodel-livewiki-v2-raw-token-evidence-v1",
                                "suiteId": suite,
                                "attemptId": attempt,
                                "modelKey": model_key,
                                "corpusProject": corpus,
                    ...
                    for layer_index in range(3):
                        container_records.append(
                            {
                                "schemaVersion": "corelm-crossmodel-livewiki-v2-container-evidence-v1",
                                "suiteId": suite,
                                "attemptId": attempt,
                                "modelKey": model_key,
                                "corpusProject": corpus,
            ...
            supervisor = {
                ...
                "completedAt": "2026-08-28T18:00:01Z",
    ...
    def test_executable_source_tree_has_no_unmanifested_or_changed_files(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (root / "v2" / "runner.py").write_bytes(source)
            manifest = {
                "files": [
                    {
                        "path": "lab/v2/runner.py",
                        "bytes": len(source),
