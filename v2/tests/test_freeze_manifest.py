...
class FreezeManifestTests(unittest.TestCase):
    ...
    def _live_git_fixture(self) -> tuple[Path, Path, dict[str, str]]:
        ...
        dependency = repository / "v2" / "release_receipt.py"
        dependency.parent.mkdir()
        dependency.write_text("RECEIPT = 'reviewed'\n", encoding="utf-8")
    ...
    def _write_assets(self) -> None:
        manifest_path = subject.V2_ROOT / "model-assets.draft.json"
    ...
    def _pud_plan_fixture(
        ...
        design = load_json_strict(subject.V2_ROOT / "design-registration.draft.json")
        ...
        configuration = {
            "schemaVersion": (
                "corelm-crossmodel-v2-real-e2e-development-configuration-v1"
            ),
            "suiteId": subject.DEVELOPMENT_SUITE_ID,
            "countsTowardScientificVerdict": False,
            "usedForCandidateSelectionOrTuning": False,
    ...
    def test_live_source_verifier_rejects_dirty_dependency_and_untracked_path(
        ...
        with mock.patch.object(subject, "PROJECT_ROOT", repository):
            ...
            untracked = repository / "v2" / "github_gate_receipt.py"
    ...
    def test_generator_is_canonical_binds_inputs_and_has_no_file_self_reference(self) -> None:
        ...
        schema = load_json_strict(
            subject.V2_ROOT / "schemas" / "freeze-manifest.schema.json"
        )
        ...
        self.assertEqual(manifest["review"]["pullRequestNumber"], GATE_PR)
        self.assertEqual(
            manifest["review"]["approvedReview"]["reviewerLogin"],
            GATE_REVIEWER,
        )
        self.assertEqual(
    ...
    def test_gate_receipt_is_reopened_and_cli_typed_review_ci_cannot_replace_it(self) -> None:
        manifest = self.build()
        gate_raw = self.gate_path.read_bytes()
        self.gate_path.write_bytes(gate_raw + b"\n")
        with self.assertRaisesRegex(subject.FreezeManifestError, "GitHub gate receipt"):
            subject.verify_artifact_inputs(
                manifest,
                runtime_manifest_path=self.runtime_path,
                asset_receipt_path=self.asset_path,
        ...
        self.assertFalse(hasattr(create, "reviewer_login"))
    ...
    def test_stage_two_design_binds_exact_manifest_file_sha(self) -> None:
        ...
        design = copy.deepcopy(
            load_json_strict(subject.V2_ROOT / "design-registration.draft.json")
        )
        design.update(
    ...
    def test_schema_exposes_exact_two_stage_and_review_ci_contract(self) -> None:
        schema = load_json_strict(
            subject.V2_ROOT / "schemas" / "freeze-manifest.schema.json"
        )
        ...
        self.assertEqual(
            schema["$defs"]["approvedReview"]["properties"]["state"]["const"],
            "APPROVED",
        )
