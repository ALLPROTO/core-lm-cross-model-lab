...
from v2 import development_corpus as records
from v2 import development_model_replay as independent
from v2 import model_worker
from v2 import run_real_e2e_control as control
from v2 import runner as scientific_runner
from v2.protocol import EXPECTED_DEVELOPMENT_CONTROLS, canonical_json_bytes
...
def _archival_inputs() -> dict[str, bytes]:
    return {
        "design-registration.draft.json": (
            control.V2_ROOT / "design-registration.draft.json"
        ).read_bytes(),
        "model-assets.draft.json": (
            control.V2_ROOT / "model-assets.draft.json"
        ).read_bytes(),
        "model-assets.full-rehash.json": (
            control.V2_ROOT / "manifests" / "model-assets.full-rehash.json"
        ).read_bytes(),
        "development-corpus.draft.json": (
            control.V2_ROOT / "development-corpus.draft.json"
        ).read_bytes(),
...
class RealE2EDevelopmentBoundaryTests(unittest.TestCase):
    ...
    def test_output_is_forbidden_under_scientific_results_and_repository(self) -> None:
        forbidden = control.V2_ROOT / "results" / "development-e2e-forbidden"
        with self.assertRaisesRegex(
            control.DevelopmentControlError,
            "outside the repository and v2/results",
        ):
    ...
    def test_scientific_worker_contract_remains_distinct_and_valid(self) -> None:
        design = json.loads(
            (control.V2_ROOT / "design-registration.draft.json").read_bytes()
        )
