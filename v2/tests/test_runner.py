...
import v2.runner as runner_module
from v2.protocol import canonical_json_bytes, load_json_strict, sha256_bytes
from v2.reproducibility import digest_regular_file, with_content_digest
...
def frozen_design() -> dict[str, object]:
    value = copy.deepcopy(load_json_strict(V2_ROOT / "design-registration.draft.json"))
    value["schemaVersion"] = "corelm-crossmodel-livewiki-v2-design-v1"
...
class RunnerContractTests(unittest.TestCase):
    ...
    def test_registered_ci_workflow_bytes_are_reopened_exactly(self) -> None:
        ...
        workflow = (V2_ROOT.parent / design["continuousIntegration"]["workflowPath"]).read_bytes()
    ...
    def test_scientific_python_probe_proves_seed_and_safe_startup(self) -> None:
        ...
        if resolved_base != Path(os.path.abspath(sys.executable)):
            ...
            with (
                mock.patch(
                    "v2.runner._scientific_python_command",
        ...
        with (
            mock.patch("v2.runner.SCIENTIFIC_PYTHON_FLAGS", ("-I", "-B")),
    ...
    def test_runtime_live_recomputes_runtime_and_base_python_trees(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                mock.patch("v2.runner.platform.system", return_value="Darwin"),
                mock.patch("v2.runner.platform.machine", return_value="arm64"),
                mock.patch(
                    "v2.runner.scan_tree", side_effect=[runtime_tree, base_tree]
                ) as scanner,
            ):
    ...
    def test_runtime_live_rejects_non_primary_manifest_and_live_hosts(self) -> None:
        ...
        with (
            mock.patch("v2.runner.platform.system", return_value="Linux"),
            mock.patch("v2.runner.platform.machine", return_value="x86_64"),
    ...
    def test_supervisor_socket_guard_is_explicitly_not_child_capability_isolation(self) -> None:
        script = r'''from v2.runner import RunnerError, install_trusted_supervisor_socket_denial
import socket
import subprocess
import sys
install_trusted_supervisor_socket_denial()
...
'''
    ...
    def test_one_shot_window_is_half_open_and_pre_marker(self) -> None:
        ...
        with self.assertRaises(RunnerError):
            ensure_one_shot_window(datetime(2026, 8, 28, 18))

    def test_nist_https_date_is_an_external_half_open_start_anchor(self) -> None:
        self.assertEqual(
            verify_external_attempt_time_anchor(
                {"responseDate": "2026-08-28T18:00:00Z"}
            ),
            ONE_SHOT_NOT_BEFORE,
        )
        with self.assertRaisesRegex(RunnerError, "outside"):
            verify_external_attempt_time_anchor(
                {"responseDate": "2026-08-28T17:59:59Z"}
            )
        with self.assertRaisesRegex(RunnerError, "outside"):
            verify_external_attempt_time_anchor(
                {"responseDate": "2026-08-29T18:00:00Z"}
            )

    def test_attempt_id_contract_is_exact(self) -> None:
        self.assertIsNotNone(
            ATTEMPT_ID.fullmatch("20260828T180000Z-0123456789abcdef")
        )
    ...
    def test_private_snapshot_rejects_unmanifested_and_tampered_bytes(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch("v2.runner.verify_copied_source") as verifier:
            ...
            with mock.patch("v2.runner.verify_copied_source"):
                with self.assertRaises(RunnerError):
