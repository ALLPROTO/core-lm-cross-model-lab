...
from v2 import package_design_release as subject
...
from v2.release_attestation_crypto import expected_known_answer_result
from v2.reproducibility import (
    canonical_json_bytes,
    sha256_bytes,
    with_content_digest,
)
from v2.tests import test_freeze_manifest as freeze_fixture_module
from v2.tests.test_github_gate_receipt import (
    PR as GATE_PR,
    REVIEWER as GATE_REVIEWER,
...
class PackageDesignReleaseTests(unittest.TestCase):
    ...
    def _ci_members(
        ...
        *,
        ...
        design_raw = (subject.V2_ROOT / "design-registration.draft.json").read_bytes()
        ...
        design_check = {
            ...
            "modelAssetManifestFileSHA256": sha256_bytes(
                (subject.V2_ROOT / "model-assets.draft.json").read_bytes()
            ),
        ...
        log = (
            b"test_contract (v2.tests.UnitContract.test_contract) ... ok\n"
            b"\n----------------------------------------------------------------------\n"
            b"Ran 1 test in 0.001s\n\nOK\n"
            b"ZERO-SKIP POLICY PASS: 1 tests, 0 skipped\n"
        )
        return {
            f"v2-preflight-{suffix}.json": self._pretty(preflight),
            f"v2-runtime-{suffix}.json": self._ci_runtime(
                role=role,
                system=system,
                machine=machine,
                platform_tag=platform_tag,
            ),
            f"v2-zero-skip-{suffix}.log": log,
            f"v2-design-check-{suffix}.json": self._pretty(design_check),
            f"v2-release-attestation-known-answer-{suffix}.json": self._pretty(
    ...
    def setUp(self) -> None:
        ...
        self.asset_source_path = subject.V2_ROOT / "model-assets.draft.json"
        ...
        draft = load_json_strict(subject.V2_ROOT / "design-registration.draft.json")
        self.draft = draft
        ...
        key_raw = (
            subject.V2_ROOT / "signing" / "corelm-crossmodel-v2-signing.pub"
        ).read_bytes()
    ...
    def test_package_is_deterministic_exact_read_only_and_self_excluding(self) -> None:
        ...
        ci_schema = load_json_strict(
            subject.V2_ROOT / "schemas" / "ci-artifact-verification.schema.json"
        )
    ...
    def test_ci_zip_digest_inventory_platform_workflow_and_zero_skip_fail_closed(self) -> None:
        ...
        preflight_name = "v2-preflight-linux.json"
        ...
        runtime_name = "v2-runtime-linux.json"
        ...
        macos_runtime_name = "v2-runtime-macos.json"
        ...
        design_check_name = "v2-design-check-linux.json"
        ...
        skip_members["v2-zero-skip-linux.log"] = (
            b"Ran 1 test in 0.001s\n\nOK (skipped=1)\n"
            b"ZERO-SKIP POLICY PASS: 1 tests, 0 skipped\n"
        )
