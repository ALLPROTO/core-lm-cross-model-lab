...
from v2.release_receipt import REQUIRED_ASSET_ROLES, ReleaseReceiptError
from v2.release_attestation_crypto import expected_known_answer_result
from v2.reproducibility import canonical_json_bytes, with_content_digest
from v2.tests.test_release_receipt import _fixture_crypto_record
from v2.zenodo_archive import (
    EVIDENCE_BOUNDARY,
    MANIFEST_FILE_NAME,
    HTTPSCapture,
    ZenodoArchiveError,
...
GATE_REVIEWER = "independent-reviewer"
LAB_FILES = {
    "README.md": (0o644, b"laboratory source\n"),
    "v2/verifier.py": (0o755, b"#!/usr/bin/env python3\nprint('verified')\n"),
}
...
def _gate_endpoints() -> dict[str, str]:
    return {
        ...
        "reviews": f"{GATE_API_BASE}/pulls/{GATE_PR}/reviews?per_page=100&page=1",
    }
...
def _gate_bodies() -> dict[str, Any]:
    ...
    return {
        ...
        "reviews": [
            {
                "id": 71001,
                "user": {"login": GATE_REVIEWER, "type": "User"},
                "state": "APPROVED",
                "commit_id": LAB_COMMIT,
...
class ZenodoFixture:
    def __init__(self, root: Path) -> None:
        ...
        def ci_zip(system: str, machine: str, suffix: str) -> bytes:
            ...
            return _zip_bytes(
                {
                    f"v2-preflight-{suffix}.json": canonical_json_bytes(preflight) + b"\n",
                    f"v2-runtime-{suffix}.json": runtime(system, machine),
                    f"v2-zero-skip-{suffix}.log": (
                        b"Ran 42 tests in 1.0s\nOK\n"
                        b"ZERO-SKIP POLICY PASS: 42 tests, 0 skipped\n"
                    ),
                    f"v2-design-check-{suffix}.json": canonical_json_bytes(check) + b"\n",
        ...
        linux_artifact_name = f"blind-v2-linux-development-{RUN_ID}-1"
        macos_artifact_name = f"blind-v2-macos-development-{RUN_ID}-1"
        ...
        development_tag = "corelm-crossmodel-livewiki-v2-development-control"
...
class ZenodoArchiveTests(unittest.TestCase):
    ...
    def test_closeout_release_attestation_must_precede_its_own_deadline(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch(
                "v2.zenodo_archive.verify_attestation_record",
    ...
    def test_manifest_semantically_verifies_citation_source_and_ci_bytes(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (fixture.deposit / "linux-ci-artifact.zip").write_bytes(
                _zip_bytes({"v2-zero-skip-linux.log": b"fabricated PASS\n"})
            )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            known_answer_name = "v2-release-attestation-known-answer-linux.json"
            ...
            runtime = json.loads(members["v2-runtime-linux.json"])
            with self.assertRaisesRegex(
