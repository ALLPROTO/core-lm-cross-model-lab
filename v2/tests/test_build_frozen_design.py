...
from v2 import build_frozen_design as subject
...
from v2.reproducibility import canonical_json_bytes, sha256_bytes, with_content_digest
from v2.tests import test_freeze_manifest as freeze_fixture
...
class FrozenDesignBuilderTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.helper = helper
        self.lab_root = helper.root / "reviewed-lab"
        (self.lab_root / "v2").mkdir(parents=True)
        self.draft_raw = (subject.V2_ROOT / "design-registration.draft.json").read_bytes()
        ...
        self._git("commit", "-q", "-m", "reviewed fixture")
        ...
        for review in bodies["reviews"]:
            review["commit_id"] = self.commit
    ...
    def _arguments(self, **overrides: object) -> dict[str, object]:
        arguments: dict[str, object] = {
            ...
            "signing_public_key_path": subject.V2_ROOT
            / "signing/corelm-crossmodel-v2-signing.pub",
