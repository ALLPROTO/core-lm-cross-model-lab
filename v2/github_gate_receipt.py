#!/usr/bin/env python3
"""Structural verification of an archived GitHub review and Actions observation.
...
SCHEMA_VERSION = "corelm-github-gate-receipt-v1"
...
REQUIRED_WORKFLOW_NAME = "Blind v2 development controls"
REQUIRED_WORKFLOW_PATH = ".github/workflows/v2-development-controls.yml"
...
REQUIRED_LINUX_ARTIFACT_PREFIX = "blind-v2-linux-development-"
REQUIRED_MACOS_ARTIFACT_PREFIX = "blind-v2-macos-development-"
...
class GitHubGateReceiptError(ValueError):
    """The archived review/CI gate is incomplete, stale, or inconsistent."""
...
def _expected_endpoints(
    *,
    ...
    return {
        ...
        "reviews": f"{api_base}/pulls/{pr_number}/reviews?per_page=100&page=1",
    }
...
def _validate_responses(
    ...
    *,
    ...
    if not isinstance(value, list) or len(value) != len(API_ROLES):
        raise GitHubGateReceiptError("exactly five GitHub API responses are required")
    bodies: dict[str, Any] = {}
    dates: list[datetime] = []
    for index, raw_record in enumerate(value):
        ...
        if role in {"reviews", "workflow-jobs", "workflow-artifacts"} and fields.get("link"):
...
def _review_time(review: Mapping[str, Any]) -> tuple[datetime, int]:
    submitted = _utc(review.get("submitted_at"), label="review submitted_at")
    return submitted, _positive_integer(review.get("id"), label="review ID")
...
def _verify_reviews(
    ...
    *,
    ...
    if not isinstance(body, list) or not body or len(body) > 100:
        raise GitHubGateReceiptError("review inventory is empty, oversized, or not one page")
    if LOGIN.fullmatch(reviewer) is None or reviewer.casefold() == repository_owner.casefold():
        raise GitHubGateReceiptError("reviewer is not an independent account login")
    ids: set[int] = set()
    substantive_by_login: dict[str, list[Mapping[str, Any]]] = {}
    for raw in body:
        if not isinstance(raw, dict):
            raise GitHubGateReceiptError("review record is not an object")
        review_id = _positive_integer(raw.get("id"), label="review ID")
        if review_id in ids:
            raise GitHubGateReceiptError("review ID is duplicated")
        ids.add(review_id)
        user = raw.get("user")
        login = user.get("login") if isinstance(user, dict) else None
        if not isinstance(login, str) or LOGIN.fullmatch(login) is None:
            raise GitHubGateReceiptError("review login is invalid")
        if user.get("type") != "User":
            raise GitHubGateReceiptError("bot or organization reviews are not independent")
        if raw.get("pull_request_url") != f"{api_base}/pulls/{pr_number}":
            raise GitHubGateReceiptError("review belongs to another pull request")
        state = raw.get("state")
        if state not in {"APPROVED", "CHANGES_REQUESTED", "COMMENTED", "DISMISSED"}:
            raise GitHubGateReceiptError("review state is unsupported or pending")
        _review_time(raw)
        if state in {"APPROVED", "CHANGES_REQUESTED", "DISMISSED"}:
            substantive_by_login.setdefault(login.casefold(), []).append(raw)
    candidates = substantive_by_login.get(reviewer.casefold(), [])
    if not candidates:
        raise GitHubGateReceiptError("expected independent reviewer has no substantive review")
    selected = max(candidates, key=_review_time)
    user = selected["user"]
    if (
        user.get("login") != reviewer
        or selected.get("state") != "APPROVED"
        or selected.get("commit_id") != commit
        or selected.get("body") != REQUIRED_REVIEW_DECLARATION
    ):
        raise GitHubGateReceiptError(
            "latest independent review is not APPROVED on the exact implementation "
            "commit with the registered declaration"
        )
    selected_key = _review_time(selected)
    for reviews in substantive_by_login.values():
        latest = max(reviews, key=_review_time)
        if latest.get("state") == "CHANGES_REQUESTED":
            raise GitHubGateReceiptError("an active changes-requested review remains")
    for raw in body:
        if _review_time(raw) > selected_key and raw.get("state") in {
            "CHANGES_REQUESTED",
            "DISMISSED",
        }:
            raise GitHubGateReceiptError("a later review invalidates the selected approval")
    return selected
...
def verify_github_gate_receipt(
    ...
    *,
    ...
    if not isinstance(expected_reviewer_login, str) or LOGIN.fullmatch(expected_reviewer_login) is None:
        raise GitHubGateReceiptError("expected reviewer login is invalid")
    if (
    ...
    if capture_policy != {
        ...
        "reviewSnapshotIsTimeBounded": True,
    ...
    review_gate = _mapping(
        root["reviewGate"],
        {"reviewId", "reviewerLogin", "state", "commit", "submittedAt"},
        label="review gate",
    )
    if review_gate != {
        "reviewId": selected["id"],
        "reviewerLogin": expected_reviewer_login,
        "state": "APPROVED",
        "commit": commit,
        "submittedAt": selected["submitted_at"],
    }:
        raise GitHubGateReceiptError("derived review gate differs from archived API bytes")
    ci_gate = _mapping(
        root["ciGate"],
        {
            "runId",
    ...
    return VerifiedGitHubGateReceipt(
        ...
        reviewer_login=expected_reviewer_login,
        review_id=selected["id"],
        review_submitted_at=selected["submitted_at"],
