#!/usr/bin/env python3
"""Hardened, auditable MediaWiki collection for blind-v2.

This module implements the request and byte-commitment rules from the local
v2 preregistration draft.  It does not import model weights, the codec, Torch,
...
from v2.protocol import canonical_json_bytes, load_json_strict_bytes, sha256_bytes
...
CRAWL_NOT_BEFORE = (
    ...
    datetime(2026, 8, 25, 6, tzinfo=timezone.utc),
)
MINIMUM_ELIGIBLE_PER_PROJECT = 64
RECORD_MAGIC = b"CORELM-LIVEWIKI-V2-RECORD\0"
MANIFEST_SCHEMA = "corelm-crossmodel-livewiki-v2-corpus-manifest-v1"
CRAWL_STAGE_SCHEMA = "corelm-crossmodel-livewiki-v2-crawl-stage-v1"
MAX_HTTP_WIRE_BYTES = 64 * 1024 * 1024
MAX_JSON_BODY_BYTES = 32 * 1024 * 1024
MAX_CRAWL_PAGES = 100_000
USER_AGENT = (
    "CoreLMBlindV2Collector/1.0 "
...
def collect_crawl_stage(
    *,
    ...
    Stage 1 can be run on/after 2026-08-24T06:00Z.  Stage 2 requires a fully
    verified stage 1 and can be run on/after 2026-08-25T06:00Z.  A partial
...
def verify_corpus_snapshot(
    ...
    if manifest["suiteId"] != "corelm-voidtoken-crossmodel-livewiki-v2":
