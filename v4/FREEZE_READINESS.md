# Blind-v4 author-self-verification freeze readiness gate
...
V4 is operated under `AUTHOR_SELF_VERIFICATION`: the repository owner is the
author, experiment operator, and release operator. No independent human review,
peer review, operator blindness, or independent replication is claimed. These
disclosures are immutable design fields, not optional prose.
...
- [ ] The v3 contour is recorded as a non-scientific failed-freeze archive;
  none of its report, tag, release, receipt identity, or source commit is reused
  as v4 evidence. The earlier v2 contour is recorded as a superseded, unfrozen
  draft. Its
  completed UD English PUD E2E is disclosed as a prior non-scientific
  observation and is not reused as v4 freeze evidence. The complete three-model
  producer-to-VTL5-to-real-model-replay control is rerun on the exact v4
  implementation and locked runtime without changing the candidate or protocol
  in response to the prior diagnostics.
...
- [ ] The report and complete deterministic artifact ZIP are published in the
  immutable `corelm-crossmodel-livewiki-v4-development-control` release from a
  signed annotated tag targeting that same commit. A canonical v4 receipt binds
  all three release assets and requires the GitHub immutable-release RFC3161
  `attestedAt` strictly before `2026-09-07T00:00:00Z`; local files,
...
- [ ] Release-receipt collection and independent replay both use exact pinned
  ...
  CI-only API receipt described below. The tracked real, non-synthetic
...
- [ ] The GitHub CI-only collector archives exactly four responses—PR,
  workflow run, all jobs, and artifacts—within its bounded TLS observation. It
  makes no reviews-endpoint request and carries no reviewer identity, approval
  state, or review declaration. The PR head, successful run, every successful
  zero-skip job, both platform identities, and artifact digests all bind the
  same exact implementation commit.
- [ ] The frozen design records
  ...
  `independentReplicationClaimed=false`. No self-review or second
  author-controlled account may be represented as external validation.
...
- [ ] Replace the current development NIST leaf, which expires at
  `2026-09-04T23:59:59Z`, with a complete offline leaf, intermediate, and root
  chain valid at the registered `2026-09-25T18:00:00.000Z` pulse. Recompute and
  bind the trust-manifest digest before freeze, revalidate the transport CA and
  hostname, and pass the non-fixture verifier at the exact target time. The
  design separately pins the allowed root DER hash; trusting a self-consistent
  manifest alone is forbidden. No replacement is allowed after observing the
  pulse.
...
- [ ] The two-stage freeze manifest re-opens all inputs and binds archived
  CI-only observations together with their structural-only evidence boundary.
  It must not represent offline replay as GitHub-origin attestation. The freeze
  manifest and frozen design are constructed
  outside the exact implementation Git tree as release assets; the frozen design binds the
...
- [ ] The design publication uses a signed annotated tag and immutable GitHub
  release. The tag targets the exact author-self-verified and CI-successful implementation
...
- [ ] The public design release is available strictly before
  `2026-09-07T00:00:00Z`; local time, draft PR time, commit author time, or an
...
`2026-09-06T12:00:00Z`, do not publish or label the current draft as frozen.
...
The replacement must receive a new exact implementation commit/tree, author
self-verification, green exact-commit CI, freeze manifest, signed tag, release
receipt, and known-answer date
