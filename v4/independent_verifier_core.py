#!/usr/bin/env python3
"""Second implementation of the blind-v4 scientific verification rules.
...
SELECTION_DOMAIN = b"corelm-voidtoken-crossmodel-livewiki-v4-author-verified/select\0"
TARGET_UNIX_MILLISECONDS = 1790359200000
TARGET_ENDPOINT = (
...
SNAPSHOT_NOT_BEFORE = datetime(2026, 9, 23, 6, tzinfo=timezone.utc)
SNAPSHOT_DEADLINE = datetime(2026, 9, 24, 18, tzinfo=timezone.utc)
...
NIST_VERIFY_SCHEMA = "corelm-crossmodel-livewiki-v4-nist-verification-v1"
SELECTION_SCHEMA = "corelm-crossmodel-livewiki-v4-selection-v1"
INDEPENDENT_RESULT_SCHEMA = (
    "corelm-crossmodel-livewiki-v4-independent-verification-v1"
)
...
def _validate_snapshot(snapshot: Any, *, allow_fixture: bool) -> bool:
    ...
    if snapshot.get("schemaVersion") == "corelm-crossmodel-livewiki-v4-snapshot-fixture-v1":
        expected = {
    ...
    if snapshot["schemaVersion"] != "corelm-crossmodel-livewiki-v4-snapshot-registration-v1":
...
def _parse_certificate(der: bytes) -> _Certificate:
    ...
    if not fields or fields[0].tag != 0xA0:
        raise IndependentVerificationError("X.509 v4 version field is absent")
    version_fields = _der_children(der, fields[0])
    if len(version_fields) != 1 or _der_integer(der, version_fields[0]) != 2:
        raise IndependentVerificationError("certificate is not X.509 v4")
    index = 1
