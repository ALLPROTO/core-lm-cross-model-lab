...
from v4.protocol import load_json_strict_bytes
...
from v4.github_release_attestation import (
    ReleaseAttestationError,
    verify_attestation_record,
)
from v4.release_receipt import (
...
from v4.source_archive import SourceArchiveError, verify_source_archive
...
CI_PAYLOADS = {
    "linux-ci-artifact": {
        ...
        "machine": "x86_64",
        "files": {
            "v4-preflight-linux.json",
            "v4-runtime-linux.json",
            "v4-zero-skip-linux.log",
            "v4-design-check-linux.json",
    ...
    "macos-arm64-ci-artifact": {
        ...
        "machine": "arm64",
        "files": {
            "v4-preflight-macos.json",
            "v4-runtime-macos.json",
            "v4-zero-skip-macos.log",
            "v4-design-check-macos.json",
            "v4-release-attestation-known-answer-macos.json",
        },
        "suffix": "macos",
...
def _github_gate_summary(
    ...
    *,
    ...
    linux = [name for name in artifacts if name.startswith("author-v4-linux-development-")]
    macos = [name for name in artifacts if name.startswith("author-v4-macos-development-")]
...
def _verify_ci_payload(
    ...
    *,
    ...
    _verify_runtime_manifest(
        payloads[f"v4-runtime-{suffix}.json"],
    ...
    preflight = _json_object(
        payloads[f"v4-preflight-{suffix}.json"], label=f"{role} preflight"
    )
    platform = preflight.get("platformSafety")
    codec = preflight.get("codec")
    if (
        preflight.get("schemaVersion") != "corelm-crossmodel-livewiki-v4-preflight-v1"
    ...
    check = _json_object(
        payloads[f"v4-design-check-{suffix}.json"], label=f"{role} design check"
    )
    if (
        check.get("schemaVersion") != "corelm-crossmodel-livewiki-v4-design-check-v1"
    ...
    known_answer = _json_object(
        payloads[f"v4-release-attestation-known-answer-{suffix}.json"],
    ...
    try:
        log = payloads[f"v4-zero-skip-{suffix}.log"].decode("utf-8", "strict")
