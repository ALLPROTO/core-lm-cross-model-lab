#!/usr/bin/env python3
"""Hardened, auditable MediaWiki collection for blind-v4.

This module implements the request and byte-commitment rules from the local
v4 preregistration draft.  It does not import model weights, the codec, Torch,
...
from v4.protocol import canonical_json_bytes, load_json_strict_bytes, sha256_bytes
...
CRAWL_NOT_BEFORE = (
    datetime(2026, 9, 22, 6, tzinfo=timezone.utc),
    datetime(2026, 9, 23, 6, tzinfo=timezone.utc),
)
MINIMUM_ELIGIBLE_PER_PROJECT = 64
RECORD_MAGIC = b"CORELM-LIVEWIKI-V4-RECORD\0"
MANIFEST_SCHEMA = "corelm-crossmodel-livewiki-v4-corpus-manifest-v1"
CRAWL_STAGE_SCHEMA = "corelm-crossmodel-livewiki-v4-crawl-stage-v1"
MAX_HTTP_WIRE_BYTES = 64 * 1024 * 1024
MAX_JSON_BODY_BYTES = 32 * 1024 * 1024
MAX_CRAWL_PAGES = 100_000
USER_AGENT = (
    "CoreLMBlindV4Collector/1.0 "
...
def collect_crawl_stage(
    *,
    ...
    Stage 1 can be run on/after 2026-09-22T06:00Z.  Stage 2 requires a fully
    verified stage 1 and can be run on/after 2026-09-23T06:00Z.  A partial
...
def verify_corpus_snapshot(
    ...
    if manifest["suiteId"] != "corelm-voidtoken-crossmodel-livewiki-v4-author-verified":
