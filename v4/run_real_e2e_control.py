...
inputs, never creates a reservation/marker, never writes ``v4/results``, never
...
from v4.create_asset_receipt import build_asset_receipt  # noqa: E402
from v4.development_model_replay import (  # noqa: E402
    CANDIDATE,
    CORPORA,
    DATASET_BYTES,
...
from v4.development_runtime import (  # noqa: E402
...
def _external_output_path(output: Path) -> tuple[Path, str]:
    ...
    if parent == project or parent.is_relative_to(project):
        raise DevelopmentControlError(
            "development output must be outside the repository and v4/results"
        )
...
def verify_codec(codec_root: Path, design: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(source, dict):
        raise DevelopmentControlError("v4 codec source binding is absent")
    try:
...
def _load_fixed_inputs(
    *,
    ...
    design_path = V4_ROOT / "design-registration.draft.json"
    manifest_path = V4_ROOT / "model-assets.draft.json"
    receipt_path = V4_ROOT / "manifests" / "model-assets.full-rehash.json"
    corpus_manifest_path = V4_ROOT / "development-corpus.draft.json"
    ...
    design = _canonical_document(design_raw, label="v4 design registration")
    manifest = _canonical_document(manifest_raw, label="v4 model asset manifest")
    receipt = _canonical_document(receipt_raw, label="v4 full asset receipt")
    ...
    if tuple(item.get("key") for item in design.get("models", [])) != MODELS:
        raise DevelopmentControlError("v4 model order differs from development control")
    candidate = design.get("candidate")
    if not isinstance(candidate, dict) or {
        key: candidate.get(key) for key in CANDIDATE
    } != CANDIDATE:
        raise DevelopmentControlError("v4 candidate differs from development control")
...
def _run_control(
    ...
    with tempfile.TemporaryDirectory(
        ...
        command, backend = _development_child_command(
            V4_ROOT / "development_model_replay.py",
            "--evidence-root",
            str(output_root),
            "--private-root",
            str(private_root),
