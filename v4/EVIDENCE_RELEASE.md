...
recomputation report produced after a complete terminal attempt. A reservation/marker-only
`CONSUMED_INCOMPLETE` package legitimately lacks the latter and must never be
given a placeholder scientific report.
...
- `payload/attempt/`: the entire one-shot result root, including the durable
  reservation and marker, raw NIST request/response, selection, environment, jobs, all worker
  logs and summaries, top-level and per-worker page-token evidence, per-token
  losses/IDs, container evidence and container bytes, producer result,
  independent-verifier report/log, and terminal outcome when they exist;
...
- `payload/bindings/`: the frozen design, snapshot registration, freeze
  ...
  receipt, CycloneDX SBOM, canonical design and snapshot publication receipts,
  the preregistered SSH signing public key, every design/snapshot release asset,
  transport CA bundle, and complete offline NIST certificate-chain directory.
...
after publishing the reservation and before publishing a canonical marker or
terminal outcome, the package
...
The packager validates frozen bindings against the canonical reservation when
no canonical marker exists; it never parses partial bytes as evidence and
never turns them into retry permission.
...
```sh
python3 -I -B v4/package_evidence_release.py package \
...
python3 -I -B v4/package_evidence_assets.py verify \
...
```sh
python3 -I -B v4/package_evidence_assets.py extract \
  --asset-root /absolute/path/to/evidence-release-assets \
...
3. Create the dedicated evidence tag/release targeting the exact frozen
   ...
   strictly before `2026-09-30T18:00:00Z`. Do not replace a failed/partial
