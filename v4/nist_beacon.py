...
from v4.mediawiki_snapshot import (
...
def parse_certificate_der(der: bytes) -> ParsedCertificate:
    ...
    if not tbs_children or tbs_children[0].tag != 0xA0:
        raise BeaconVerificationError("X.509 v4 version field is absent")
    version_fields = _children(der, tbs_children[0])
    if len(version_fields) != 1 or _der_integer(der, version_fields[0]) != 2:
        raise BeaconVerificationError("certificate is not X.509 v4")
    index = 1
...
def load_offline_trust_bundle(
    ...
    *,
    ...
    fields = {"schemaVersion", "status", "fixtureOnly", "certificates"}
    ...
    expected_status = "KNOWN_ANSWER_FIXTURE_ONLY" if fixture_only else "FROZEN_OFFLINE_TRUST_BUNDLE"
    ...
    if not isinstance(certificates, dict) or not certificates:
        raise BeaconVerificationError("NIST trust bundle has no certificates")
