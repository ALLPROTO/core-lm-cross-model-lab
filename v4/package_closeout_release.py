...
from v4.experiment_closeout import (
...
REPORT_SCHEMA = "corelm-crossmodel-livewiki-v4-closeout-verifier-report-v1"
MANIFEST_SCHEMA = "corelm-crossmodel-livewiki-v4-closeout-release-manifest-v1"
...
BASIS_SCHEMA = "corelm-crossmodel-livewiki-v4-closeout-basis-v1"
...
def _bindings_from_args(args: argparse.Namespace) -> PublicationBindings:
    return PublicationBindings.from_exact_bytes(
        design_registration=_safe_read(args.design_registration, label="design registration"),
        design_publication_receipt=_safe_read(
            args.design_publication_receipt, label="design publication receipt"
        ),
        snapshot_registration=_safe_read(
            args.snapshot_registration, label="snapshot registration"
        ),
        snapshot_publication_receipt=_safe_read(
            args.snapshot_publication_receipt, label="snapshot publication receipt"
        ),
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        common = {
            ...
            "cryptographic_attestation_verifier": (
                PinnedCosignReleaseAttestationVerifier(args.cosign)
            ),
        }
