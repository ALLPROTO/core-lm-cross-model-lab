# Cross-model live-corpus v4 author-self-verified — normative protocol draft
...
used by that control. This document becomes a preregistration only when the
fail-closed freeze validator binds it into a signed, immutable public design
release after every author-self-verification, exact-commit CI, real-data E2E,
signing, packaging, and verifier gate passes.

The v3 contour is a **non-scientific failed-freeze archive**. Its immutable
report, tag, release, receipt identity, and source commit are retained for
transparency but cannot satisfy any v4 gate. The earlier v2 contour is a
**superseded, unfrozen draft**. Its completed UD
English PUD producer-to-VTL5-to-real-model-replay E2E is a prior non-scientific
development observation. It remains preserved and disclosed, but it is not v4
freeze evidence, is not a scientific attempt, and cannot satisfy any v4 gate.
V4 must repeat the real-data E2E on its own exact commit and locked runtime.
...
```text
corelm-voidtoken-crossmodel-livewiki-v4-author-verified
```
...
`v4/.assets/ud-english-pud-r2.18/en_pud-ud-test.conllu`.
...
The currently tracked NIST leaf is a development known-answer only: it expires
at `2026-09-04T23:59:59Z` and is invalid at the proposed
`2026-09-25T18:00:00.000Z` pulse. This suite must remain
`DRAFT_NOT_PREREGISTERED` until a replacement chain valid at that pulse is
archived and independently verified, and its new manifest digest is propagated
through the design, schemas, freeze manifest, and tests. Trust cannot be
changed after the pulse is observed.

The design registration must be published as an immutable GitHub release under
the proposed tag `corelm-crossmodel-livewiki-v4-design` strictly before
`2026-09-07T00:00:00Z`. The signed annotated tag must target the exact
...
bundle offline against `v4/trust/github/trusted_root.json`; the independent
...
strictly earlier than `2026-09-07T00:00:00Z`; `published_at`, HTTP `Date`, and
...
`.github/workflows/v4-development-controls.yml`: exactly 13,560 bytes with
SHA-256
`cc68011dc189b4616cece9376adb077f769f7d3405f40b32cfacbe57a663714e`.
...
```text
[2026-09-08T00:00:00Z, 2026-09-22T00:00:00Z)
```
...
action=query
list=recentchanges
rcstart=2026-09-08T00:00:00Z
rcend=2026-09-22T00:00:00Z
rcdir=newer
rctype=new
rcnamespace=0
rcshow=!bot|!redirect
...
`2026-09-22T06:00:00Z` and the second no earlier than
`2026-09-23T06:00:00Z`. Each crawl follows `rccontinue` until exhaustion. The
...
```text
ASCII("CORELM-LIVEWIKI-V4-RECORD\0") ||
...
`corelm-crossmodel-livewiki-v4-snapshot`. Its signed annotated tag MUST target
...
strictly earlier than `2026-09-24T18:00:00Z`. The exact pre-publication snapshot-registration bytes
...
Unix milliseconds: 1790359200000
https://beacon.nist.gov/beacon/2.0/pulse/time/1790359200000
```
...
```text
corelm-voidtoken-crossmodel-livewiki-v4-author-verified/select\0
```
...
real-model replay in `v4/independent_model_replay.py`. That module imports none
...
frozen exact-commit code launches only the registered workers and verifier after this
...
The one-shot may not start before `2026-09-26T18:00:00Z`, twenty-four hours
...
`2026-09-27T18:00:00Z`. Marker creation first uses the host UTC clock; after
...
`2026-09-30T18:00:00Z`. The evidence annotated tag MUST also target the exact
...
`corelm-crossmodel-livewiki-v4-closeout` and published as an immutable release
strictly before `2026-10-07T18:00:00Z`. The closeout release contains exactly
...
The tracked `v4/` contour must contain the design registration, model manifest,
...
`2026-09-06T12:00:00Z`, this suite must remain an unfrozen draft. A replacement
