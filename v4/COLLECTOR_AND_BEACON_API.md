...
This v4 contract is author-self-verified. It claims no independent human
...
This document is the supervisor contract for the blind-v4 corpus collector and
...
1. On or after `2026-09-22T06:00:00Z`, call
   `collect_crawl_stage(root=..., crawl_index=0, transport=..., clock=...)` with
   a new or empty root.
2. On or after `2026-09-23T06:00:00Z`, call the same function with
...
```sh
python3 -I -B v4/build_snapshot_registration.py \
...
checks X.509 v4 framing, validity, Basic Constraints, Key Usage,
...
`v4/test-vectors/nist-chain1-pulse1.json` and its certificate are a historical
...
- The target `2026-09-25T18:00:00.000Z` pulse does not exist yet.
...
```sh
python3 -m unittest v4.tests.test_mediawiki_snapshot v4.tests.test_nist_beacon -v
```
