...
from v4 import package_design_release as subject
from v4.collect_github_gate_receipt import collect_github_gate_receipt_to_path
from v4.create_sbom import build_sbom
from v4.protocol import load_json_strict
from v4.release_attestation_crypto import expected_known_answer_result
from v4.reproducibility import (
    canonical_json_bytes,
    sha256_bytes,
    with_content_digest,
)
from v4.tests import test_freeze_manifest as freeze_fixture_module
...
class PackageDesignReleaseTests(unittest.TestCase):
    ...
    def _ci_members(
        ...
        *,
        ...
        design_raw = (subject.V4_ROOT / "design-registration.draft.json").read_bytes()
        design_sha256 = sha256_bytes(design_raw)
        preflight = {
            "schemaVersion": "corelm-crossmodel-livewiki-v4-preflight-v1",
        ...
        design_check = {
            "schemaVersion": "corelm-crossmodel-livewiki-v4-design-check-v1",
            ...
            "modelAssetManifestFileSHA256": sha256_bytes(
                (subject.V4_ROOT / "model-assets.draft.json").read_bytes()
            ),
        ...
        log = (
            b"test_contract (v4.tests.UnitContract.test_contract) ... ok\n"
            b"\n----------------------------------------------------------------------\n"
            b"Ran 1 test in 0.001s\n\nOK\n"
            b"ZERO-SKIP POLICY PASS: 1 tests, 0 skipped\n"
        )
        return {
            f"v4-preflight-{suffix}.json": self._pretty(preflight),
            f"v4-runtime-{suffix}.json": self._ci_runtime(
                role=role,
                system=system,
                machine=machine,
                platform_tag=platform_tag,
            ),
            f"v4-zero-skip-{suffix}.log": log,
            f"v4-design-check-{suffix}.json": self._pretty(design_check),
            f"v4-release-attestation-known-answer-{suffix}.json": self._pretty(
    ...
    def setUp(self) -> None:
        ...
        self.asset_source_path = subject.V4_ROOT / "model-assets.draft.json"
        ...
        draft = load_json_strict(subject.V4_ROOT / "design-registration.draft.json")
        self.draft = draft
        ...
        archive_receipt = with_content_digest(
            {
                ...
                "release": {
                    "publishedAt": "2026-09-06T10:05:00Z",
        ...
        def fake_archive_verifier(*_args: Any, **_kwargs: Any) -> dict[str, Any]:
            return {
                ...
                "publishedAt": "2026-09-06T10:05:00Z",
                "attestedAt": "2026-09-06T10:05:01Z",
        ...
        collect_github_gate_receipt_to_path(
            ...
            transport=GateFakeTransport(
                ...
                base=datetime(2026, 9, 6, 10, 5, 0, tzinfo=timezone.utc),
            ),
            now=lambda: "2026-09-06T10:05:30Z",
        )
        ...
        key_raw = (
            subject.V4_ROOT / "signing" / "corelm-crossmodel-v4-signing.pub"
        ).read_bytes()
        self.key_path.write_bytes(key_raw)

        design = copy.deepcopy(draft)
        design.update(
            schemaVersion="corelm-crossmodel-livewiki-v4-design-v1",
    ...
    def test_package_is_deterministic_exact_read_only_and_self_excluding(self) -> None:
        ...
        ci_schema = load_json_strict(
            subject.V4_ROOT / "schemas" / "ci-artifact-verification.schema.json"
        )
    ...
    def test_ci_zip_digest_inventory_platform_workflow_and_zero_skip_fail_closed(self) -> None:
        ...
        preflight_name = "v4-preflight-linux.json"
        ...
        runtime_name = "v4-runtime-linux.json"
        ...
        macos_runtime_name = "v4-runtime-macos.json"
        ...
        design_check_name = "v4-design-check-linux.json"
        ...
        known_answer_name = (
            "v4-release-attestation-known-answer-linux.json"
        )
        ...
        skip_members["v4-zero-skip-linux.log"] = (
            b"Ran 1 test in 0.001s\n\nOK (skipped=1)\n"
            b"ZERO-SKIP POLICY PASS: 1 tests, 0 skipped\n"
        )
