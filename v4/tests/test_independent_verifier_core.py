...
from v4 import independent_verifier_core as independent_subject
from v4 import evidence as producer_evidence
from v4 import nist_beacon as producer_nist
from v4 import protocol as producer_protocol
from v4.independent_verifier_core import (
...
from v4.mediawiki_snapshot import ArchivedHTTPResponse
from v4.github_gate_receipt import verify_github_gate_receipt
from v4.state_machine import create_attempt_marker
from v4.tests import test_github_gate_receipt as gate_fixture
from v4.tests.test_evidence import (
    ATTEMPT,
    BITS,
    CORPORA,
    LAYERS,
...
from v4.tests.test_github_gate_receipt import (
...
from v4.verify_evidence import (
...
SELECTION_VECTOR = V4_ROOT / "test-vectors" / "selection-v1.json"
NIST_VECTOR = V4_ROOT / "test-vectors" / "nist-chain1-pulse1.json"
NIST_PEM = V4_ROOT / "test-vectors" / "nist-chain1-cert.pem"
...
def _nist_fixture(root: Path):
    ...
    manifest = {
        "schemaVersion": "corelm-crossmodel-livewiki-v4-nist-trust-bundle-v1",
...
class IndependentVerifierDifferentialTests(unittest.TestCase):
    ...
    def test_normative_nist_chain_root_and_current_profile_are_independent(self) -> None:
        manifest = V4_ROOT / "trust" / "nist" / "manifest.json"
        bundle = load_independent_trust_bundle(
            manifest,
            expected_time=datetime(2026, 9, 2, 18, tzinfo=timezone.utc),
            expected_manifest_sha256=hashlib.sha256(manifest.read_bytes()).hexdigest(),
    ...
    def test_scientific_paths_are_unaffected_by_monkeypatched_producer(self) -> None:
        ...
        fake_worker = types.ModuleType("v4.model_worker")
        fake_worker.validate_job = Mock(side_effect=poison)
        with patch.dict(sys.modules, {"v4.model_worker": fake_worker}):
    ...
    def test_module_has_no_producer_scientific_import(self) -> None:
        source = (V4_ROOT / "independent_verifier_core.py").read_text(encoding="utf-8")
        for forbidden in (
            "from v4.evidence", "from v4.protocol", "from v4.nist_beacon",
            "from v4.model_worker", "from v4.mediawiki_snapshot",
        ):
    ...
    def test_github_gate_is_replayed_against_author_verification_and_ci(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch.object(
                gate_fixture, "NOW", "2026-09-06T11:58:10Z"
            ):
                gate_raw = collect_gate_fixture(
                    private_root,
                    gate_base_bodies(),
                    base=datetime(2026, 9, 6, 11, 58, tzinfo=timezone.utc),
                )
            ...
            marker = create_attempt_marker(
                ...
                attempt_id="20260926T180000Z-0123456789abcdef",
                design_sha256="8" * 64,
            ...
            freeze = {
                "schemaVersion": "corelm-crossmodel-livewiki-v4-freeze-manifest-v1",
                ...
                "developmentControl": {
                    ...
                    "artifactCount": 2088,
                    "completedAt": "2026-09-06T11:50:00Z",
                    "archivePublishedAt": "2026-09-06T11:51:00Z",
                    "archiveAttestedAt": "2026-09-06T11:52:00Z",
