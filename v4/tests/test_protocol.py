...
class ProtocolTests(unittest.TestCase):
    ...
    def test_ledger_requires_canonical_order(self) -> None:
        records = [
            {"timestamp": "2026-09-08T00:00:02Z", "pageid": 2, "revid": 2},
            {"timestamp": "2026-09-08T00:00:01Z", "pageid": 1, "revid": 1},
        ] * 8
        with self.assertRaises(ValueError):
    ...
    def test_ledger_rejects_duplicate_revision_or_page(self) -> None:
        vector = json.loads(
            (V4_ROOT / "test-vectors" / "selection-v1.json").read_text()
        )
    ...
    def test_normative_ledger_size_and_time_window_are_enforced(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "at least 64 records"):
            validate_ledger(
                ...
                records,
                minimum_records=64,
                timestamp_start=datetime(2026, 9, 8, tzinfo=timezone.utc),
                timestamp_end=datetime(2026, 9, 22, tzinfo=timezone.utc),
            )
        expanded = [
            {
                "timestamp": f"2026-09-08T00:{index // 60:02d}:{index % 60:02d}Z",
                "pageid": index + 1,
                "revid": index + 1,
            }
            for index in range(64)
        ]
        expanded[-1]["timestamp"] = "2026-09-22T00:00:00Z"
    ...
    def test_normative_snapshot_binds_caller_projects_and_models(self) -> None:
        ...
        snapshot = {
            "schemaVersion": "corelm-crossmodel-livewiki-v4-snapshot-registration-v1",
        ...
        before_second_crawl["createdAt"] = "2026-09-23T05:59:59Z"
        ...
        after_release_deadline["createdAt"] = "2026-09-24T18:00:01Z"
        with self.assertRaisesRegex(
    ...
    def test_normative_snapshot_rejects_old_self_referential_release_form(self) -> None:
        ...
        old_snapshot = {
            ...
            "designRelease": release(
                "corelm-crossmodel-livewiki-v4-design", "2026-09-06T12:00:00Z"
            ),
            "snapshotRelease": release(
                "corelm-crossmodel-livewiki-v4-snapshot", "2026-09-23T07:00:00Z"
            ),
            "projects": [
                "de.wikipedia.org",
                "en.wikipedia.org",
    ...
    def test_freeze_status_cannot_be_enabled_by_deleting_blocker_strings(self) -> None:
        registration = json.loads(
            (V4_ROOT / "design-registration.draft.json").read_text()
        )
    ...
    def test_design_remains_explicitly_not_freezable(self) -> None:
        ...
        self.assertTrue(
            any(
                ...
                and "expires before the target pulse" in blocker
                for blocker in blockers
            )
        )
    ...
    def test_model_asset_manifest_matches_design(self) -> None:
        ...
        manifest = json.loads((V4_ROOT / "model-assets.draft.json").read_text())
