...
from v4.reproducibility import with_content_digest
from v4.state_machine import RESERVATION_FILENAME, create_attempt_marker
...
def frozen_design_fixture() -> dict[str, object]:
    ...
    design = json.loads((project_root / "v4/design-registration.draft.json").read_text())
    design["schemaVersion"] = "corelm-crossmodel-livewiki-v4-design-v1"
...
class IndependentVerifierContractTests(unittest.TestCase):
    def test_publication_inputs_share_one_explicit_cryptographic_verifier(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch(
                "v4.verify_evidence.verify_publication",
                side_effect=publications,
            ) as publication, mock.patch(
                "v4.verify_evidence.require_frozen_lab_publication_source"
            ), mock.patch(
                "v4.verify_evidence.verify_design_release_package",
    ...
    def test_private_snapshot_manifest_is_bound_to_attempt_marker_bytes(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            manifest = {
                "schemaVersion": "corelm-crossmodel-livewiki-v4-private-snapshot-manifest-v1",
                "suiteId": marker["suiteId"],
                "status": "SEALED_BEFORE_ATTEMPT",
                "createdAt": "2026-09-26T17:59:59Z",
                "countsTowardScientificVerdict": False,
                    ...
                    [
                        {
                            "path": (
                                "tools/cosign"
                                if role == "pinned-cosign-binary"
                                else f"sealed/{index:02d}-{role}.bin"
                            ),
                            "bytes": 1,
    ...
    def test_independent_reservation_binding_rejects_rewritten_commitment(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            design = {
                ...
                "beacon": {"targetTimestamp": "2026-09-25T18:00:00.000Z"},
            }
            marker = create_attempt_marker(
                root,
    ...
    def test_private_snapshot_requires_every_sealed_input_role(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            roles = sorted(PRIVATE_ROLES - {"model-asset"})
            manifest = {
                "schemaVersion": "corelm-crossmodel-livewiki-v4-private-snapshot-manifest-v1",
                "suiteId": marker["suiteId"],
                "status": "SEALED_BEFORE_ATTEMPT",
    ...
    def test_worker_jobs_summaries_and_jsonl_are_exactly_bound(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for corpus in corpora:
                for page_index, selected in enumerate(selected_pages[corpus]):
                    ...
                    for prediction_index in range(128):
                        raw_records.append(
                            {
                                "schemaVersion": "corelm-crossmodel-livewiki-v4-raw-token-evidence-v1",
                                "suiteId": suite,
                                "attemptId": attempt,
                                "modelKey": model_key,
                                "corpusProject": corpus,
                    ...
                    for layer_index in range(3):
                        container_records.append(
                            {
                                "schemaVersion": "corelm-crossmodel-livewiki-v4-container-evidence-v1",
                                "suiteId": suite,
                                "attemptId": attempt,
                                "modelKey": model_key,
                                "corpusProject": corpus,
            ...
            supervisor = {
                ...
                "completedAt": "2026-09-26T18:00:01Z",
            ...
            summary = {
                "schemaVersion": "corelm-crossmodel-livewiki-v4-worker-summary-v1",
                "suiteId": suite,
                "attemptId": attempt,
                "modelKey": model_key,
                "geometry": {
    ...
    def test_executable_source_tree_has_no_unmanifested_or_changed_files(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (root / "v4" / "runner.py").write_bytes(source)
            manifest = {
                "files": [
                    {
                        "path": "lab/v4/runner.py",
                        "bytes": len(source),
