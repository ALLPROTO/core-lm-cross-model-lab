...
from v4.collect_github_gate_receipt import (
    HTTPSCapture as GateCapture,
    collect_github_gate_receipt_to_path,
)
from v4.github_gate_receipt import (
...
from v4.github_release_attestation import build_attestation_record
from v4.git_source import (
    GitSourceFile,
    GitSourceSeal,
...
from v4.release_receipt import REQUIRED_ASSET_ROLES, ReleaseReceiptError
from v4.release_attestation_crypto import expected_known_answer_result
from v4.reproducibility import canonical_json_bytes, with_content_digest
from v4.tests.test_release_receipt import _fixture_crypto_record
...
LAB_FILES = {
    ...
    "v4/verifier.py": (0o755, b"#!/usr/bin/env python3\nprint('verified')\n"),
}
...
class ZenodoFixture:
    def __init__(self, root: Path) -> None:
        ...
        def ci_zip(system: str, machine: str, suffix: str) -> bytes:
            ...
            return _zip_bytes(
                {
                    f"v4-preflight-{suffix}.json": canonical_json_bytes(preflight) + b"\n",
                    f"v4-runtime-{suffix}.json": runtime(system, machine),
                    f"v4-zero-skip-{suffix}.log": (
                        b"Ran 42 tests in 1.0s\nOK\n"
                        b"ZERO-SKIP POLICY PASS: 42 tests, 0 skipped\n"
                    ),
                    f"v4-design-check-{suffix}.json": canonical_json_bytes(check) + b"\n",
                    f"v4-release-attestation-known-answer-{suffix}.json": (
                        json.dumps(
        ...
        linux_artifact_name = f"author-v4-linux-development-{RUN_ID}-1"
        macos_artifact_name = f"author-v4-macos-development-{RUN_ID}-1"
        ...
        development_tag = "corelm-crossmodel-livewiki-v4-development-control"
...
class ZenodoArchiveTests(unittest.TestCase):
    ...
    def test_closeout_release_attestation_must_precede_its_own_deadline(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch(
                "v4.zenodo_archive.verify_attestation_record",
    ...
    def test_manifest_semantically_verifies_citation_source_and_ci_bytes(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (fixture.deposit / "linux-ci-artifact.zip").write_bytes(
                _zip_bytes({"v4-zero-skip-linux.log": b"fabricated PASS\n"})
            )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            known_answer_name = "v4-release-attestation-known-answer-linux.json"
            ...
            runtime = json.loads(members["v4-runtime-linux.json"])
            with self.assertRaisesRegex(
