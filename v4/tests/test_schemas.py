...
from v4.evidence import CONTAINER_SCHEMA, PAGE_TOKEN_SCHEMA, RAW_TOKEN_SCHEMA
...
class SchemaIntegrityTests(unittest.TestCase):
    ...
    def test_schemas_are_strict_closed_draft_2020_12_documents(self) -> None:
        ...
        for name, schema in schemas.items():
            ...
            self.assertTrue(schema["$id"].endswith(f"/v4/schemas/{name}"))
    ...
    def test_design_schema_covers_the_exact_tracked_registration_boundary(self) -> None:
        ...
        registration = load_json_strict(V4_ROOT / "design-registration.draft.json")
        ...
        self.assertEqual(
            replay["implementation"], "v4/independent_model_replay.py"
        )
        ...
        self.assertEqual(controls["dataset"]["manifestBytes"], 1985)
        self.assertEqual(
        ...
        for name in normative_python_files:
            source = (V4_ROOT / name).read_text(encoding="utf-8")
            self.assertNotIn("3.12.13", source)
            self.assertNotIn("(3, 12, 13)", source)
        bootstrap_source = (V4_ROOT / "bootstrap_runtime.sh").read_text(
            encoding="utf-8"
        )
        self.assertIn(
    ...
    def test_design_lifecycle_branches_fail_closed_and_snapshot_plan_is_exact(self) -> None:
        ...
        for field in ("transportCABundleSHA256", "offlineTrustBundleSHA256"):
            self.assertEqual(
                frozen["beacon"]["properties"][field]["$ref"],
                "#/$defs/sha256",
            )
        ...
        self.assertEqual(
            ...
            "corelm-crossmodel-livewiki-v4-snapshot",
        )
        self.assertEqual(
    ...
    def test_state_result_and_publication_schema_identities_are_consistent(self) -> None:
        ...
        attempt_pattern = "^[0-9]{8}T[0-9]{6}Z-[0-9a-f]{16}$"
        for name in (
            "attempt.schema.json",
            "outcome.schema.json",
            "page-token-evidence.schema.json",
