#!/usr/bin/env python3
"""Read-only, no-inference preflight for the prospective blind v4 suite."""
...
from v4.reproducibility import verify_content_digest  # noqa: E402
...
def verify_asset_receipt(
    ...
    *,
    ...
    if receipt.get("schemaVersion") != "corelm-crossmodel-livewiki-v4-asset-receipt-v1":
...
def result_boundary() -> dict[str, Any]:
    result_root = V4_ROOT / "results"
    entries = sorted(path.name for path in result_root.iterdir())
    if entries != ["README.md"]:
        raise ValueError("blind v4 result directory is not pristine")
...
def main() -> int:
    ...
    manifest = load_json_strict(V4_ROOT / "model-assets.draft.json")
    ...
    result = {
        ...
        "designSHA256": sha256_bytes(
            (V4_ROOT / "design-registration.draft.json").read_bytes()
        ),
        "codec": codec,
