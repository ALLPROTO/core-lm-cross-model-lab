#!/usr/bin/env python3
"""Fail-closed supervisor for the registered blind-v4 one-shot.
...
from v4.mediawiki_snapshot import (  # noqa: E402
...
from v4.package_design_release import (  # noqa: E402
    DesignReleaseError,
    verify_design_release_package,
)
from v4.package_snapshot_release import (  # noqa: E402
    SnapshotReleaseError,
    verify_snapshot_release,
)
from v4.preflight import (  # noqa: E402
    platform_safety,
...
from v4.publication import (  # noqa: E402
    PublicationError,
...
from v4.state_machine import (  # noqa: E402
...
PRIVATE_SCHEMA = "corelm-crossmodel-livewiki-v4-private-snapshot-manifest-v1"
FREEZE_SCHEMA = "corelm-crossmodel-livewiki-v4-freeze-manifest-v1"
RESULT_SCHEMA = "corelm-crossmodel-livewiki-v4-result-v1"
ONE_SHOT_NOT_BEFORE = datetime(2026, 9, 26, 18, tzinfo=timezone.utc)
HARD_DEADLINE = datetime(2026, 9, 27, 18, tzinfo=timezone.utc)
...
SCIENTIFIC_HASH_INPUT = "corelm-crossmodel-livewiki-v4"
SCIENTIFIC_HASH_KNOWN_ANSWER = 6381993545148000455
...
def _validate_schema(instance: Any, filename: str) -> None:
    ...
    schema = load_json_strict(V4_ROOT / "schemas" / filename)
...
def validate_frozen_design(design: Any) -> None:
    ...
    reference = load_json_strict(V4_ROOT / "design-registration.draft.json")
    if design.get("schemaVersion") != "corelm-crossmodel-livewiki-v4-design-v1":
...
def verify_runtime_live(runtime_manifest: Mapping[str, Any], runtime_root: Path) -> None:
    ...
    if runtime_manifest.get("schemaVersion") != "corelm-crossmodel-livewiki-v4-runtime-manifest-v1":
...
def _worker_job(
    *,
    ...
    return {
        "schemaVersion": "corelm-crossmodel-livewiki-v4-worker-job-v1",
        "suiteId": SUITE_ID,
        "attemptId": attempt,
    ...
    try:
        while True:
            ...
            if return_code is not None:
                ...
                return {
                    "schemaVersion": "corelm-crossmodel-livewiki-v4-supervisor-receipt-v1",
                ...
                supervisor_receipt = _supervise_worker(
                    _networkless_macos_command(
                        _scientific_python_command(
                            sys.executable,
                            str(private_root / "lab" / "v4" / "model_worker.py"),
                            "--job",
                            str(job_path),
                            "--snapshot-root",
                            str(private_root),
...
def execute_private_one_shot(private_root: Path, result_root: Path) -> str:
    ...
    expected_script = (private_root / "lab" / "v4" / "runner.py").resolve(strict=True)
    ...
    host_safety.update(
        {
            "schemaVersion": "corelm-crossmodel-livewiki-v4-host-environment-v1",
            "suiteId": SUITE_ID,
            ...
            try:
                verifier_supervisor_receipt = _supervise_worker(
                    _networkless_macos_command(
                        _scientific_python_command(
                            sys.executable,
                            str(private_root / "lab" / "v4" / "verify_evidence.py"),
                            "--evidence-root",
                            str(result_root),
                            "--codec-root",
                            str(private_root / "codec"),
...
def reexec_private_one_shot(
    *, private_root: Path, result_root: Path
    ...
    script = _private_file(private_root, "lab/v4/runner.py")
...
def main() -> int:
    ...
    try:
        ...
        print(f"BLIND-V4 RUNNER FAIL: {error}", file=sys.stderr)
        return 1
