# Blind multi-model v4 — author self-verification

Status: **implementation draft — not frozen, not published, not
preregistered, and scientific one-shot not run**.

P0 trust blocker: the tracked real NIST signing leaf expires at
`2026-09-04T23:59:59Z`, before the proposed pulse on
`2026-09-25T18:00:00.000Z`. It is retained only as a real-chain development
known-answer. V4 cannot be frozen until a replacement chain valid at the target
time is independently verified, pinned, and rebound into every trust/design
commitment before the design deadline.

This directory is the fail-closed development contour for the proposed
`corelm-voidtoken-crossmodel-livewiki-v4-author-verified` experiment. It is intentionally
...
operator, and release operator. V4 has no independent human review, peer
...
- Corpus: eligible page-creation revisions from the German, English, and
  French Wikipedias in `[2026-09-08T00:00:00Z,
...
- Selection: two language editions, sixteen distinct pages from each, and
  ...
  pulse at `2026-09-25T18:00:00.000Z`. The normative selector accepts exact
...
No v4 scientific result exists. The earlier v2 contour is a superseded,
...
observation; it cannot satisfy the v4 freeze gate. V4 must repeat that full
...
```sh
/path/to/locked/python -I -B v4/run_zero_skip_tests.py
/path/to/locked/python -I -B v4/verify_design.py
/path/to/locked/python -I -B v4/preflight.py \
...
`v4/.assets/ud-english-pud-r2.18/en_pud-ud-test.conllu` (SHA-256
...
receipt before `2026-09-07T00:00:00Z`. Local files, an unsigned tag, or an
operator-entered timestamp do not satisfy the freeze gate.

The tracked receipt commits a complete local rehash of all 24 real files. A
fresh clone must regenerate it from its own downloaded bytes and require exact
byte-for-byte equality. The tracked receipt is
...
```sh
/path/to/locked/python v4/collect_snapshot.py \
  --phase crawl-1 \
  --asset-manifest v4/model-assets.draft.json \
  ...
  --output-root /new/path/livewiki-v4-snapshot
```
...
release binding of the already-rehashed assets and replacement NIST trust
material that is valid at the target pulse,
