# Cross-model live-corpus v3 author-self-verified — normative protocol draft
...
The earlier v2 contour is a **superseded, unfrozen draft**. Its completed UD
English PUD producer-to-VTL5-to-real-model-replay E2E is a prior non-scientific
development observation. It remains preserved and disclosed, but it is not v3
freeze evidence, is not a scientific attempt, and cannot satisfy any v3 gate.
V3 must repeat the real-data E2E on its own exact commit and locked runtime.
...
```text
corelm-voidtoken-crossmodel-livewiki-v3-author-verified
```
...
`v3/.assets/ud-english-pud-r2.18/en_pud-ud-test.conllu`.
...
the proposed tag `corelm-crossmodel-livewiki-v3-design` strictly before
`2026-08-15T00:00:00Z`. The signed annotated tag must target the exact
...
bundle offline against `v3/trust/github/trusted_root.json`; the independent
...
strictly earlier than `2026-08-15T00:00:00Z`; `published_at`, HTTP `Date`, and
...
`.github/workflows/v3-development-controls.yml`: exactly 13,560 bytes with
SHA-256
`650316fea247f1ae480180885f762c20f5b9b4b056f2fde41f86646955226d43`.
...
```text
[2026-08-16T00:00:00Z, 2026-08-30T00:00:00Z)
```
...
`2026-08-30T06:00:00Z` and the second no earlier than
`2026-08-31T06:00:00Z`. Each crawl follows `rccontinue` until exhaustion. The
...
```text
ASCII("CORELM-LIVEWIKI-V3-RECORD\0") ||
...
`corelm-crossmodel-livewiki-v3-snapshot`. Its signed annotated tag MUST target
...
strictly earlier than `2026-09-01T18:00:00Z`. The exact pre-publication snapshot-registration bytes
...
Unix milliseconds: 1788372000000
https://beacon.nist.gov/beacon/2.0/pulse/time/1788372000000
```
...
```text
corelm-voidtoken-crossmodel-livewiki-v3-author-verified/select\0
```
...
real-model replay in `v3/independent_model_replay.py`. That module imports none
...
The one-shot may not start before `2026-09-03T18:00:00Z`, twenty-four hours
...
`2026-09-04T18:00:00Z`. Marker creation first uses the host UTC clock; after
...
`2026-09-07T18:00:00Z`. The evidence annotated tag MUST also target the exact
...
`corelm-crossmodel-livewiki-v3-closeout` and published as an immutable release
strictly before `2026-09-14T18:00:00Z`. The closeout release contains exactly
...
The tracked `v3/` contour must contain the design registration, model manifest,
...
`2026-08-14T12:00:00Z`, this suite must remain an unfrozen draft. A replacement
