# Canonical signed GitHub release receipts for v3
...
replication. V3 remains `AUTHOR_SELF_VERIFICATION`, with the repository owner
...
For every v3 development-control, design, snapshot, evidence, and closeout
...
```sh
python3 -I -B \
  v3/tests/integration_release_attestation_crypto_known_answer.py \
...
```sh
python3 -I -B v3/package_design_release.py package \
...
`corelm-crossmodel-livewiki-v3-development-control`; its GitHub immutable-release
attestation must have `attestedAt` strictly before `2026-08-15T00:00:00Z`.
