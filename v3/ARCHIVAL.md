...
the later design, snapshot, or one-shot evidence releases.

The corresponding v2 E2E is preserved only as a prior non-scientific
observation from a superseded, unfrozen draft. It cannot be relabeled or reused
as the mandatory exact-commit v3 development control.
...
The evidence DOI must identify the actual terminal outcome, including a
failure or reservation/marker-only interruption. A later regression receives another
release identity and must not overwrite the one-shot record.
...
```bash
python3 -m v3.create_zenodo_deposit_manifest \
...
python3 -m v3.collect_zenodo_receipt \
...
```bash
python3 -m v3.verify_zenodo_receipt \
