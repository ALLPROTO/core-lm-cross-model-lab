# Blind-v3 author-self-verification freeze readiness gate
...
V3 is operated under `AUTHOR_SELF_VERIFICATION`: the repository owner is the
...
- [ ] The earlier v2 contour is recorded as a superseded, unfrozen draft. Its
  completed UD English PUD E2E is disclosed as a prior non-scientific
  observation and is not reused as v3 freeze evidence. The complete three-model
  producer-to-VTL5-to-real-model-replay control is rerun on the exact v3
...
- [ ] The report and complete deterministic artifact ZIP are published in the
  immutable `corelm-crossmodel-livewiki-v3-development-control` release from a
  signed annotated tag targeting that same commit. A canonical v3 receipt binds
  all three release assets and requires the GitHub immutable-release RFC3161
  `attestedAt` strictly before `2026-08-15T00:00:00Z`; local files,
...
- [ ] Release-receipt collection and independent replay both use exact pinned
  ...
  `v3/trust/github/trusted_root.json` (`28,886` bytes, SHA-256
...
- [ ] The public design release is available strictly before
  `2026-08-15T00:00:00Z`; local time, draft PR time, commit author time, or an
...
`2026-08-14T12:00:00Z`, do not publish or label the current draft as frozen.
