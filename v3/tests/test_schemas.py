...
from v3.evidence import CONTAINER_SCHEMA, PAGE_TOKEN_SCHEMA, RAW_TOKEN_SCHEMA
...
class SchemaIntegrityTests(unittest.TestCase):
    ...
    def test_schemas_are_strict_closed_draft_2020_12_documents(self) -> None:
        ...
        for name, schema in schemas.items():
            ...
            self.assertTrue(schema["$id"].endswith(f"/v3/schemas/{name}"))
    ...
    def test_design_schema_covers_the_exact_tracked_registration_boundary(self) -> None:
        ...
        self.assertEqual(
            replay["implementation"], "v3/independent_model_replay.py"
        )
        ...
        workflow = (
            V3_ROOT.parent / ci["workflowPath"]
        ).read_bytes()
        ...
        for name in normative_python_files:
            source = (V3_ROOT / name).read_text(encoding="utf-8")
            self.assertNotIn("3.12.13", source)
            self.assertNotIn("(3, 12, 13)", source)
        bootstrap_source = (V3_ROOT / "bootstrap_runtime.sh").read_text(
            encoding="utf-8"
        )
        self.assertIn(
    ...
    def test_design_lifecycle_branches_fail_closed_and_snapshot_plan_is_exact(self) -> None:
        ...
        self.assertEqual(
            ...
            "corelm-crossmodel-livewiki-v3-snapshot",
        )
        self.assertEqual(
