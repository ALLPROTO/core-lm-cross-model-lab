...
from v3.protocol import canonical_json_bytes
from v3.reproducibility import with_content_digest
from v3.state_machine import RESERVATION_FILENAME, create_attempt_marker
from v3.verify_evidence import (
...
def frozen_design_fixture() -> dict[str, object]:
    ...
    design = json.loads((project_root / "v3/design-registration.draft.json").read_text())
    design["schemaVersion"] = "corelm-crossmodel-livewiki-v3-design-v1"
...
class IndependentVerifierContractTests(unittest.TestCase):
    def test_publication_inputs_share_one_explicit_cryptographic_verifier(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch(
                "v3.verify_evidence.verify_publication",
                side_effect=publications,
            ) as publication, mock.patch(
                "v3.verify_evidence.require_frozen_lab_publication_source"
            ), mock.patch(
                "v3.verify_evidence.verify_design_release_package",
    ...
    def test_independent_verifier_replays_external_attempt_time_anchor(self) -> None:
        ...
        self.assertEqual(
            verify_external_attempt_time_anchor(
                {"responseDate": "2026-09-03T18:00:00Z"}, design
            ),
            "2026-09-03T18:00:00Z",
        )
        with self.assertRaisesRegex(EvidenceError, "outside"):
            verify_external_attempt_time_anchor(
                {"responseDate": "2026-09-04T18:00:00Z"}, design
            )

    def test_runtime_assets_private_bytes_and_sbom_are_semantically_replayed(self) -> None:
        project_root = Path(__file__).resolve().parents[2]
        source_raw = (project_root / "v3/model-assets.draft.json").read_bytes()
        receipt_raw = (
            project_root / "v3/manifests/model-assets.full-rehash.json"
        ).read_bytes()
    ...
    def test_scientific_result_has_exact_canonical_schema_fields(self) -> None:
        verification = {
            ...
            "attemptId": "20260903T180000Z-0123456789abcdef",
            "verdict": "FAIL_GATES",
            "cells": [
                {
    ...
    def test_private_snapshot_manifest_is_bound_to_attempt_marker_bytes(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            manifest = {
                "schemaVersion": "corelm-crossmodel-livewiki-v3-private-snapshot-manifest-v1",
                "suiteId": marker["suiteId"],
                "status": "SEALED_BEFORE_ATTEMPT",
                "createdAt": "2026-09-03T17:59:59Z",
                "countsTowardScientificVerdict": False,
    ...
    def test_independent_reservation_binding_rejects_rewritten_commitment(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            design = {
                ...
                "beacon": {"targetTimestamp": "2026-09-02T18:00:00.000Z"},
            }
            marker = create_attempt_marker(
                root,
    ...
    def test_worker_jobs_summaries_and_jsonl_are_exactly_bound(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            suite = "corelm-voidtoken-crossmodel-livewiki-v3-author-verified"
            attempt = "20260903T180000Z-0123456789abcdef"
            ...
            for corpus in corpora:
                for page_index, selected in enumerate(selected_pages[corpus]):
                    ...
                    for prediction_index in range(128):
                        raw_records.append(
                            {
                                "schemaVersion": "corelm-crossmodel-livewiki-v3-raw-token-evidence-v1",
                                "suiteId": suite,
                                "attemptId": attempt,
                                "modelKey": model_key,
                                "corpusProject": corpus,
                    ...
                    for layer_index in range(3):
                        container_records.append(
                            {
                                "schemaVersion": "corelm-crossmodel-livewiki-v3-container-evidence-v1",
                                "suiteId": suite,
                                "attemptId": attempt,
                                "modelKey": model_key,
                                "corpusProject": corpus,
            ...
            supervisor = {
                ...
                "completedAt": "2026-09-03T18:00:01Z",
    ...
    def test_executable_source_tree_has_no_unmanifested_or_changed_files(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (root / "v3" / "runner.py").write_bytes(source)
            manifest = {
                "files": [
                    {
                        "path": "lab/v3/runner.py",
                        "bytes": len(source),
