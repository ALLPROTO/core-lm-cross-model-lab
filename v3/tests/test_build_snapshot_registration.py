...
from v3.build_snapshot_registration import (
...
from v3.release_receipt import API_ROLES, GITHUB_API_VERSION, REQUIRED_ASSET_ROLES
...
class SnapshotRegistrationBuilderTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.signing_public_key_path.write_bytes(
            (V3_ROOT / "signing/corelm-crossmodel-v3-signing.pub").read_bytes()
        )
        ...
        self.design = copy.deepcopy(
            load_json_strict(V3_ROOT / "design-registration.draft.json")
        )
        self.design["schemaVersion"] = "corelm-crossmodel-livewiki-v3-design-v1"
        ...
        self.design["developmentControls"]["realDataE2EFreezeGate"].update(
            ...
            archiveAttestedAt="2026-08-14T10:06:00Z",
        ...
        self.asset_manifest_raw = (V3_ROOT / "model-assets.draft.json").read_bytes()
        self.asset_manifest_path.write_bytes(self.asset_manifest_raw)
        self.asset_receipt_raw = (
            V3_ROOT / "manifests/model-assets.full-rehash.json"
        ).read_bytes()
    ...
    def _write_design_receipt(self) -> None:
        ...
        receipt = {
            ...
            "receiptCreatedAt": "2026-08-14T12:10:00Z",
        }
    ...
    def test_derives_one_deterministic_registration_and_invokes_replay(self) -> None:
        ...
        with mock.patch(
            "v3.build_snapshot_registration.verify_publication",
            return_value=self.publication_result(),
        ) as publication, mock.patch(
            "v3.build_snapshot_registration._load_verified_tokenizers",
            return_value=owned,
        ) as load_tokenizers, mock.patch(
            "v3.build_snapshot_registration.verify_corpus_snapshot",
    ...
    def test_created_at_replay_and_post_replay_ledger_are_fail_closed(self) -> None:
        ...
        for timestamp in ("2026-08-31T05:59:59Z", "2026-09-01T18:00:00Z"):
    ...
    def test_owned_tokenizer_loader_cross_checks_receipt_and_is_cli_mandatory(self) -> None:
        ...
        with mock.patch(
            "v3.build_snapshot_registration.verify_assets_and_load_tokenizer_bytes",
            return_value=verified,
        ) as verify_assets, mock.patch(
            "v3.build_snapshot_registration.default_tokenizer_factory",
        ...
        argv = [
            ...
            "--created-at", "2026-08-31T06:30:00Z",
    ...
    def test_symlinked_parent_ledgers_and_internal_output_are_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(
            ...
            build_snapshot_registration_to_path(
                output=V3_ROOT / ".working/forbidden-snapshot-registration.json",
                frozen_design_path=self.design_path,
                corpus_root=self.corpus_root,
                asset_root=self.asset_root,
                design_release_asset_root=self.design_release_asset_root,
