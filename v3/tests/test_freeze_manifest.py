...
from v3 import development_artifact_verifier
from v3 import development_model_replay
from v3 import freeze_manifest as subject
from v3 import run_real_e2e_control
...
class FreezeManifestTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        collect_github_gate_receipt_to_path(
            ...
            transport=GateFakeTransport(
                gate_base_bodies(),
                base=datetime(2026, 8, 14, 10, 5, 0, tzinfo=timezone.utc),
            ),
            now=lambda: "2026-08-14T10:05:10Z",
        )
    ...
    def _live_git_fixture(self) -> tuple[Path, Path, dict[str, str]]:
        ...
        dependency = repository / "v3" / "release_receipt.py"
    ...
    def _write_assets(self) -> None:
        manifest_path = subject.V3_ROOT / "model-assets.draft.json"
    ...
    def development_archive_verifier(*_args: Any, **_kwargs: Any) -> dict[str, Any]:
        return {
            ...
            "attestedAt": "2026-08-14T10:05:01Z",
    ...
    def _pud_plan_fixture(
        ...
        design = load_json_strict(subject.V3_ROOT / "design-registration.draft.json")
        ...
        configuration = {
            "schemaVersion": (
                "corelm-crossmodel-v3-real-e2e-development-configuration-v1"
            ),
            "suiteId": subject.DEVELOPMENT_SUITE_ID,
            "countsTowardScientificVerdict": False,
            "usedForCandidateSelectionOrTuning": False,
    ...
    def test_live_source_verifier_rejects_dirty_dependency_and_untracked_path(
        ...
        with mock.patch.object(subject, "PROJECT_ROOT", repository):
            ...
            untracked = repository / "v3" / "github_gate_receipt.py"
    ...
    def test_generator_is_canonical_binds_inputs_and_has_no_file_self_reference(self) -> None:
        ...
        schema = load_json_strict(
            subject.V3_ROOT / "schemas" / "freeze-manifest.schema.json"
        )
    ...
    def test_validator_rejects_cross_identity_extra_field_and_self_digest_tampering(self) -> None:
        ...
        tampered["createdAt"] = "2026-08-14T10:07:00Z"
    ...
    def test_decision_checkpoint_and_publication_deadline_fail_closed(self) -> None:
        ...
        late_gate["createdAt"] = "2026-08-14T12:00:02Z"
        ...
        late_freeze["createdAt"] = "2026-08-15T00:00:01Z"
    ...
    def test_stage_two_design_binds_exact_manifest_file_sha(self) -> None:
        ...
        design = copy.deepcopy(
            load_json_strict(subject.V3_ROOT / "design-registration.draft.json")
        )
        design.update(
