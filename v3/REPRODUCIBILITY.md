# Blind-v3 author-self-verified clean-clone reproducibility
...
git -C lab fetch origin tag corelm-crossmodel-livewiki-v3-design
test "$(git -C lab rev-list -n 1 corelm-crossmodel-livewiki-v3-design)" = \
...
```sh
cd lab
RUNTIME_ROOT="$PWD/v3/.runtime/linux"
./v3/bootstrap_runtime.sh \
  --platform linux \
  --codec-root ../codec-source \
  --runtime "$RUNTIME_ROOT" \
  --python "$(command -v python3.12)"
"$RUNTIME_ROOT/bin/python" -I -B v3/run_zero_skip_tests.py
```
...
```sh
cd lab
RUNTIME_ROOT="$PWD/v3/.runtime/macos"
./v3/bootstrap_runtime.sh \
  --platform macos \
  --codec-root ../codec-source \
  --runtime "$RUNTIME_ROOT" \
...
mkdir -p v3/.working
python3 v3/create_asset_receipt.py \
  --asset-root v3/.assets \
  --output v3/.working/asset-receipt.json
```

The same standard-library downloader writes the byte-pinned real-data input to
`v3/.assets/ud-english-pud-r2.18/en_pud-ud-test.conllu`. It verifies commit
...
`v3/manifests/model-assets.full-rehash.json` must compare byte-for-byte equal to
...
```sh
"$RUNTIME_ROOT/bin/python" -I -B v3/create_runtime_manifest.py \
  ...
  --codec-root ../codec-source \
  --output v3/.working/runtime-manifest.json
```
...
```sh
"$RUNTIME_ROOT/bin/python" -I -B v3/create_sbom.py \
  --runtime-manifest v3/.working/runtime-manifest.json \
  --asset-receipt v3/.working/asset-receipt.json \
  --output v3/.working/cyclonedx-sbom.json
```
...
```sh
"$RUNTIME_ROOT/bin/python" -B -m v3.create_zenodo_deposit_manifest \
...
"$RUNTIME_ROOT/bin/python" -B -m v3.verify_zenodo_receipt \
...
```sh
"$RUNTIME_ROOT/bin/python" -I -B v3/freeze_manifest.py create \
  --runtime-manifest v3/.working/runtime-manifest.json \
  --asset-receipt v3/.working/asset-receipt.json \
  --transport-ca-bundle v3/trust/transport-ca.pem \
  --offline-trust-manifest v3/trust/nist/manifest.json \
  ...
  --output v3/.working/freeze-manifest.json
```
...
"$RUNTIME_ROOT/bin/python" -I -B v3/build_frozen_design.py \
  ...
  --signing-public-key v3/signing/corelm-crossmodel-v3-signing.pub \
  --cosign /absolute/path/to/pinned-cosign-v3.0.6 \
...
```sh
"$RUNTIME_ROOT/bin/python" -I -B v3/freeze_manifest.py verify \
  --manifest v3/.working/freeze-manifest.json \
  --runtime-manifest v3/.working/runtime-manifest.json \
  --asset-receipt v3/.working/asset-receipt.json \
  --transport-ca-bundle v3/trust/transport-ca.pem \
  --offline-trust-manifest v3/trust/nist/manifest.json \
...
```sh
"$RUNTIME_ROOT/bin/python" -I -B v3/package_design_release.py package \
  --frozen-design /path/to/frozen-design.json \
  --development-control-report /path/to/development-control-report.json \
  --development-control-archive-receipt /path/to/development-control-archive-receipt.json \
  --freeze-manifest v3/.working/freeze-manifest.json \
  --github-gate-receipt /path/to/github-gate-receipt.json \
  --linux-ci-artifact /path/to/downloaded-linux-artifact.zip \
  --macos-arm64-ci-artifact /path/to/downloaded-macos-artifact.zip \
  --asset-source-manifest v3/model-assets.draft.json \
  --full-asset-receipt v3/.working/asset-receipt.json \
  --runtime-manifest v3/.working/runtime-manifest.json \
  --sbom v3/.working/cyclonedx-sbom.json \
  --signing-public-key v3/signing/corelm-crossmodel-v3-signing.pub \
  --output-root /new/path/design-release-assets

"$RUNTIME_ROOT/bin/python" -I -B v3/package_design_release.py verify \
...
```sh
"$RUNTIME_ROOT/bin/python" -I -B v3/verify_design.py
"$RUNTIME_ROOT/bin/python" -I -B v3/preflight.py \
  --codec-root ../codec-source \
  --asset-root v3/.assets \
  --asset-receipt v3/.working/asset-receipt.json \
  --require-assets
...
RUNTIME_ROOT="$PWD/v3/.runtime/macos"
RUNTIME_MANIFEST="$PWD/v3/.working/runtime-manifest.json"
DATASET="$PWD/v3/.assets/ud-english-pud-r2.18/en_pud-ud-test.conllu"
...
```sh
PYTHONHASHSEED=0 "$RUNTIME_ROOT/bin/python" -P -s -B \
  v3/package_development_control_release.py package \
...
`v3/trust/github/trusted_root.json`. The release ID is an explicit input:

```sh
PYTHONHASHSEED=0 "$RUNTIME_ROOT/bin/python" -P -s -B \
  v3/collect_release_receipt.py \
  --repository ALLPROTO/core-lm-cross-model-lab \
  --kind development-control \
  --tag corelm-crossmodel-livewiki-v3-development-control \
