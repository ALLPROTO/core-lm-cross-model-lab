...
This v3 contract is author-self-verified. It claims no independent human
...
This document is the supervisor contract for the blind-v3 corpus collector and
...
1. On or after `2026-08-30T06:00:00Z`, call
   `collect_crawl_stage(root=..., crawl_index=0, transport=..., clock=...)` with
   a new or empty root.
2. On or after `2026-08-31T06:00:00Z`, call the same function with
...
```sh
python3 -I -B v3/build_snapshot_registration.py \
...
`v3/test-vectors/nist-chain1-pulse1.json` and its certificate are a historical
...
- The target `2026-09-02T18:00:00.000Z` pulse does not exist yet.
...
```sh
python3 -m unittest v3.tests.test_mediawiki_snapshot v3.tests.test_nist_beacon -v
```
