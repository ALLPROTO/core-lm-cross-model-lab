...
inputs, never creates a reservation/marker, never writes ``v3/results``, never
...
from v3.create_asset_receipt import build_asset_receipt  # noqa: E402
from v3.development_model_replay import (  # noqa: E402
    CANDIDATE,
    CORPORA,
    DATASET_BYTES,
...
from v3.development_runtime import (  # noqa: E402
...
REPORT_SCHEMA = "corelm-crossmodel-v3-real-e2e-development-report-v1"
...
def _external_output_path(output: Path) -> tuple[Path, str]:
    ...
    if parent == project or parent.is_relative_to(project):
        raise DevelopmentControlError(
            "development output must be outside the repository and v3/results"
        )
...
def verify_codec(codec_root: Path, design: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(source, dict):
        raise DevelopmentControlError("v3 codec source binding is absent")
    try:
    ...
    if commit != source.get("commit") or tree != source.get("tree"):
        raise DevelopmentControlError("codec commit/tree differs from exact v3 binding")
...
def _load_fixed_inputs(
    *,
    ...
    design_path = V3_ROOT / "design-registration.draft.json"
    manifest_path = V3_ROOT / "model-assets.draft.json"
    receipt_path = V3_ROOT / "manifests" / "model-assets.full-rehash.json"
    corpus_manifest_path = V3_ROOT / "development-corpus.draft.json"
    ...
    design = _canonical_document(design_raw, label="v3 design registration")
    manifest = _canonical_document(manifest_raw, label="v3 model asset manifest")
    receipt = _canonical_document(receipt_raw, label="v3 full asset receipt")
    ...
    if tuple(item.get("key") for item in design.get("models", [])) != MODELS:
        raise DevelopmentControlError("v3 model order differs from development control")
    candidate = design.get("candidate")
    if not isinstance(candidate, dict) or {
        key: candidate.get(key) for key in CANDIDATE
    } != CANDIDATE:
        raise DevelopmentControlError("v3 candidate differs from development control")
...
def _run_control(
    ...
    start_marker = _with_content_digest(
        {
            "schemaVersion": "corelm-crossmodel-v3-real-e2e-development-start-v1",
            "suiteId": SUITE_ID,
    ...
    with tempfile.TemporaryDirectory(
        ...
        command, backend = _development_child_command(
            V3_ROOT / "development_model_replay.py",
            "--evidence-root",
            str(output_root),
            "--private-root",
            str(private_root),
...
def _write_failure_receipt(arguments: argparse.Namespace, error: Exception) -> None:
    ...
    failure = _with_content_digest(
        {
            "schemaVersion": "corelm-crossmodel-v3-real-e2e-development-failure-v1",
            "suiteId": SUITE_ID,
