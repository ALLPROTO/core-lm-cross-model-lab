...
# Build only the hash-locked Python runtime needed by blind-v3 controls.  This
...
fail() {
    printf 'V3 RUNTIME BOOTSTRAP FAIL: %s\n' "$*" >&2
    exit 1
}
...
usage() {
    printf '%s\n' \
        'Usage: v3/bootstrap_runtime.sh --platform linux|macos \' \
...
[ "$(git -C "$CODEC_ROOT" rev-parse HEAD)" = "$CODEC_COMMIT" ] \
    || fail 'codec commit differs from the registered v3 binding'
[ "$(git -C "$CODEC_ROOT" rev-parse 'HEAD^{tree}')" = "$CODEC_TREE" ] \
    || fail 'codec tree differs from the registered v3 binding'
...
STAGING=$(mktemp -d "$RUNTIME_PARENT/.corelm-v3-runtime-stage.XXXXXX") \
    || fail 'could not create runtime staging directory'

cleanup() {
    if [ -n "$STAGING" ]; then
        case "$STAGING" in
            "$RUNTIME_PARENT"/.corelm-v3-runtime-stage.*)
                [ ! -e "$STAGING" ] || rm -rf -- "$STAGING"
                ;;
            *) printf 'V3 RUNTIME CLEANUP REFUSED: %s\n' "$STAGING" >&2 ;;
        esac
    fi
}
on_signal() {
