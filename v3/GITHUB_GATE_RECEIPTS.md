...
The v3 GitHub gate is **CI-only**. It is not a human-review gate and it does
...
The v3 collector and schema must contain no reviewer identity, review approval
...
account is not accepted as a substitute for independent review because v3 makes
...
`.github/workflows/v3-development-controls.yml`: 13,560 bytes with SHA-256
...
python3 -I -B v3/collect_github_gate_receipt.py \
  ...
  --workflow-name 'Author-verified v3 development controls' \
  --workflow-path '.github/workflows/v3-development-controls.yml' \
  --token-env CORELM_GITHUB_READ_TOKEN \
  --output /absolute/path/to/github-ci-gate-receipt.json
```

The normative v3 CLI must not accept a reviewer-related argument. If the
tracked implementation still requires one, v3 is not freeze-ready and the CLI,
...
Absence of a human approval is intentional in v3; absence or failure of any
