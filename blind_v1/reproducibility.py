#!/usr/bin/env python3
"""Deterministic, dependency-free provenance helpers for blind-v1.
...
RUNTIME_MANIFEST_SCHEMA = "corelm-blind-crossmodel-v1-runtime-manifest-v1"
...
def _fsync_directory_descriptor(descriptor: int) -> None:
    """Commit a previously opened directory entry to stable storage."""

    os.fsync(descriptor)


def _native_at_fdcwd(platform_name: str) -> int:
    ...
    raise OSError(
        errno.ENOTSUP,
        "platform lacks atomic exclusive path publication",
    )


def publish_new_path_exclusive(source: Path, destination: Path) -> None:
    """Atomically move ``source`` while refusing an existing destination.

    Both regular files and directories are supported.  The native no-replace
    primitive closes the check/rename race that a preceding ``exists()`` test
    cannot close.  After a successful rename, later fsync failures deliberately
    leave the published object in place for forensic inspection.
    """
    ...
    at_fdcwd = _native_at_fdcwd(sys.platform)
...
def write_new_bytes(path: Path, value: bytes) -> None:
    ...
    Once the exclusive inode exists, any bytes are retained on every later
    failure.  In particular, a directory-fsync failure must not erase a fully
    fsynced evidence artifact.  The retained inode is intentionally fail-closed:
    a subsequent call cannot silently turn an uncertain publication into a new
    receipt.
    """
