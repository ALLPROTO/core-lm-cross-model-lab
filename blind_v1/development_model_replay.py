...
from blind_v1 import independent_model_replay as replay  # noqa: E402
from blind_v1.development_corpus import verify_rights_evidence  # noqa: E402


PLAN_SCHEMA = "corelm-blind-crossmodel-v1-real-e2e-development-plan-v1"
JOB_SCHEMA = "corelm-blind-crossmodel-v1-real-e2e-development-worker-job-v1"
SUMMARY_SCHEMA = "corelm-blind-crossmodel-v1-real-e2e-development-model-replay-v1"
SUITE_ID = "corelm-blind-crossmodel-v1-development-e2e"
...
RAW_TOKEN_SCHEMA = "corelm-blind-crossmodel-v1-real-e2e-development-raw-token-v1"
PAGE_TOKEN_SCHEMA = "corelm-blind-crossmodel-v1-real-e2e-development-page-token-v1"
CONTAINER_SCHEMA = "corelm-blind-crossmodel-v1-real-e2e-development-container-v1"
...
EXPECTED_CORPUS_MANIFEST = {
    "schemaVersion": "corelm-blind-crossmodel-v1-development-corpus-v1",
...
def _load_archived_inputs(
    ...
    paths = {
        ...
        "modelAssetManifest": "inputs/development-model-assets.json",
    ...
    control_models = controls.get("modelAssets")
    if (
        not isinstance(control_models, dict)
        or control_models.get("path") != "blind_v1/development-model-assets.json"
        or control_models.get("modelKeys") != list(MODELS)
        or control_models.get("excludedFromConfirmatoryPool") is not True
    ):
        raise DevelopmentReplayError("development pilot binding differs")
    confirmatory_keys = {
        item.get("key")
        for item in design.get("models", [])
        if isinstance(item, dict)
    }
    if confirmatory_keys.intersection(MODELS):
        raise DevelopmentReplayError("development and confirmatory model pools overlap")
    if [item.get("key") for item in plan["models"]] != list(MODELS):
        raise DevelopmentReplayError("development plan model order differs")
    for plan_model in plan["models"]:
        identity = EXPECTED_MODEL_IDENTITIES[plan_model["key"]]
        for field in ("repository", "revision", "layers", "vocabSize"):
            if plan_model.get(field) != identity[field]:
                raise DevelopmentReplayError(
                    f"development plan model binding differs: "
                    f"{plan_model['key']}/{field}"
                )
...
def validate_plan(plan: Any) -> None:
    ...
    if bindings["fullAssetReceipt"] != {
        "bytes": 4289,
        "sha256": "b3ecf610877c63110b071ef3b9d02cb88d7e5c3dd736211290a00d3f29dab87d",
    }:
...
def _development_backend_arguments(
    *,
    model_key: str,
    model: dict[str, Any],
    model_bytes: dict[str, bytes],
) -> dict[str, Any]:
    """Derive strict replay arguments from the pinned spec and config bytes."""

    identity = EXPECTED_MODEL_IDENTITIES.get(model_key)
    if identity is None or model.get("key") != model_key:
        raise DevelopmentReplayError("development backend model identity differs")
    expected = {
        "expected_model_type": identity["modelType"],
        "expected_model_vocab_size": identity["vocabSize"],
        "expected_tokenizer_vocab_size": identity["tokenizerVocabSize"],
        "expected_layers": identity["layers"],
    }
    if (
        model.get("vocabSize") != expected["expected_model_vocab_size"]
        or model.get("layers") != expected["expected_layers"]
    ):
        raise DevelopmentReplayError(
            f"development backend model spec differs: {model_key}"
        )
    files = model.get("files")
    if not isinstance(files, dict):
        raise DevelopmentReplayError(
            f"development backend model files differ: {model_key}"
        )
    for filename in ("config.json", "tokenizer.json"):
        raw = model_bytes.get(filename)
        specification = files.get(filename)
        if (
            type(raw) is not bytes
            or not isinstance(specification, dict)
            or len(raw) != specification.get("bytes")
            or hashlib.sha256(raw).hexdigest() != specification.get("sha256")
        ):
            raise DevelopmentReplayError(
                f"development backend asset differs: {model_key}/{filename}"
            )
    config = _strict_json_object(
        model_bytes["config.json"], label=f"development config {model_key}"
    )
    layer_field = {
        "gpt_neo": "num_layers",
    ...
    if (
        config.get("model_type") != expected["expected_model_type"]
        or type(config.get("vocab_size")) is not int
        or config["vocab_size"] != expected["expected_model_vocab_size"]
        or type(config.get(layer_field)) is not int
        or config[layer_field] != expected["expected_layers"]
    ):
        raise DevelopmentReplayError(
            f"development backend config differs: {model_key}"
        )
    return expected
...
def _consume_owned_development_record(
    record_raw: bytes,
    consumer: Callable[[bytes], Any],
) -> Any:
    """Consume one development record and release it before the next read."""
...
def run_development_replay(*, evidence_root: Path, private_root: Path) -> dict[str, Any]:
    ...
    for model_key in MODELS:
        ...
        backend_arguments = _development_backend_arguments(
            model_key=model_key,
            model=job["model"],
        ...
        try:
            ...
            raw_bytes = replay.read_beneath(
                evidence_root,
                raw_relative,
                maximum_bytes=128 * 1024 * 1024,
            )
            container_bytes = replay.read_beneath(
                evidence_root,
                container_relative,
                maximum_bytes=64 * 1024 * 1024,
            )
            page_bytes = replay.read_beneath(
                evidence_root,
                page_relative,
                maximum_bytes=16 * 1024 * 1024,
            )
            raw_records = replay.load_canonical_jsonl(
                raw_bytes, label=raw_relative
            )
            container_records = replay.load_canonical_jsonl(
                container_bytes, label=container_relative
            )
            page_records = replay.load_canonical_jsonl(
                page_bytes, label=page_relative
            )
            expected_pages = 32
            if (
        ...
        try:
            for corpus in CORPORA:
                for page in job["pages"][corpus]:
                    ...
                    page_summary, record_commitment = (
                        _consume_owned_development_record(
                            replay.read_beneath(
                                private_root,
                                page["recordPath"],
                                maximum_bytes=64 * 1024 * 1024,
