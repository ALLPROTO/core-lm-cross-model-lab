#!/usr/bin/env python3
"""Build and independently verify a blind-v1 public evidence package.
...
replace ``blind_v1/verify_evidence.py``, which independently recomputes the scientific
...
PUBLIC_ATTEMPT_ID_PREFIX = "20260821T180000Z-"
ATTEMPT_ID = re.compile(
    re.escape(PUBLIC_ATTEMPT_ID_PREFIX) + r"[0-9a-f]{16}\Z"
)
...
TARGET_PULSE_TIMESTAMP = "2026-08-21T18:00:00.000Z"
...
INNER_PAGE_TOKEN_PATHS = frozenset({"page-token-evidence.jsonl"})
...
BINDING_DESTINATIONS = {
    ...
    "reservationReceipt": (
        "payload/bindings/publication/reservation-receipt.json",
        "reservation-publication-receipt",
    ),
    "signingKey": (
...
def _derive_public_attempt_id(value: dict[str, Any]) -> str:
    unsigned = dict(value)
    unsigned.pop("attemptId", None)
    unsigned.pop("reservationContentSHA256", None)
    expected = PUBLIC_RESERVATION_FIELDS - {
        "attemptId",
        "reservationContentSHA256",
    }
    if set(unsigned) != expected:
        raise EvidenceReleaseError("public execution reservation fields differ")
    return PUBLIC_ATTEMPT_ID_PREFIX + sha256_bytes(canonical_json_bytes(unsigned))[:16]
...
def _validate_reservation(raw: bytes) -> dict[str, Any]:
    ...
    if (
        ...
        != "corelm-blind-crossmodel-v1-attempt-reservation-v1"
    ...
    if not ONE_SHOT_NOT_BEFORE <= created < MARKER_NO_LATER_THAN:
        raise EvidenceReleaseError(
            "attempt reservation is outside the frozen marker window"
        )
    for field in (
        "designSHA256",
        "snapshotRegistrationSHA256",
...
def _validate_marker(raw: bytes) -> dict[str, Any]:
    ...
    if (
        marker["schemaVersion"] != "corelm-blind-crossmodel-v1-attempt-v1"
    ...
    if not ONE_SHOT_NOT_BEFORE <= created < MARKER_NO_LATER_THAN:
        raise EvidenceReleaseError("attempt marker is outside the frozen marker window")
    for field in (
        "designSHA256",
        "snapshotRegistrationSHA256",
        "designPublicationReceiptSHA256",
...
def _validate_outcome(
    ...
    if (
        outcome["schemaVersion"] != "corelm-blind-crossmodel-v1-outcome-v1"
...
def _expected_role(path: str) -> str:
    ...
    if path.startswith("payload/bindings/publication/reservation-assets/"):
        return "reservation-release-asset"
    fixed = {
...
def _verify_inner_evidence_manifest(
    ...
    if manifest["schemaVersion"] != "corelm-blind-crossmodel-v1-evidence-manifest-v1":
    ...
    for entry in entries:
        ...
        if sha256_bytes(payload) != entry["sha256"]:
            raise EvidenceReleaseError("producer evidence entry digest differs")
    selection = _load_canonical_line(
        _read_beneath(root, ARTIFACT_PATHS["selection"]),
        label="selection",
    )
    ...
    if (
        ...
        or any(
            ...
            or re.fullmatch(r"[a-z0-9][a-z0-9._-]*", model) is None
            for model in model_order
        )
    ):
        raise EvidenceReleaseError("selection model execution order differs")
    expected_page_tokens = INNER_PAGE_TOKEN_PATHS.union(
        f"workers/{model}/page-token-evidence.jsonl" for model in model_order
    )
    missing_page_tokens = sorted(expected_page_tokens - observed_paths)
...
def _verify_component_bindings(
    ...
    *,
    ...
    if artifact_presence["privateSnapshotManifest"]:
        ...
        reservation_receipt_raw = _read_beneath(
            root, "payload/bindings/publication/reservation-receipt.json"
        )
        if private_manifest.get("reservationPublicationReceiptSHA256") != sha256_bytes(
            reservation_receipt_raw
        ):
            raise EvidenceReleaseError(
                "public execution-reservation receipt differs from private snapshot"
            )
    ...
    if (
        ...
        != "corelm-blind-crossmodel-v1-model-assets-draft-v1"
    ...
    execution_reservation_raw = _read_beneath(
        root,
        "payload/bindings/publication/reservation-assets/execution-reservation.json",
    )
    reservation_snapshot_receipt_raw = _read_beneath(
        root,
        "payload/bindings/publication/reservation-assets/"
        "snapshot-publication-receipt.json",
    )
    execution_reservation = _load_canonical_line(
        execution_reservation_raw,
        label="public execution reservation",
    )
    reservation_unsigned = dict(execution_reservation)
    reservation_self_digest = reservation_unsigned.pop(
        "reservationContentSHA256", None
    )
    if (
        set(execution_reservation) != PUBLIC_RESERVATION_FIELDS
        or execution_reservation.get("attemptId") != binding["attemptId"]
        or not isinstance(execution_reservation.get("attemptId"), str)
        or ATTEMPT_ID.fullmatch(execution_reservation["attemptId"]) is None
        or execution_reservation["attemptId"]
        != _derive_public_attempt_id(execution_reservation)
        or execution_reservation.get("schemaVersion")
        != "corelm-blind-crossmodel-v1-execution-reservation-v1"
        or execution_reservation.get("suiteId") != SUITE_ID
        or execution_reservation.get("status") != "PUBLIC_EXECUTION_RESERVED"
        or execution_reservation.get("targetPulseTimestamp")
        != TARGET_PULSE_TIMESTAMP
        or execution_reservation.get("oneShotNotBefore") != ONE_SHOT_NOT_BEFORE
        or execution_reservation.get("markerNoLaterThan")
        != MARKER_NO_LATER_THAN
        or execution_reservation.get("hardDeadline") != HARD_DEADLINE
        or execution_reservation.get("retryPermitted") is not False
        or execution_reservation.get("countsTowardScientificVerdict") is not False
        or reservation_self_digest
        != sha256_bytes(canonical_json_bytes(reservation_unsigned))
    ):
        raise EvidenceReleaseError("public execution reservation differs")
    if execution_reservation.get("designRegistration") != {
        "bytes": len(binding_raw["payload/bindings/design.json"]),
        "sha256": sha256_bytes(binding_raw["payload/bindings/design.json"]),
    } or execution_reservation.get("snapshotRegistration") != {
        "bytes": len(binding_raw["payload/bindings/snapshot-registration.json"]),
        "sha256": sha256_bytes(
            binding_raw["payload/bindings/snapshot-registration.json"]
        ),
    }:
        raise EvidenceReleaseError("public execution reservation input binding differs")
    if reservation_snapshot_receipt_raw != snapshot_receipt_raw:
        raise EvidenceReleaseError(
            "execution reservation does not preserve the snapshot receipt bytes"
        )
    if execution_reservation.get("snapshotPublicationReceipt") != {
        "bytes": len(snapshot_receipt_raw),
        "sha256": sha256_bytes(snapshot_receipt_raw),
    }:
        raise EvidenceReleaseError(
            "execution reservation snapshot-receipt binding differs"
        )
    ...
    if not reservation_receipt_raw:
        raise EvidenceReleaseError("execution reservation publication receipt is absent")
...
def _historical_package_release(
    *,
    ...
    source_directories = {
        ...
        "reservationReleaseAssets": _resolved_real_path(
            reservation_release_assets, directory=True
        ),
    }
    source_files = {
        ...
        "reservationReceipt": _resolved_real_path(
            reservation_publication_receipt, directory=False
        ),
    ...
    try:
        ...
        entries.extend(
            _copy_tree(
                source_directories["reservationReleaseAssets"],
                stage,
                "payload/bindings/publication/reservation-assets",
                "reservation-release-asset",
            )
...
def parse_arguments(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    package.add_argument("--reservation-release-assets", type=Path, required=True)
    ...
    try:
        if arguments.command == "package":
            report = package_release(
                ...
                reservation_release_assets=arguments.reservation_release_assets,
