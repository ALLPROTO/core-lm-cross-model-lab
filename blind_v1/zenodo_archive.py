...
from blind_v1.source_archive import SourceArchiveError, verify_source_archive
...
CI_PAYLOADS = {
    "linux-ci-artifact": {
        ...
        "machine": "x86_64",
        "files": {
            "blind_v1-preflight-linux.json",
            "blind_v1-runtime-linux.json",
            "blind_v1-zero-skip-linux.log",
            "blind_v1-design-check-linux.json",
            "blind_v1-release-attestation-known-answer-linux.json",
        },
        "suffix": "linux",
    ...
    "macos-arm64-ci-artifact": {
        ...
        "machine": "arm64",
        "files": {
            "blind_v1-preflight-macos.json",
            "blind_v1-runtime-macos.json",
            "blind_v1-zero-skip-macos.log",
            "blind_v1-design-check-macos.json",
            "blind_v1-release-attestation-known-answer-macos.json",
        },
        "suffix": "macos",
...
def _github_gate_summary(
    ...
    *,
    ...
    linux = [name for name in artifacts if name.startswith("author-blind_v1-linux-development-")]
    macos = [name for name in artifacts if name.startswith("author-blind_v1-macos-development-")]
...
def _verify_ci_payload(
    ...
    *,
    ...
    _verify_runtime_manifest(
        payloads[f"blind_v1-runtime-{suffix}.json"],
    ...
    preflight = _json_object(
        payloads[f"blind_v1-preflight-{suffix}.json"], label=f"{role} preflight"
    )
    platform = preflight.get("platformSafety")
    codec = preflight.get("codec")
    if (
        preflight.get("schemaVersion") != "corelm-blind-crossmodel-v1-preflight-v1"
    ...
    check = _json_object(
        payloads[f"blind_v1-design-check-{suffix}.json"], label=f"{role} design check"
    )
    if (
        check.get("schemaVersion") != "corelm-blind-crossmodel-v1-design-check-v1"
    ...
    known_answer = _json_object(
        payloads[f"blind_v1-release-attestation-known-answer-{suffix}.json"],
    ...
    try:
        log = payloads[f"blind_v1-zero-skip-{suffix}.log"].decode("utf-8", "strict")
...
def _verify_license_material(
    ...
    *,
    codec_commit: str,
    confirmatory_models: Sequence[Mapping[str, Any]],
    model_card_evidence_binding: Mapping[str, Any],
) -> None:
    ...
    if not {
        ...
        MODEL_CARD_EVIDENCE_ARCHIVE_PATH,
    }.issubset(files):
        raise ZenodoArchiveError("LICENSES archive omits its matrix or source evidence")
    model_card_manifest_raw = files[MODEL_CARD_EVIDENCE_ARCHIVE_PATH][1]
    try:
        model_card_summary = validate_model_card_evidence_bytes(
            model_card_manifest_raw,
            {path: value[1] for path, value in files.items()},
            confirmatory_models,
        )
        validate_model_card_design_binding(
            model_card_evidence_binding, model_card_summary
        )
    except ModelCardEvidenceError as error:
        raise ZenodoArchiveError(
            "LICENSES archive model-card evidence differs"
        ) from error
...
def _validate_archival_semantics(
    ...
    *,
    ...
    registered_design_raw = _read_stable_bytes(
        BLIND_V1_ROOT / "design-registration.draft.json",
        label="registered Blind V1 design draft",
        maximum_bytes=MAXIMUM_RECEIPT_BYTES,
    )
    try:
        registered_design = load_json_strict_bytes(
            registered_design_raw, label="registered Blind V1 design draft"
        )
    except ValueError as error:
        raise ZenodoArchiveError(
            "registered Blind V1 design draft is not strict JSON"
        ) from error
    if not isinstance(registered_design, dict):
        raise ZenodoArchiveError(
            "registered Blind V1 design draft is not an object"
        )
    confirmatory_models = registered_design.get("models")
    model_card_evidence_binding = registered_design.get("modelCardEvidence")
    if (
        not isinstance(confirmatory_models, list)
        or not isinstance(model_card_evidence_binding, dict)
    ):
        raise ZenodoArchiveError(
            "registered Blind V1 model-card evidence binding is absent"
        )
    ...
    if github_release["kind"] == "design":
        ...
        confirmatory_models = design.get("models")
        model_card_evidence_binding = design.get("modelCardEvidence")
        if (
            not isinstance(confirmatory_models, list)
            or not isinstance(model_card_evidence_binding, dict)
        ):
            raise ZenodoArchiveError(
                "frozen design model-card evidence binding is absent"
            )
    ...
    _verify_license_material(
        role_bytes("license-material"),
        codec_commit=codec_commit,
        confirmatory_models=confirmatory_models,
        model_card_evidence_binding=model_card_evidence_binding,
    )
