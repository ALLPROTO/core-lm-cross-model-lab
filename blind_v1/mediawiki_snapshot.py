#!/usr/bin/env python3
"""Hardened, auditable MediaWiki collection for blind-v1.

This module implements the request and byte-commitment rules from the local
blind_v1 preregistration draft.  It does not import model weights, the codec, Torch,
...
CRAWL_NOT_BEFORE = (
    datetime(2026, 8, 18, 6, tzinfo=timezone.utc),
...
MAX_ELIGIBLE_CANONICAL_RECORD_BYTES = 8 * 1024 * 1024
CANONICAL_RECORD_SIZE_INELIGIBILITY_REASON = (
    "canonical-record-exceeds-8388608-bytes"
)
MANIFEST_SCHEMA = "corelm-blind-crossmodel-v1-corpus-manifest-v1"
CRAWL_STAGE_SCHEMA = "corelm-blind-crossmodel-v1-crawl-stage-v1"
MAX_HTTP_WIRE_BYTES = 64 * 1024 * 1024
MAX_JSON_BODY_BYTES = 32 * 1024 * 1024
MAX_CRAWL_PAGES = 100_000
USER_AGENT = (
    "CoreLMBlindCrossModelV1Collector/1.0 "
...
def _historical_fetch_and_inventory_revision(
    *,
    ...
    if not all(item["tokenCount"] >= 512 for item in commitments.values()):
        ineligibility_reasons.append(
            "fewer-than-512-tokens-under-at-least-one-tokenizer"
        )
    if len(record_bytes) > MAX_ELIGIBLE_CANONICAL_RECORD_BYTES:
        ineligibility_reasons.append(
            CANONICAL_RECORD_SIZE_INELIGIBILITY_REASON
        )
...
def collect_crawl_stage(
    *,
    ...
    Stage 1 can be run on/after 2026-08-18T06:00Z.  Stage 2 requires a fully
    verified stage 1 and can be run on/after 2026-08-19T06:00Z.  A partial
...
def load_record_bytes(
    ...
    record_commitment = matches[0]["record"]
    if (
        not isinstance(record_commitment, Mapping)
        or type(record_commitment.get("bytes")) is not int
        or not 1
        <= record_commitment["bytes"]
        <= MAX_ELIGIBLE_CANONICAL_RECORD_BYTES
    ):
        raise SnapshotError("selected record exceeds the canonical byte bound")
    value = _read_committed(root, record_commitment)
...
def _validate_token_commitments(
    ...
    *,
    ...
    # Canonical JSON sorts object keys, whereas MODEL_KEYS preserves the
    # preregistered pool order used by selection.  The inventory is a keyed
    # commitment, so require exact membership here and iterate MODEL_KEYS below.
    if not isinstance(commitments, dict) or set(commitments) != set(MODEL_KEYS):
        raise SnapshotError("inventory tokenizer set differs")
...
def _verify_inventory_item(
    ...
    *,
    ...
    token_eligible = _validate_token_commitments(
    ...
    if len(expected_record) > MAX_ELIGIBLE_CANONICAL_RECORD_BYTES:
        expected_reasons.append(CANONICAL_RECORD_SIZE_INELIGIBILITY_REASON)
    eligible = not expected_reasons
    if type(item["eligible"]) is not bool or item["eligible"] != eligible:
        raise SnapshotError("inventory eligibility differs from replayed bounds")
...
def verify_corpus_snapshot(
    ...
    Supplying the six frozen pool tokenizers additionally recomputes every token
    ...
    if manifest["suiteId"] != "corelm-blind-crossmodel-v1":
