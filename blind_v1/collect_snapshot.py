#!/usr/bin/env python3
"""Collect and independently replay the registered blind-v1 corpus snapshot.
...
six committed ``tokenizer.json`` files into owned byte buffers, and then
...
from blind_v1.fetch_assets import (  # noqa: E402
    DEFAULT_MANIFEST,
    AssetFetchError,
    AssetSpecification,
    load_manifest,
...
def _freeze_ready_report(
    *,
    ...
    return {
        "schemaVersion": "corelm-blind-crossmodel-v1-collector-run-v1",
    ...
    *,
    ...
    if phase in {"crawl-1", "crawl-2"}:
        ...
        if (
            ...
            raise CollectorCLIError("crawl stage completion manifest differs")
        return {
            "schemaVersion": "corelm-blind-crossmodel-v1-collector-stage-run-v1",
