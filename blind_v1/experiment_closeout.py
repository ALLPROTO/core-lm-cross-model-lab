#!/usr/bin/env python3
"""Canonical public closeout classifications for the blind-v1 experiment.
...
OBSERVATION_SCHEMA = (
    "corelm-blind-crossmodel-v1-empty-result-root-observation-v1"
)
AUDIT_REPORT_SCHEMA = "corelm-blind-crossmodel-v1-empty-result-root-audit-v1"
...
DESIGN_PUBLICATION_DEADLINE = "2026-08-09T00:00:00Z"
SNAPSHOT_PUBLICATION_DEADLINE = "2026-08-20T18:00:00Z"
EXECUTION_HARD_DEADLINE = "2026-08-22T18:00:00Z"
EVIDENCE_PUBLICATION_DEADLINE = "2026-08-26T18:00:00Z"
CLOSEOUT_PUBLICATION_DEADLINE = "2026-08-30T18:00:00Z"
EVIDENCE_REPOSITORY = "ALLPROTO/core-lm-cross-model-lab"
EVIDENCE_TAG = "corelm-blind-crossmodel-v1-evidence"
CLOSEOUT_TAG = "corelm-blind-crossmodel-v1-closeout"
SIGNATURE_TYPE = "SSH"
...
PUBLIC_ATTEMPT_ID = re.compile(r"20260821T180000Z-[0-9a-f]{16}\Z")
...
class PublicationBindings:
    ...
    reservation_publication_receipt_sha256: str
    execution_reservation_sha256: str
    reservation_release_manifest_sha256: str
    closeout_source_commit: str
    closeout_source_tree: str

    def __post_init__(self) -> None:
        if (
            not isinstance(self.reserved_attempt_id, str)
            or PUBLIC_ATTEMPT_ID.fullmatch(self.reserved_attempt_id) is None
        ):
            raise ExperimentCloseoutError(
                "reserved attempt identity differs from the frozen suite"
            )
        for field, value in (
            ...
            (
                ...
                self.reservation_publication_receipt_sha256,
            ),
            ("execution reservation", self.execution_reservation_sha256),
            (
                "reservation release manifest",
                self.reservation_release_manifest_sha256,
            ),
        ):
    ...
    def from_exact_bytes(
        ...
        *,
        ...
        reservation_publication_receipt: bytes,
        execution_reservation: bytes,
        reservation_release_manifest: bytes,
        ...
        reservation = _load_canonical_line(
            execution_reservation,
            label="public execution reservation",
        )
        reserved_attempt_id = reservation.get("attemptId")
        if (
            ...
            or not isinstance(reserved_attempt_id, str)
            or PUBLIC_ATTEMPT_ID.fullmatch(reserved_attempt_id) is None
        ):
            raise ExperimentCloseoutError(
                "public execution reservation identity differs"
            )
        return cls(
            ...
            reservation_publication_receipt_sha256=sha256_bytes(
                reservation_publication_receipt
            ),
            execution_reservation_sha256=sha256_bytes(execution_reservation),
            reservation_release_manifest_sha256=sha256_bytes(
                reservation_release_manifest
            ),
    ...
    def as_dict(self, *, evidence_receipt_sha256: str | None) -> dict[str, Any]:
        ...
        return {
            "reservedAttemptId": self.reserved_attempt_id,
            ...
            "executionReservationSHA256": self.execution_reservation_sha256,
