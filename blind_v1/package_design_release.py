...
``blind_v1.release_receipt`` verifies it separately.
"""
...
from blind_v1 import freeze_manifest  # noqa: E402
...
from blind_v1.release_receipt import REQUIRED_ASSET_ROLES  # noqa: E402
...
SHA256_MANIFEST_SCHEMA = (
    "corelm-blind-crossmodel-v1-design-release-sha256-manifest-v1"
)
VERIFICATION_SCHEMA = (
    "corelm-blind-crossmodel-v1-design-release-verification-v1"
)
...
class CIArtifactVerification:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            "schemaVersion": (
                "corelm-blind-crossmodel-v1-ci-artifact-verification-v1"
            ),
            "suiteId": SUITE_ID,
...
class DesignReleaseVerification:
    ...
    model_card_evidence_sha256: str
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            ...
            "modelCardEvidenceSHA256": self.model_card_evidence_sha256,
...
def _verify_ci_artifact_payload(
    *,
    ...
    names = {
        "preflight": f"blind_v1-preflight-{suffix}.json",
        "runtime-manifest": f"blind_v1-runtime-{suffix}.json",
        "zero-skip-log": f"blind_v1-zero-skip-{suffix}.log",
        "design-check": f"blind_v1-design-check-{suffix}.json",
    ...
    if (
        ...
        != "corelm-blind-crossmodel-v1-preflight-v1"
    ...
    if (
        ...
        != "corelm-blind-crossmodel-v1-design-check-v1"
    ...
    *,
    ...
    try:
        model_card_summary = verify_model_card_evidence_tree(
            PROJECT_ROOT, design["models"]
        )
        validate_model_card_design_binding(
            design.get("modelCardEvidence"), model_card_summary
        )
    except (KeyError, ModelCardEvidenceError) as error:
        raise DesignReleaseError("model-card evidence failed verification") from error
    ...
    if (
        ...
        raise DesignReleaseError("model-card evidence differs from freeze commitment")
    ...
    expected_author_verification, expected_ci = freeze_manifest._gate_manifest_sections(
        verified_gate,
        implementation_repository=freeze["implementation"]["repository"],
    )
    if freeze["authorVerification"] != expected_author_verification:
        raise DesignReleaseError(
            "freeze author verification differs from GitHub CI gate receipt"
        )
...
def verify_design_release_package(
    ...
    *,
    ...
    return DesignReleaseVerification(
        ...
        model_card_evidence_sha256=report["modelCardEvidenceSHA256"],
