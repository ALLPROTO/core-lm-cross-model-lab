...
from blind_v1.package_design_release import (  # noqa: E402
    DesignReleaseError,
    verify_design_release_package,
)
from blind_v1.package_execution_reservation import (  # noqa: E402
    ExecutionReservationError,
    verify_execution_reservation_package,
)
from blind_v1.package_snapshot_release import (  # noqa: E402
    SnapshotReleaseError,
    verify_snapshot_release,
)
from blind_v1.preflight import (  # noqa: E402
    platform_safety,
...
from blind_v1.publication import (  # noqa: E402
    PublicationError,
...
from blind_v1.state_machine import (  # noqa: E402
    StateMachineError,
...
PRIVATE_SCHEMA = "corelm-blind-crossmodel-v1-private-snapshot-manifest-v1"
FREEZE_SCHEMA = "corelm-blind-crossmodel-v1-freeze-manifest-v1"
RESULT_SCHEMA = "corelm-blind-crossmodel-v1-result-v1"
ONE_SHOT_NOT_BEFORE = datetime(2026, 8, 21, 18, tzinfo=timezone.utc)
MARKER_DEADLINE = datetime(2026, 8, 21, 18, 15, tzinfo=timezone.utc)
HARD_DEADLINE = datetime(2026, 8, 22, 18, tzinfo=timezone.utc)
ATTEMPT_ID = re.compile(r"20260821T180000Z-[0-9a-f]{16}\Z")
HEX_40 = re.compile(r"[0-9a-f]{40}\Z")
HEX_64 = re.compile(r"[0-9a-f]{64}\Z")
MAX_FAILURE_REASON = 4096
MAX_OUTER_AUTHORIZATION_BYTES = 4096
OUTER_AUTHORIZATION_SCHEMA = (
    "corelm-blind-crossmodel-v1-outer-supervision-authorization-v1"
)
OUTER_SUPERVISION_POLL_MILLISECONDS = 250
...
SCIENTIFIC_HASH_INPUT = "corelm-blind-crossmodel-v1"
SCIENTIFIC_HASH_KNOWN_ANSWER = -5471761816802073765
SCIENTIFIC_WORKER_AUTHORIZATION_SCHEMA = (
    "corelm-blind-crossmodel-v1-worker-authorization-v1"
)
MAX_WORKER_AUTHORIZATION_BYTES = 16 * 1024
...
def require_reserved_attempt_id(
    reservation_verification: Mapping[str, Any],
) -> str:
    """Return the exact attempt identity committed by the public reservation."""

    value = reservation_verification.get("attemptId")
    marker_deadline = reservation_verification.get("markerNoLaterThan")
    if (
        not isinstance(value, str)
        or ATTEMPT_ID.fullmatch(value) is None
        or marker_deadline != MARKER_DEADLINE.strftime("%Y-%m-%dT%H:%M:%SZ")
    ):
        raise RunnerError(
            "public execution reservation attempt/marker identity differs"
        )
    return value


def scientific_tokenizer_vocabulary_sizes(
    design: Mapping[str, Any],
) -> dict[str, int]:
    """Return the exact tokenizer domains used by corpus-token evidence.

    A model's logits vocabulary may be padded beyond the tokenizer domain
    (the two registered Pythia revisions use 50,304 versus 50,277).  Corpus
    ledgers and page-token records therefore bind ``tokenizerVocabSize``;
    using the logits dimension here would reject every selected Pythia cell.
    """

    models = design.get("models")
    if not isinstance(models, list) or len(models) != len(MODELS):
        raise RunnerError("frozen model pool is absent or incomplete")
    result: dict[str, int] = {}
    for model in models:
        if not isinstance(model, Mapping):
            raise RunnerError("frozen model specification is invalid")
        key = model.get("key")
        model_vocabulary_size = model.get("vocabSize")
        tokenizer_vocabulary_size = model.get("tokenizerVocabSize")
        if (
            not isinstance(key, str)
            or key in result
            or key not in MODELS
            or type(model_vocabulary_size) is not int
            or type(tokenizer_vocabulary_size) is not int
            or not 1 <= tokenizer_vocabulary_size <= model_vocabulary_size <= 2**32
        ):
            raise RunnerError("frozen model/tokenizer vocabulary binding differs")
        result[key] = tokenizer_vocabulary_size
    if set(result) != set(MODELS):
        raise RunnerError("frozen model/tokenizer vocabulary set differs")
    return result
...
def ensure_one_shot_window(now: datetime) -> None:
    ...
    if observed >= MARKER_DEADLINE:
        raise RunnerError(
            "scientific marker deadline expired before marker creation"
        )
...
def verify_external_attempt_time_anchor(verification: Mapping[str, Any]) -> datetime:
    ...
    if observed < ONE_SHOT_NOT_BEFORE or observed >= MARKER_DEADLINE:
        raise RunnerError(
            "verified NIST HTTPS Date is outside the registered marker window"
        )
    return observed
...
def _validate_schema(instance: Any, filename: str) -> None:
    ...
    schema = load_json_strict(BLIND_V1_ROOT / "schemas" / filename)
...
def validate_frozen_design(design: Any) -> None:
    ...
    reference = load_json_strict(BLIND_V1_ROOT / "design-registration.draft.json")
    if design.get("schemaVersion") != "corelm-blind-crossmodel-v1-design-v1":
    ...
    normalized["beacon"]["trustBundleStatus"] = reference["beacon"][
        "trustBundleStatus"
    ]
...
def _verify_reservation_publication_window(
    publication: Any,
    design: Mapping[str, Any],
    reservation_verification: Mapping[str, Any],
) -> None:
    """Bind asserted reservation time to the public release timestamp window."""

    release = design["reservationRelease"]
    try:
        not_before = datetime.strptime(
            release["publishNotBefore"], "%Y-%m-%dT%H:%M:%SZ"
        ).replace(tzinfo=timezone.utc)
        deadline = datetime.strptime(
            release["publishNoLaterThan"], "%Y-%m-%dT%H:%M:%SZ"
        ).replace(tzinfo=timezone.utc)
        reserved = datetime.strptime(
            reservation_verification["reservedAt"], "%Y-%m-%dT%H:%M:%SZ"
        ).replace(tzinfo=timezone.utc)
        published = datetime.strptime(
            publication.receipt.published_at, "%Y-%m-%dT%H:%M:%SZ"
        ).replace(tzinfo=timezone.utc)
        attested = datetime.strptime(
            publication.receipt.attested_at, "%Y-%m-%dT%H:%M:%SZ"
        ).replace(tzinfo=timezone.utc)
    except (KeyError, TypeError, ValueError) as error:
        raise RunnerError("execution reservation publication times are invalid") from error
    if not not_before <= reserved <= published <= attested < deadline:
        raise RunnerError(
            "execution reservation publication is outside its half-open window"
        )
...
def verify_runtime_live(runtime_manifest: Mapping[str, Any], runtime_root: Path) -> None:
    ...
    if runtime_manifest.get("schemaVersion") != "corelm-blind-crossmodel-v1-runtime-manifest-v1":
...
def _historical_prepare_private_snapshot(
    *,
    ...
    reservation_publication_receipt_path: Path,
    reservation_release_asset_root: Path,
    ...
    reservation_release = design["reservationRelease"]
    try:
        reservation_verification = verify_execution_reservation_package(
            reservation_release_asset_root,
            design_raw=design_path.read_bytes(),
            snapshot_raw=snapshot_raw,
        )
    except ExecutionReservationError as error:
        raise RunnerError(f"execution reservation package failed: {error}") from error
    reservation_publication = verify_publication(
        reservation_publication_receipt_path,
        reservation_release_asset_root,
        kind="reservation",
        tag=reservation_release["tag"],
        deadline=reservation_release["publishNoLaterThan"],
        signing_public_key_path=signing_public_key_path,
        signing_key_fingerprint=reservation_release["signingKeyFingerprint"],
        signing_public_key_sha256=reservation_release["signingPublicKeySHA256"],
        expected_role_paths={
            "execution-reservation": reservation_release_asset_root
            / "execution-reservation.json",
            "snapshot-publication-receipt": reservation_release_asset_root
            / "snapshot-publication-receipt.json",
            "sha256-manifest": reservation_release_asset_root
            / "sha256-manifest.json",
        },
        cryptographic_attestation_verifier=(
            cryptographic_attestation_verifier
    ...
    _verify_reservation_publication_window(
        reservation_publication,
        design,
    ...
    try:
        metadata_sources = (
            ...
            (
                reservation_publication_receipt_path,
                "publication/reservation-receipt.json",
                "reservation-publication-receipt",
            ),
            (
        ...
        manifest: dict[str, Any] = {
            ...
            "reservationPublicationReceiptSHA256": (
                reservation_publication.receipt_sha256
            ),
        ...
        publish_new_path_exclusive(temporary, destination)
        return {
...
def _canonical_worker_path(path: Path) -> str:
    try:
        raw = os.fspath(path)
    except TypeError as error:
        raise RunnerError("worker capability path is invalid") from error
    if not isinstance(raw, str) or not raw or "\x00" in raw:
        raise RunnerError("worker capability path is invalid")
    try:
        return os.path.realpath(os.path.abspath(raw))
    except OSError as error:
        raise RunnerError("worker capability path cannot be canonicalized") from error


def _canonical_control_bytes(path: Path, *, label: str) -> tuple[dict[str, Any], bytes]:
    try:
        raw = path.read_bytes()
        value = load_json_strict_bytes(raw, label=label)
    except (OSError, ValueError) as error:
        raise RunnerError(f"{label} cannot be loaded canonically") from error
...
def _worker_authorization(
    *,
    ...
    """Create an ephemeral post-marker/post-selection worker capability.

    This blocks accidental standalone invocation.  It does not claim to
    isolate the experiment from a malicious owner of the host machine.
    """

    if output_root.exists() or output_root.is_symlink():
        raise RunnerError("worker output root already exists")
    expected_job_raw = canonical_json_bytes(job) + b"\n"
    observed_job, job_raw = _canonical_control_bytes(job_path, label="worker job")
    if observed_job != job or job_raw != expected_job_raw:
        raise RunnerError("worker job changed before authorization")
    expected_selection_raw = canonical_json_bytes(selection) + b"\n"
    observed_selection, selection_raw = _canonical_control_bytes(
        result_root / "selection.json", label="scientific selection"
    )
    if observed_selection != selection or selection_raw != expected_selection_raw:
        raise RunnerError("scientific selection changed before authorization")
    try:
        marker = load_attempt_marker(result_root)
    except StateMachineError as error:
        raise RunnerError("durable attempt marker is invalid") from error
    if (
        marker is None
        or marker.get("suiteId") != SUITE_ID
        or marker.get("attemptId") != attempt
        or observed_selection.get("suiteId") != SUITE_ID
        or observed_selection.get("modelExecutionOrder", []).count(model_key) != 1
    ):
        raise RunnerError("worker authorization lifecycle identity differs")
    marker_path = result_root / "attempt-marker.json"
    _marker_value, marker_raw = _canonical_control_bytes(
        marker_path, label="durable attempt marker"
    )
    private_manifest, private_manifest_raw = _canonical_control_bytes(
        private_root / "private-snapshot-manifest.json",
        label="private snapshot manifest",
    )
    private_manifest_sha = sha256_bytes(private_manifest_raw)
    snapshot_registration_sha = marker.get("snapshotRegistrationSHA256")
    if (
        marker.get("privateSnapshotManifestSHA256") != private_manifest_sha
        or private_manifest.get("suiteId") != SUITE_ID
        or private_manifest.get("status") != "SEALED_BEFORE_ATTEMPT"
        or private_manifest.get("snapshotRegistrationSHA256")
        != snapshot_registration_sha
        or observed_selection.get("snapshotRegistrationSHA256")
        != snapshot_registration_sha
        or not isinstance(snapshot_registration_sha, str)
        or HEX_64.fullmatch(snapshot_registration_sha) is None
    ):
        raise RunnerError("worker authorization snapshot identity differs")
    canonical_job = _canonical_worker_path(job_path)
    canonical_snapshot = _canonical_worker_path(private_root)
    canonical_codec = _canonical_worker_path(private_root / "codec")
    canonical_output = _canonical_worker_path(output_root)
    if (
        Path(canonical_job)
        != Path(_canonical_worker_path(result_root)) / "jobs" / f"{model_key}.json"
        or Path(canonical_codec) != Path(canonical_snapshot) / "codec"
        or Path(canonical_output).parent.name != "workers"
        or Path(canonical_output).parent.parent
        != Path(_canonical_worker_path(result_root))
    ):
        raise RunnerError("worker authorization path topology differs")
    return {
        "schemaVersion": SCIENTIFIC_WORKER_AUTHORIZATION_SCHEMA,
        "suiteId": SUITE_ID,
        "attemptId": attempt,
        "attemptMarkerSHA256": sha256_bytes(marker_raw),
        "selectionSHA256": sha256_bytes(selection_raw),
        "jobSHA256": sha256_bytes(job_raw),
        "modelKey": model_key,
        "snapshotRegistrationSHA256": snapshot_registration_sha,
        "privateSnapshotManifestSHA256": private_manifest_sha,
        "canonicalJobPath": canonical_job,
        "canonicalSnapshotRoot": canonical_snapshot,
        "canonicalCodecRoot": canonical_codec,
        "canonicalOutputRoot": canonical_output,
        "capabilityNonce": secrets.token_hex(32),
    }


def _open_worker_authorization_pipe(authorization: Mapping[str, Any]) -> int:
    raw = canonical_json_bytes(authorization) + b"\n"
    if not 0 < len(raw) <= MAX_WORKER_AUTHORIZATION_BYTES:
        raise RunnerError("worker authorization exceeds its fixed bound")
    read_descriptor, write_descriptor = os.pipe()
    try:
        offset = 0
        while offset < len(raw):
            written = os.write(write_descriptor, raw[offset:])
            if written < 1:
                raise RunnerError("worker authorization pipe write made no progress")
            offset += written
    except BaseException:
...
def _supervise_worker(
    ...
    *,
    ...
    if (
        ...
        raise RunnerError("worker inherited descriptor set is invalid")
    ...
    try:
        while True:
            ...
            try:
                ...
                raise
            # Count every observation, including the one made after poll()
            # reports process exit.  This is a sampled watchdog, not a kernel
            # RLIMIT; sub-poll-interval peaks can remain unobserved.
...
def _run_workers(
    *,
    ...
    for model_key in selection["modelExecutionOrder"]:
        ...
        authorization_fd = _open_worker_authorization_pipe(authorization)
        try:
                    ...
                    supervisor_receipt = _supervise_worker(
                        _networkless_macos_command(
                            _scientific_python_command(
                                sys.executable,
                                str(private_root / "lab" / "blind_v1" / "model_worker.py"),
                                "--job",
                                str(job_path),
                                "--snapshot-root",
                                str(private_root),
...
def _outer_supervision_authorization_bytes(
    *,
    private_root: Path,
    result_root: Path,
    outer_pid: int,
    ...
    if (
        type(outer_pid) is not int
        or type(child_pid) is not int
        or outer_pid <= 1
        or child_pid <= 1
        or outer_pid == child_pid
        or not isinstance(nonce, str)
        or HEX_64.fullmatch(nonce) is None
    ):
        raise RunnerError("outer-supervision authorization identity is invalid")
    value = {
        "schemaVersion": OUTER_AUTHORIZATION_SCHEMA,
        "status": "OUTER_SUPERVISION_AUTHORIZED",
        "suiteId": SUITE_ID,
        "privateRoot": _absolute_path_text(private_root),
        "resultRoot": _absolute_path_text(result_root),
        "outerPid": outer_pid,
        "childPid": child_pid,
        "processGroupId": child_pid,
        "hardDeadline": "2026-08-22T18:00:00Z",
        "pollMilliseconds": OUTER_SUPERVISION_POLL_MILLISECONDS,
        "nonce": nonce,
...
def _read_outer_supervision_authorization(
    descriptor: int, *, private_root: Path, result_root: Path
) -> dict[str, Any]:
    """Consume the one-use anonymous-pipe handoff from the outer watchdog."""

    if type(descriptor) is not int or descriptor < 3:
        raise RunnerError("private execution lacks an outer-supervision pipe")
    try:
        metadata = os.fstat(descriptor)
        if not stat.S_ISFIFO(metadata.st_mode):
            raise RunnerError("outer-supervision authorization FD is not a pipe")
        chunks: list[bytes] = []
        observed = 0
        while observed <= MAX_OUTER_AUTHORIZATION_BYTES:
            chunk = os.read(
                descriptor,
                min(4096, MAX_OUTER_AUTHORIZATION_BYTES + 1 - observed),
            )
            if not chunk:
                break
            chunks.append(chunk)
            observed += len(chunk)
        if observed > MAX_OUTER_AUTHORIZATION_BYTES:
            raise RunnerError("outer-supervision authorization exceeds its bound")
    except OSError as error:
        raise RunnerError("outer-supervision authorization cannot be read") from error
    finally:
        try:
            os.close(descriptor)
        except OSError:
    ...
    if not raw.endswith(b"\n") or raw == b"\n":
        raise RunnerError("outer-supervision authorization framing differs")
    value = load_json_strict_bytes(
        raw[:-1], label="outer-supervision authorization"
    )
    if not isinstance(value, dict) or canonical_json_bytes(value) + b"\n" != raw:
        raise RunnerError("outer-supervision authorization is noncanonical")
    expected_fields = {
        "schemaVersion",
        "status",
        "suiteId",
    ...
    if set(value) != expected_fields:
        raise RunnerError("outer-supervision authorization fields differ")
    current_pid = os.getpid()
    current_parent = os.getppid()
    current_group = os.getpgrp()
    if (
        value.get("schemaVersion") != OUTER_AUTHORIZATION_SCHEMA
        or value.get("status") != "OUTER_SUPERVISION_AUTHORIZED"
        or value.get("suiteId") != SUITE_ID
        or value.get("privateRoot") != _absolute_path_text(private_root)
        or value.get("resultRoot") != _absolute_path_text(result_root)
        or value.get("outerPid") != current_parent
        or value.get("childPid") != current_pid
        or value.get("processGroupId") != current_group
        or current_group != current_pid
        or value.get("hardDeadline") != "2026-08-22T18:00:00Z"
        or value.get("pollMilliseconds") != OUTER_SUPERVISION_POLL_MILLISECONDS
        or not isinstance(value.get("nonce"), str)
        or HEX_64.fullmatch(value["nonce"]) is None
        or value.get("retryPermitted") is not False
    ):
        raise RunnerError("outer-supervision authorization binding differs")
    return value


def _spawn_authorized_private_execution(
    command: tuple[str, ...],
    *,
    cwd: Path,
    environment: Mapping[str, str],
    ...
    """Spawn one session-leader child and authorize it through an owned pipe."""

    authorization_fd, authorization_writer = os.pipe()
    process: subprocess.Popen[bytes] | None = None
    try:
        process = subprocess.Popen(
            command + ("--outer-authorization-fd", str(authorization_fd)),
            cwd=cwd,
            env=dict(environment),
            stdin=subprocess.DEVNULL,
            start_new_session=True,
        ...
        os.close(authorization_fd)
        authorization_fd = -1
        payload = _outer_supervision_authorization_bytes(
            private_root=private_root,
            result_root=result_root,
            outer_pid=os.getpid(),
        ...
        while view:
            written = os.write(authorization_writer, view)
            if written <= 0:
                raise RunnerError("short outer-supervision authorization write")
            view = view[written:]
        os.close(authorization_writer)
        authorization_writer = -1
        return process
    except BaseException:
...
def _historical_execute_private_one_shot(
    ...
    *,
    ...
    if (
        outer_authorization.get("status") != "OUTER_SUPERVISION_AUTHORIZED"
        or outer_authorization.get("privateRoot") != _absolute_path_text(private_root)
        or outer_authorization.get("resultRoot") != _absolute_path_text(result_root)
    ):
        raise RunnerError("private execution is not bound to the outer supervisor")
    ...
    expected_script = (private_root / "lab" / "blind_v1" / "runner.py").resolve(strict=True)
    ...
    reservation_root = private_root / "publication" / "reservation-assets"
    try:
        reservation_verification = verify_execution_reservation_package(
            reservation_root,
            design_raw=design_path.read_bytes(),
        ...
        raise RunnerError(f"private execution reservation failed: {error}") from error
    reservation_release = design["reservationRelease"]
    reservation_publication = verify_publication(
        bound_path("publication/reservation-receipt.json"),
        reservation_root,
        kind="reservation",
        tag=reservation_release["tag"],
        deadline=reservation_release["publishNoLaterThan"],
        ...
        signing_public_key_sha256=reservation_release["signingPublicKeySHA256"],
        expected_role_paths={
            "execution-reservation": bound_path(
                "publication/reservation-assets/execution-reservation.json"
            ),
            "snapshot-publication-receipt": bound_path(
    ...
    if (
        ...
        or reservation_publication.receipt_sha256
        != private_manifest["reservationPublicationReceiptSHA256"]
    ...
    try:
        if utc_now() >= MARKER_DEADLINE:
            raise RunnerError(
                "marker deadline crossed while the attempt marker became durable"
            )
        write_new_bytes(
        ...
        pool_tokenizer_vocabulary_sizes = scientific_tokenizer_vocabulary_sizes(design)
        selected_models = selection["modelExecutionOrder"]
        ledger_commitments = selected_ledger_token_commitments(
            full_ledgers,
            models=[item["key"] for item in design["models"]],
            selected_models=selected_models,
            vocabulary_sizes=pool_tokenizer_vocabulary_sizes,
            selected_revisions=selected_revisions,
        )
        selected_tokenizer_vocabulary_sizes = {
            key: pool_tokenizer_vocabulary_sizes[key] for key in selected_models
        }
        selected_model_vocabulary_sizes = {
            key: model_specs[key]["vocabSize"] for key in selected_models
        }
        verify_page_token_evidence(
            page_token_records,
            raw_records,
            ...
            tokenizer_vocabulary_sizes=selected_tokenizer_vocabulary_sizes,
            model_vocabulary_sizes=selected_model_vocabulary_sizes,
            selected_revisions=selected_revisions,
            ...
            try:
                verifier_supervisor_receipt = _supervise_worker(
                    _networkless_macos_command(
                        _scientific_python_command(
                            sys.executable,
                            str(private_root / "lab" / "blind_v1" / "verify_evidence.py"),
                            "--evidence-root",
                            str(result_root),
                            "--codec-root",
                            str(private_root / "codec"),
...
def reexec_private_one_shot(
    *, private_root: Path, result_root: Path
    ...
    script = _private_file(private_root, "lab/blind_v1/runner.py")
    ...
    process = _spawn_authorized_private_execution(
        command,
        cwd=private_root / "lab",
    ...
    try:
        while True:
            ...
            time.sleep(
                design["execution"]["outerSupervisorPollMilliseconds"] / 1000
            )
    except BaseException as error:
        try:
            _terminate_process_group(process)
...
def parse_arguments() -> argparse.Namespace:
    ...
    prepare.add_argument(
        "--reservation-publication-receipt", type=Path, required=True
    )
    prepare.add_argument("--reservation-release-assets", type=Path, required=True)
    ...
    try:
        ...
        if arguments.command == "prepare":
            summary = prepare_private_snapshot(
                ...
                reservation_publication_receipt_path=(
                    arguments.reservation_publication_receipt
                ),
                reservation_release_asset_root=(
                    arguments.reservation_release_assets
                ),
        ...
        if arguments.private_execution:
            outer_authorization = _read_outer_supervision_authorization(
                arguments.outer_authorization_fd,
            ...
            print(state)
            return 0
        if arguments.outer_authorization_fd is not None:
            raise RunnerError(
                "outer-supervision pipe is valid only for private execution"
            )
        ...
        print(f"BLIND-V1 RUNNER FAIL: {error}", file=sys.stderr)
        return 1
