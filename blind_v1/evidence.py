#!/usr/bin/env python3
"""Independent blind-v1 raw-evidence arithmetic and container verification."""
...
from blind_v1.protocol import canonical_json_bytes, evaluate_model_aggregate
...
def verify_page_token_evidence(
    ...
    *,
    ...
    for model in models:
        tokenizer_vocabulary_size = tokenizer_vocabulary_sizes[model]
        model_vocabulary_size = model_vocabulary_sizes[model]
        if (
            type(tokenizer_vocabulary_size) is not int
            or type(model_vocabulary_size) is not int
            or not 1 <= tokenizer_vocabulary_size <= model_vocabulary_size <= 2**32
        ):
            raise EvidenceError(f"registered vocabulary sizes are invalid: {model}")
    ...
    for record in records:
        ...
        tokenizer_vocabulary_size = tokenizer_vocabulary_sizes.get(model)
        if (
            tokenizer_vocabulary_size is None
            or record["vocabSize"] != tokenizer_vocabulary_size
        ):
    ...
    for identity, page_record in by_page.items():
        ...
        tokenizer_vocabulary_size = page_record["vocabSize"]
        model_vocabulary_size = model_vocabulary_sizes[identity[0]]
        token_ids = page_record["first512TokenIds"]
        for prediction_index, item in enumerate(records_for_page):
            if item["targetTokenId"] >= tokenizer_vocabulary_size:
                raise EvidenceError(
                    "raw targetTokenId is outside the registered tokenizer vocabulary"
                )
            for field in ("baselineTop1TokenId", "candidateTop1TokenId"):
                if item[field] >= model_vocabulary_size:
                    raise EvidenceError(
                        f"raw {field} is outside the registered model vocabulary"
                    )
...
def selected_ledger_token_commitments(
    ...
    *,
    ...
    """Validate the full pool ledger, then return exact selected-model revisions."""
    ...
    if (
        ...
        raise EvidenceError("selected full-ledger model order is invalid")
    ...
    for corpus in selected_revisions:
        ...
        for record in records:
            ...
            for model in models:
                ...
                if revision in selected_set and model in selected_model_set:
...
def verify_sha256_manifest(root: Path, manifest_path: Path) -> tuple[dict[str, Any], str]:
    ...
    if value["schemaVersion"] != "corelm-blind-crossmodel-v1-evidence-manifest-v1":
