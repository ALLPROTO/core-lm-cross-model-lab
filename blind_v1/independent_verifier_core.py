#!/usr/bin/env python3
"""Second implementation of the blind-v1 scientific verification rules.
...
SELECTION_DOMAIN = b"corelm-blind-crossmodel-v1/select\0"
...
SNAPSHOT_NOT_BEFORE = datetime(2026, 8, 19, 6, tzinfo=timezone.utc)
SNAPSHOT_DEADLINE = datetime(2026, 8, 20, 18, tzinfo=timezone.utc)
...
NIST_VERIFY_SCHEMA = "corelm-blind-crossmodel-v1-nist-verification-v1"
...
SELECTION_SCHEMA = "corelm-blind-crossmodel-v1-selection-v1"
INDEPENDENT_RESULT_SCHEMA = (
    "corelm-blind-crossmodel-v1-independent-verification-v1"
)
...
# Deliberately duplicated from the frozen contract instead of imported from
# producer code: the second implementation must reject any other suite-v1
# identity while preserving its independent fail-closed boundary.
...
def _validate_snapshot(snapshot: Any, *, allow_fixture: bool) -> bool:
    ...
    if snapshot.get("schemaVersion") == "corelm-blind-crossmodel-v1-snapshot-fixture-v1":
        expected = {
    ...
    if snapshot["schemaVersion"] != "corelm-blind-crossmodel-v1-snapshot-registration-v1":
...
def derive_selection(
    ...
    *,
    ...
    if (
        len(project_pool) != 3
        or len(set(project_pool)) != 3
        or any(not isinstance(item, str) or not item for item in project_pool)
    ):
        raise IndependentVerificationError("projects must be three unique strings")
    if (
        len(model_pool) != 6
        or len(set(model_pool)) != 6
        or any(not isinstance(item, str) or not item for item in model_pool)
    ):
        raise IndependentVerificationError("models must be six unique strings")
...
def validate_worker_job(job: Any) -> None:
    ...
    if model["key"] not in MODELS:
        raise IndependentVerificationError(
            "worker model is outside the frozen six-model pool"
        )
...
def extract_ledger_token_commitments(
    ...
    *,
    ...
    if (
        ...
        or not set(selected_models).issubset(models)
    ):
        raise IndependentVerificationError("selected full-ledger model order differs")
    ...
    for project, wanted in selected_revisions.items():
        ...
        for record in records:
            ...
            for model in models:
                ...
                if revision in wanted_set and model in selected_model_set:
...
def verify_page_token_bindings(
    ...
    *,
    ...
    models: list[str],
    corpora: list[str],
    tokenizer_vocabulary_sizes: dict[str, int],
    model_vocabulary_sizes: dict[str, int],
    selected_revisions: dict[str, list[int]],
    ...
    if (
        set(tokenizer_vocabulary_sizes) != set(models)
        or set(model_vocabulary_sizes) != set(models)
    ):
        raise IndependentVerificationError("page-token vocabulary commitments differ")
    if any(
        type(tokenizer_vocabulary_sizes[model]) is not int
        or type(model_vocabulary_sizes[model]) is not int
        or not 1
        <= tokenizer_vocabulary_sizes[model]
        <= model_vocabulary_sizes[model]
        <= 2**32
        for model in models
    ):
        raise IndependentVerificationError("page-token vocabulary sizes are invalid")
    ...
    for item in records:
        ...
        vocab = tokenizer_vocabulary_sizes.get(item["modelKey"])
    ...
    for identity, page in by_page.items():
        ...
        for index, item in enumerate(raw):
            if item["targetTokenId"] >= page["vocabSize"]:
                raise IndependentVerificationError(
                    "raw target token ID exceeds tokenizer vocabulary"
                )
            model_vocab = model_vocabulary_sizes[identity[0]]
            if any(
                item[field] >= model_vocab
                for field in ("baselineTop1TokenId", "candidateTop1TokenId")
            ):
                raise IndependentVerificationError(
                    "raw top-1 token ID exceeds model vocabulary"
                )
...
def _validate_independent_trust_policy(
    ...
    if not isinstance(value, dict) or set(value) != fields:
        raise IndependentVerificationError("trust policy fields differ")
    ...
    if (
        ...
        raise IndependentVerificationError(
            "trust policy must allow exactly one lowercase certificate ID"
        )
    if value["rotationPolicy"] != TRUST_ROTATION_POLICY:
        raise IndependentVerificationError("certificate rotation policy differs")
    if value["revocationPolicy"] != TRUST_REVOCATION_POLICY:
        raise IndependentVerificationError("certificate revocation policy differs")
    if value["revocationChecked"] is not False:
        raise IndependentVerificationError(
            "no-revocation-check policy must declare revocationChecked=false"
        )
    if value["revocationResidualRisk"] != TRUST_REVOCATION_RESIDUAL_RISK:
        raise IndependentVerificationError("revocation residual-risk disclosure differs")
    ...
    if (
        ...
        raise IndependentVerificationError(
            "accepted leaf ExtendedKeyUsage policy differs"
        )
    if (
        ...
        raise IndependentVerificationError(
            "certificate map differs from singleton allowedCertificateIds"
        )
    return tuple(allowed)


def _verify_independent_leaf_policy(certificate: _Certificate) -> None:
    if not certificate.basic_constraints_present or certificate.is_ca:
        raise IndependentVerificationError(
            "NIST signing leaf BasicConstraints is absent/not CA=false"
        )
    if (
        not certificate.key_usage_present
        or not certificate.digital_signature
        or certificate.key_cert_sign
    ):
        raise IndependentVerificationError(
            "NIST signing leaf KeyUsage does not permit only an end-entity signature"
        )
    if certificate.dns_names != ("engine.beacon.nist.gov",):
        raise IndependentVerificationError(
...
def load_independent_trust_bundle(
    ...
    *,
    ...
    if (
        type(allow_known_answer_fixture) is not bool
        or type(allow_candidate) is not bool
    ):
        raise IndependentVerificationError("trust loader policy flags are invalid")
    ...
    if fixture:
        ...
        if not allow_candidate:
            raise IndependentVerificationError(
                "candidate trust bundle is not freeze-ready"
            )
    elif manifest["status"] != TRUST_FROZEN_STATUS:
        raise IndependentVerificationError("trust manifest status differs")
    specifications = manifest["certificates"]
    _validate_independent_trust_policy(manifest["trustPolicy"], specifications)
    ...
    for certificate_id, specification in specifications.items():
        ...
        _verify_independent_leaf_policy(chain[0])
        if fixture:
...
def verify_nist_response(
    *,
    ...
    if pulse["statusCode"] != 0 and not (
        trust_bundle.fixture_only
        and allow_known_answer_fixture
        and pulse["statusCode"] == 1
    ):
        raise IndependentVerificationError(
            "NIST pulse statusCode is not zero/normal"
        )
    ...
    _verify_independent_leaf_policy(record.leaf)
...
__all__ = [
    ...
    "TRUST_ACCEPTED_LEAF_EXTENDED_KEY_USAGES", "TRUST_CANDIDATE_STATUS",
    "TRUST_FIXTURE_STATUS", "TRUST_FROZEN_STATUS", "TRUST_REVOCATION_POLICY",
    "TRUST_REVOCATION_RESIDUAL_RISK", "TRUST_ROTATION_POLICY", "TRUST_SCHEMA",
