...
replication. Blind-v1 remains `AUTHOR_SELF_VERIFICATION`, with the repository owner
...
For every blind-v1 development-control, design, snapshot, public execution
reservation, evidence, and closeout release, the expected commit and tree are
exactly the author-self-verified, zero-skip-CI implementation
...
`design`, `snapshot`, `reservation`, `evidence`, and `closeout` releases. Symlinks, extra
...
`blind_v1/trust/github/trusted_root.json` (`28,886` bytes, SHA-256
...
/usr/bin/sandbox-exec \
  ...
  blind_v1/tests/integration_release_attestation_crypto_known_answer.py \
...
uses `/usr/bin/unshare --net --`, the byte-pinned Linux x86-64 binary, and
`--network-isolation LINUX_UNSHARE_NETWORK_NAMESPACE`, exactly as in the
tracked workflow.
...
```sh
python3 -I -B blind_v1/package_design_release.py package \
...
`corelm-blind-crossmodel-v1-development-control`; its GitHub immutable-release
attestation must have `attestedAt` strictly before `2026-08-09T00:00:00Z`.
...
## Public execution-reservation receipt

After the snapshot release and its canonical receipt exist, generate the public
execution-reservation directory only with
`package_execution_reservation.py`. It contains exactly:

- `execution-reservation.json` — role `execution-reservation`;
- `snapshot-publication-receipt.json` — role
  `snapshot-publication-receipt`;
- `sha256-manifest.json` — role `sha256-manifest`.

The reservation fixes the sole public `attemptId`: its value is
`20260821T180000Z-` plus the first 16 lowercase hexadecimal characters of
SHA-256 over the canonical reservation object before `attemptId` and
`reservationContentSHA256` are added. The reservation packager reopens the
package, rederives that value, and returns it as `verification.attemptId`.
Do not derive the prefix from `reservedAt`, the GitHub release time, or a local
clock, and do not publish a package whose value was supplied manually.

Publish those exact files under the signed annotated tag
`corelm-blind-crossmodel-v1-execution-reservation`, targeting the exact frozen
`labSource.commit` and `labSource.tree`. Then collect its receipt:
...
python3 -I -B blind_v1/collect_release_receipt.py \
  ...
  --deadline '2026-08-21T17:45:00Z' \
  --signature-type SSH \
  --key-fingerprint 'SHA256:8A4y/GkoFglweSfg3rP21BtWWqIBOeQAUoAJDQM8sMM' \
  --public-key blind_v1/signing/corelm-blind-crossmodel-v1-signing.pub \
  --repo-path /absolute/path/to/clean-clone \
  --release-id '<numeric-release-id>' \
  --assets-dir /absolute/path/to/execution-reservation-assets \
  --github-cli /absolute/path/to/pinned-gh-2.97.0-macos-arm64 \
  --cosign /absolute/path/to/pinned-cosign-v3.0.6-macos-arm64 \
  --asset execution-reservation=execution-reservation.json \
  --asset snapshot-publication-receipt=snapshot-publication-receipt.json \
  --asset sha256-manifest=sha256-manifest.json \
  --signature-failure-output /absolute/path/to/reservation-signature-failure.json \
  --output /absolute/path/to/execution-reservation-publication-receipt.json
```

The generic receipt enforces the strict upper deadline. Composed reservation
verification must also derive `attestedAt` from the verified RFC3161 bytes and
require the full half-open interval
`[2026-08-20T18:00:00Z, 2026-08-21T17:45:00Z)`, while byte-binding the receipt
to the exact execution-reservation package and frozen design. The operator's
`reservedAt`, GitHub API `published_at`, HTTP `Date`, or a local clock cannot
satisfy that lower bound. This public release records execution intent and the
outcome-publication obligation; it is not local `attempt-reservation.json`, an
attempt marker, or a scientific result. The receipt byte-binds the package but
does not mint a new attempt identity: downstream local state, scientific
evidence, and any closeout must preserve the rederived reservation ID exactly.
...
The same collection performs exactly one pinned
`gh release verify TAG -R REPOSITORY --format json` invocation. Its online
verification must succeed and
...
RFC3161 `attestedAt`, and transcript. Normal receipts require
`attestedAt < deadline`; the explicit late-closeout path alone requires
`attestedAt >= deadline`.
...
Those identifiers belong only to v3. Blind-v1 has no development-control receipt or
release yet. It must execute and package a fresh development control on the
three excluded pilot revisions using the exact blind-v1 commit, publish the
blind-v1 tag and assets, and collect a new receipt. The
v3 incident is retained here solely as a regression requirement and cannot be
mechanically relabeled as blind-v1 evidence.
