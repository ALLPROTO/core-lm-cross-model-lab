...
# Build only the hash-locked Python runtime needed by blind-v1 controls.  This
...
fail() {
    printf 'BLIND_V1 RUNTIME BOOTSTRAP FAIL: %s\n' "$*" >&2
    exit 1
}
...
verify_codec_checkout() {
    ...
    [ "$(codec_git rev-parse --verify 'HEAD^{commit}')" = "$CODEC_COMMIT" ] \
        || fail 'codec commit differs from the registered blind_v1 binding'
    [ "$(codec_git rev-parse --verify 'HEAD^{tree}')" = "$CODEC_TREE" ] \
        || fail 'codec tree differs from the registered blind_v1 binding'
...
usage() {
    printf '%s\n' \
        'Usage: blind_v1/bootstrap_runtime.sh --platform linux|macos \' \
...
cleanup() {
    if [ -n "$STAGING" ]; then
        case "$STAGING" in
            "$RUNTIME_PARENT"/.corelm-blind-v1-runtime-stage.*)
                [ ! -e "$STAGING" ] || rm -rf -- "$STAGING"
                ;;
            *) printf 'BLIND_V1 RUNTIME CLEANUP REFUSED: %s\n' "$STAGING" >&2 ;;
        esac
    fi
    if [ -n "$PYCACHE_ROOT" ]; then
        case "$PYCACHE_ROOT" in
            "$RUNTIME_PARENT"/.corelm-blind-v1-pycache.*)
                [ ! -e "$PYCACHE_ROOT" ] || rm -rf -- "$PYCACHE_ROOT"
                ;;
            *) printf 'BLIND_V1 PYCACHE CLEANUP REFUSED: %s\n' "$PYCACHE_ROOT" >&2 ;;
        esac
    fi
}
on_signal() {
...
STAGING=$(mktemp -d "$RUNTIME_PARENT/.corelm-blind-v1-runtime-stage.XXXXXX") \
    || fail 'could not create runtime staging directory'
PYCACHE_ROOT=$(mktemp -d "$RUNTIME_PARENT/.corelm-blind-v1-pycache.XXXXXX") \
...
printf '%s\n' \
    'BLIND_V1 RUNTIME BOOTSTRAP PASS' \
    "Platform: $PLATFORM" \
    "Runtime: $RUNTIME" \
