...
identity: its signed annotated tag must target the exact author-verified and
CI-successful implementation commit/tree recorded in ``labSource``.  The frozen
...
from blind_v1.protocol import canonical_json_bytes, load_json_strict_bytes
...
SIGNER_IDENTITY = "corelm-blind-crossmodel-v1-release"
...
def verify_ssh_signing_key(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="corelm-blind-v1-key-check-") as temporary:
...
def verify_archived_ssh_tag_signature(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="corelm-blind-v1-ssh-verify-") as temporary:
        root = Path(temporary)
