#!/usr/bin/env python3
"""Create and verify the non-self-referential author-verified blind_v1 freeze manifest.
...
from blind_v1.mediawiki_snapshot import PROJECTS, PinnedHTTPSClient  # noqa: E402
from blind_v1.development_artifact_verifier import (  # noqa: E402
...
from blind_v1.github_gate_receipt import (  # noqa: E402
...
from blind_v1.nist_beacon import (  # noqa: E402
    ...
    TRUST_SCHEMA as NIST_TRUST_SCHEMA,
...
SCHEMA_VERSION = "corelm-blind-crossmodel-v1-freeze-manifest-v1"
STATUS = "IMPLEMENTATION_FREEZE_READY_FOR_DESIGN_BINDING"
SUITE_ID = "corelm-blind-crossmodel-v1"
RUNTIME_SCHEMA = "corelm-blind-crossmodel-v1-runtime-manifest-v1"
ASSET_RECEIPT_SCHEMA = "corelm-blind-crossmodel-v1-asset-receipt-v1"
TRUST_SCHEMA = NIST_TRUST_SCHEMA
TRACKED_NIST_CANDIDATE_MANIFEST = (
    BLIND_V1_ROOT / "trust" / "nist" / "manifest.json"
)
FROZEN_NIST_TRUST_MANIFEST_BYTES = 1_930
PULSE_TIME = datetime(2026, 8, 21, 18, 0, 0, tzinfo=timezone.utc)
DECISION_CHECKPOINT = datetime(2026, 8, 8, 12, 0, 0, tzinfo=timezone.utc)
DESIGN_PUBLISH_DEADLINE = datetime(2026, 8, 9, 0, 0, 0, tzinfo=timezone.utc)
...
DEVELOPMENT_MODEL_KEYS = (
    "gpt-neo-125m",
    "smollm2-360m",
    "tiny-starcoder-py",
)
CONFIRMATORY_MODEL_KEYS = (
    "pythia-160m",
    "pythia-70m",
    "smollm-135m",
    "smollm-360m",
...
EXPECTED_ASSET_FILES = 18
EXPECTED_ASSET_BYTES = 3_438_516_676
EXPECTED_WEIGHT_BYTES = 3_427_365_620
...
DEVELOPMENT_ARCHIVE_MANIFEST_SCHEMA = (
    "corelm-blind-crossmodel-v1-real-e2e-development-archive-manifest-v1"
)
DEVELOPMENT_ARCHIVE_TAG = (
    "corelm-blind-crossmodel-v1-development-control"
)
...
DEVELOPMENT_PLAN_SCHEMA = "corelm-blind-crossmodel-v1-real-e2e-development-plan-v1"
DEVELOPMENT_REPLAY_SCHEMA = (
    "corelm-blind-crossmodel-v1-real-e2e-development-model-replay-v1"
)
...
DEVELOPMENT_ARCHIVED_INPUTS = {
    "designRegistration": (
        ...
        "blind_v1/design-registration.draft.json",
    ),
    "modelAssetManifest": (
        "inputs/development-model-assets.json",
        "blind_v1/development-model-assets.json",
    ),
    "fullAssetReceipt": (
    ...
    "developmentCorpusManifest": (
        ...
        "blind_v1/development-corpus.draft.json",
    ),
...
def _expected_development_artifact_paths() -> set[str]:
    paths = {
        ...
        *(f"jobs/{model}.json" for model in DEVELOPMENT_MODEL_KEYS),
        *(f"logs/{model}.log" for model in DEVELOPMENT_MODEL_KEYS),
        "logs/independent-development-replay.log",
        *(f"supervision/{model}.json" for model in DEVELOPMENT_MODEL_KEYS),
        "supervision/independent-development-replay.json",
    }
    for model in DEVELOPMENT_MODEL_KEYS:
        paths.update(
            {
...
def _validate_development_replay(value: Any, *, run_id: str, config_sha256: str) -> None:
    ...
    if (
        ...
        or replay["modelOrder"] != list(DEVELOPMENT_MODEL_KEYS)
    ...
    for model in models:
        ...
        if not isinstance(model, dict) or set(model) != model_fields or key not in DEVELOPMENT_MODEL_KEYS:
    ...
    if (
        replay["totalReplayedPages"] != len(DEVELOPMENT_MODEL_KEYS) * DEVELOPMENT_PAGES_PER_MODEL
        or replay["totalReplayedPredictions"]
        != len(DEVELOPMENT_MODEL_KEYS)
...
def _validate_development_supervision(
    ...
    expected_subjects = [
        *(f"producer:{model}" for model in DEVELOPMENT_MODEL_KEYS),
...
def validate_development_control_report(
    ...
    expected_phases = [
        ...
        *(f"before-producer:{model}" for model in DEVELOPMENT_MODEL_KEYS),
...
def _validate_development_rights_evidence(
    ...
    try:
        ...
        tracked_manifest_raw = read_regular_bytes(
            BLIND_V1_ROOT / "development-corpus.draft.json",
...
def _validate_development_plan(
    ...
    *,
    ...
    if (
        ...
        != report["controlConfigurationSHA256"]
        or plan["modelExecutionOrder"] != list(DEVELOPMENT_MODEL_KEYS)
    ...
    expected_private = {
        *(f"models/{model}/{name}" for model in DEVELOPMENT_MODEL_KEYS for name in DEVELOPMENT_MODEL_FILES),
        *(
            f"{DEVELOPMENT_RECORD_ROOT}/slice-{index:02d}.bin"
    ...
    jobs = plan["jobs"]
    if not isinstance(jobs, dict) or tuple(jobs) != DEVELOPMENT_MODEL_KEYS:
        raise FreezeManifestError("development job commitments differ")
    for key in DEVELOPMENT_MODEL_KEYS:
    ...
    configuration = {
        ...
        "modelExecutionOrder": list(DEVELOPMENT_MODEL_KEYS),
...
def verify_development_control_report(
    ...
    *,
    ...
    if (
        ...
        != "corelm-blind-crossmodel-v1-real-e2e-development-start-v1"
    ...
    for index, subject in enumerate(
        [*(f"producer:{model}" for model in DEVELOPMENT_MODEL_KEYS), "independent-real-model-replay"]
    ):
        filename = (
            f"{DEVELOPMENT_MODEL_KEYS[index]}.json"
            if index < len(DEVELOPMENT_MODEL_KEYS)
    ...
    *,
    ...
    try:
        verified = verify_release_receipt(
            ...
            expected_deadline="2026-08-09T00:00:00Z",
...
def _verify_asset_receipt(assets: Mapping[str, Any]) -> None:
    ...
    if not isinstance(models, dict) or set(models) != set(CONFIRMATORY_MODEL_KEYS):
        raise FreezeManifestError("asset receipt model set differs")
    total_files = 0
    total_bytes = 0
    total_weight_bytes = 0
    for model_key in CONFIRMATORY_MODEL_KEYS:
        ...
        if (
            ...
            or set(files) != {"config.json", "model.safetensors", "tokenizer.json"}
        ):
...
def verify_frozen_nist_trust_bundle(
    path: Path, expected_sha256: str
) -> dict[str, Any]:
    """Non-injectable cryptographic discharge for the NIST trust freeze blocker."""

    if expected_sha256 != NIST_FROZEN_TRUST_BUNDLE_SHA256:
        raise FreezeManifestError(
            "frozen NIST trust manifest is not the preregistered status-only artifact"
        )
    try:
        ...
        raise FreezeManifestError(
            "frozen NIST trust bundle failed mandatory offline verification"
        ) from error
    if (
        bundle.fixture_only
        or independent.fixture_only
        or len(bundle.records) != 1
        or len(independent.records) != 1
        or tuple(bundle.records) != tuple(independent.records)
        or bundle.manifest_sha256 != independent.manifest_sha256
    ):
        raise FreezeManifestError(
            "frozen NIST trust producer/independent outcomes disagree"
        )
    certificate_id, record = next(iter(bundle.records.items()))
    independent_record = independent.records[certificate_id]
    leaf = record.leaf
    independent_leaf = independent_record.leaf
    if (
        not record.chain_verified
        or not independent_record.chain_verified
        or not leaf.basic_constraints_present
        or not independent_leaf.basic_constraints_present
        or leaf.is_ca
        or independent_leaf.is_ca
        or not leaf.key_usage_present
        or not independent_leaf.key_usage_present
        or not leaf.digital_signature
        or not independent_leaf.digital_signature
        or leaf.key_cert_sign
        or independent_leaf.key_cert_sign
        or leaf.extended_key_usages
        != TRUST_ACCEPTED_LEAF_EXTENDED_KEY_USAGES
        or independent_leaf.extended_key_usages
        != TRUST_ACCEPTED_LEAF_EXTENDED_KEY_USAGES
        or leaf.dns_names != ("engine.beacon.nist.gov",)
        or independent_leaf.dns_names != ("engine.beacon.nist.gov",)
    ):
        raise FreezeManifestError(
            "frozen NIST trust producer/independent leaf discharge differs"
        )
    raw = read_regular_bytes(path, maximum_bytes=MAX_TRUST_MANIFEST_BYTES)
    if (
        len(raw) != FROZEN_NIST_TRUST_MANIFEST_BYTES
        or sha256_bytes(raw) != NIST_FROZEN_TRUST_BUNDLE_SHA256
    ):
        raise FreezeManifestError(
            "frozen NIST trust bytes differ from the preregistered promotion"
        )
    value = load_json_strict_bytes(raw, label="frozen NIST trust manifest")
    policy = value.get("trustPolicy") if isinstance(value, dict) else None
    if (
        not isinstance(policy, dict)
        or policy.get("allowedCertificateIds") != [certificate_id]
        or policy.get("rotationPolicy") != TRUST_ROTATION_POLICY
        or policy.get("revocationPolicy") != TRUST_REVOCATION_POLICY
        or policy.get("revocationChecked") is not False
        or policy.get("revocationResidualRisk")
        != TRUST_REVOCATION_RESIDUAL_RISK
        or policy.get("acceptedLeafExtendedKeyUsages")
        != list(TRUST_ACCEPTED_LEAF_EXTENDED_KEY_USAGES)
    ):
        raise FreezeManifestError("frozen NIST trust policy discharge differs")
    candidate_raw = read_regular_bytes(
        TRACKED_NIST_CANDIDATE_MANIFEST,
        maximum_bytes=MAX_TRUST_MANIFEST_BYTES,
    )
    if sha256_bytes(candidate_raw) != NIST_CANDIDATE_TRUST_BUNDLE_SHA256:
        raise FreezeManifestError("tracked NIST trust candidate commitment differs")
    normalized_candidate = dict(value)
    normalized_candidate["status"] = "CANDIDATE_OFFLINE_TRUST_BUNDLE"
    if canonical_json_bytes(normalized_candidate) != candidate_raw:
        raise FreezeManifestError(
            "frozen NIST trust is not an exact status-only candidate promotion"
        )
    return {
        "status": "VERIFIED_FROZEN_NIST_TRUST_DISCHARGE",
        "schemaVersion": TRUST_SCHEMA,
        "manifestStatus": TRUST_FROZEN_STATUS,
        "manifestSHA256": expected_sha256,
        "candidateManifestSHA256": NIST_CANDIDATE_TRUST_BUNDLE_SHA256,
        "manifestBytes": len(raw),
        "statusOnlyPromotionVerified": True,
        "producerVerified": True,
        "independentVerified": True,
        "targetTimestamp": TARGET_TIMESTAMP,
        "certificateId": certificate_id,
        "allowedCertificateIds": [certificate_id],
        "rotationPolicy": TRUST_ROTATION_POLICY,
        "revocationPolicy": TRUST_REVOCATION_POLICY,
        "revocationChecked": False,
        "revocationResidualRisk": TRUST_REVOCATION_RESIDUAL_RISK,
        "acceptedLeafExtendedKeyUsages": list(
            TRUST_ACCEPTED_LEAF_EXTENDED_KEY_USAGES
        ),
        "certificateChainVerified": True,
        "leafEndEntityKeyUsageVerified": True,
        "leafExtendedKeyUsageVerified": True,
        "leafHostnameVerified": True,
        "targetValidityVerified": True,
    }


def default_trust_verifier(path: Path, expected_sha256: str) -> None:
    verify_frozen_nist_trust_bundle(path, expected_sha256)


def _verify_tracked_model_card_evidence() -> dict[str, Any]:
    """Bind rights provenance to the exact tracked draft and its six models."""
    ...
    draft_path = BLIND_V1_ROOT / "design-registration.draft.json"
    ...
    if not isinstance(draft, dict) or not isinstance(draft.get("models"), list):
        raise FreezeManifestError("tracked design draft model pool is absent")
    try:
        summary = verify_model_card_evidence_tree(PROJECT_ROOT, draft["models"])
        validate_model_card_design_binding(draft.get("modelCardEvidence"), summary)
    except ModelCardEvidenceError as error:
        raise FreezeManifestError("tracked model-card evidence differs") from error
    return summary
...
def verify_artifact_inputs(
    ...
    *,
    ...
    model_card_summary = _verify_tracked_model_card_evidence()
    ...
    if (
        model_card_summary["manifestSHA256"]
        != commitments["modelCardEvidenceSHA256"]
    ):
        raise FreezeManifestError("model-card evidence file SHA-256 differs")
    ...
    if (
        ...
        or trust_value.get("status") != TRUST_FROZEN_STATUS
        or trust_value.get("fixtureOnly") is not False
    ):
        raise FreezeManifestError("offline trust manifest is not normative")
    # This mandatory verifier cannot be replaced by an injected test or plugin
    # callback. The callback below is additive only.
    trust_discharge = verify_frozen_nist_trust_bundle(
        trust_manifest_path, trust_sha256
    )
    ...
    return {
        ...
        "modelCardEvidenceSHA256": model_card_summary["manifestSHA256"],
        "transportCABundleSHA256": ca_sha256,
        "offlineTrustBundleSHA256": trust_sha256,
        "nistTrustDischarge": trust_discharge,
...
def verify_design_binding(
    ...
    if (
        design.get("schemaVersion") != "corelm-blind-crossmodel-v1-design-v1"
    ...
    model_card_binding = design.get("modelCardEvidence")
    if (
        not isinstance(model_card_binding, dict)
        or model_card_binding.get("sha256")
        != artifacts["modelCardEvidenceSHA256"]
    ):
        raise FreezeManifestError("frozen design model-card evidence binding differs")
