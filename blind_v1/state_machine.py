#!/usr/bin/env python3
"""Durable, fail-closed state transitions for the blind-v1 one-shot.
...
def _macos_extended_acl_present(descriptor: int) -> bool:
    """Inspect the exact open directory for a macOS extended ACL.

    Mode bits do not account for macOS extended ACL entries.  Use the
    descriptor API so this check cannot be redirected to a different path
    between the no-follow open and ACL inspection.  Absence of an extended
    ACL is reported by macOS as ``ENOENT``; every other inspection failure is
    rejected rather than treated as ACL-free.
    """
    ...
    try:
        ...
        raise StateMachineError(
            "result root macOS extended ACL could not be inspected"
        ) from error
    ...
    try:
        ...
        # The result is rejected regardless; release the libc allocation
        # without weakening the fail-closed decision if acl_free itself fails.
        acl_free(acl)
    ...
    try:
        ...
        for index, component in enumerate(components):
            ...
            if final and (
                ...
                os.close(child)
                raise StateMachineError(
                    "result root must be owned by the current user with mode 0700"
                )
            if final:
                try:
                    extended_acl_present = _macos_extended_acl_present(child)
                except BaseException:
                    os.close(child)
                    raise
                if extended_acl_present:
                    os.close(child)
                    raise StateMachineError(
                        "result root must not have a macOS extended ACL"
                    )
            if (
                not final
                and mode & 0o022
                and not observed.st_mode & stat.S_ISVTX
            ):
                os.close(child)
                raise StateMachineError(
                    "result-root path contains an unprotected writable directory"
                )
            os.close(descriptor)
            descriptor = child
    except BaseException:
...
def _historical_create_attempt_marker(
    ...
    *,
    ...
    if not ONE_SHOT_NOT_BEFORE <= created < MARKER_NO_LATER_THAN:
        raise StateMachineError("attempt marker is outside the frozen marker window")
    commitments = {
        "suiteId": suite_id,
    ...
    reservation = {
        "schemaVersion": "corelm-blind-crossmodel-v1-attempt-reservation-v1",
        "status": "RESERVED",
        **commitments,
...
def _validate_reservation(
    ...
    if not ONE_SHOT_NOT_BEFORE <= created < MARKER_NO_LATER_THAN:
        raise StateMachineError("attempt reservation is outside the frozen marker window")
    for field in (
        "designSHA256",
        "snapshotRegistrationSHA256",
        "designPublicationReceiptSHA256",
...
def _validate_marker(marker: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        marker.get("schemaVersion") != "corelm-blind-crossmodel-v1-attempt-v1"
...
def _validate_outcome(
    ...
    if outcome.get("schemaVersion") != "corelm-blind-crossmodel-v1-outcome-v1":
