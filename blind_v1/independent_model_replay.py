#!/usr/bin/env python3
"""Independent real-model replay for the blind multi-model blind_v1 evidence.
...
SUMMARY_SCHEMA = "corelm-blind-crossmodel-v1-independent-model-replay-v1"
RAW_TOKEN_SCHEMA = "corelm-blind-crossmodel-v1-raw-token-evidence-v1"
PAGE_TOKEN_SCHEMA = "corelm-blind-crossmodel-v1-page-token-evidence-v1"
CONTAINER_SCHEMA = "corelm-blind-crossmodel-v1-container-evidence-v1"
WORKER_JOB_SCHEMA = "corelm-blind-crossmodel-v1-worker-job-v1"
RECORD_MAGIC = b"CORELM-BLIND-CROSSMODEL-V1-RECORD\0"
PREFILL_TOKENS = 383
PREDICTION_TOKENS = 128
PAGE_TOKENS = 512
PAGES_PER_CORPUS = 16
# Duplicated deliberately instead of importing the producer collector.  A
# selected eligible canonical record is bounded independently at exactly 8 MiB.
MAX_SELECTED_CANONICAL_RECORD_BYTES = 8_388_608
...
ARCHITECTURE_MODEL_TYPES = {
    "gpt-neox-mha": "gpt_neox",
    "llama-gqa": "llama",
    "gpt2-mha": "gpt2",
}
RAW_FIELDS = {
...
_REPLAY_GPT2_BUFFER = re.compile(
    r"transformer\.h\.[0-9]+\.attn\.(?:bias|masked_bias)\Z"
)
_REPLAY_GPT_NEO_BUFFER = re.compile(
    r"transformer\.h\.[0-9]+\.attn(?:\.attention)?\.(?:bias|masked_bias)\Z"
)
_REPLAY_GPT_NEOX_BUFFER = re.compile(
...
def _prepare_exact_state_for_replay(
    source: dict[str, Any],
    *,
    model_type: str,
    tied_word_embeddings: bool,
) -> dict[str, Any]:
    """Reconcile only the six frozen upstream safetensors layouts.

    This implementation intentionally lives in the independent replay and
    does not call the producer worker.  Its accepted rewrites are closed:
    legacy GPT-2 base-model keys gain the ``transformer.`` namespace,
    Pythia's untied ``embed_out`` becomes ``lm_head``, and only the exact
    non-persistent attention/rotary buffers emitted by the pinned upstream
    exporters are discarded.  Ambiguous rewrites fail here, and the
    immediately following strict replay load rejects unknown keys or shapes.
    """

    if not isinstance(source, dict) or not source:
        raise IndependentModelReplayError("replay weight dictionary is empty")
    supported = {"gpt_neo", "gpt_neox", "gpt2", "llama", "gpt_bigcode"}
    if model_type not in supported:
        raise IndependentModelReplayError(
            f"replay weight model_type is unsupported: {model_type!r}"
        )
    if type(tied_word_embeddings) is not bool:
        raise IndependentModelReplayError(
            "replay tied_word_embeddings flag is not boolean"
        )
    keys = tuple(source.keys())
    if any(not isinstance(key, str) or not key or "\x00" in key for key in keys):
        raise IndependentModelReplayError("replay weights contain an invalid key")

    bare_gpt2 = False
    if model_type == "gpt2":
        has_base = any(
            key.startswith(("h.", "wte.", "wpe.", "ln_f.")) for key in keys
        )
        has_lm_namespace = any(key.startswith("transformer.") for key in keys)
        if has_base and has_lm_namespace:
            raise IndependentModelReplayError(
                "replay GPT-2 weights mix base and causal-LM namespaces"
            )
        if has_base:
            invalid = [
                key
            ...
            if invalid:
                raise IndependentModelReplayError(
                    "replay GPT-2 base namespace contains an unknown key"
                )
            bare_gpt2 = True
        elif not has_lm_namespace:
            raise IndependentModelReplayError(
                "replay GPT-2 weights have no recognized namespace"
            )

    result: dict[str, Any] = {}
    for original, value in source.items():
        if model_type == "gpt2" and bare_gpt2 and original != "lm_head.weight":
            destination = "transformer." + original
        elif model_type == "gpt_neox" and original == "embed_out.weight":
            destination = "lm_head.weight"
        else:
            destination = original

        discard = False
        if model_type == "gpt2":
            discard = _REPLAY_GPT2_BUFFER.fullmatch(destination) is not None
        elif model_type in {"gpt_neo", "gpt_bigcode"}:
            discard = _REPLAY_GPT_NEO_BUFFER.fullmatch(destination) is not None
        elif model_type == "gpt_neox":
            discard = _REPLAY_GPT_NEOX_BUFFER.fullmatch(destination) is not None
        if discard:
            continue
        if destination in result:
            raise IndependentModelReplayError(
                f"replay weight normalization collides at {destination}"
            )
    ...
    input_name = input_names[model_type]
    if input_name not in result:
        raise IndependentModelReplayError(
            "replay input-embedding weight is missing after normalization"
        )
    if tied_word_embeddings:
        existing_head = result.get("lm_head.weight")
        if existing_head is not None and existing_head is not result[input_name]:
            raise IndependentModelReplayError(
                "replay tied model provides a distinct output-head weight"
            )
        result["lm_head.weight"] = result[input_name]
    elif "lm_head.weight" not in result:
        raise IndependentModelReplayError(
            "replay untied model has no output-head weight"
        )
    return result


def _decode_replay_state_and_discard_input(
    model_bytes: dict[str, bytes], decoder: Callable[[bytes], Any]
) -> dict[str, Any]:
    """Own and discard replay weight bytes before constructing FP32 storage."""

    if not isinstance(model_bytes, dict) or "model.safetensors" not in model_bytes:
        raise IndependentModelReplayError("replay safetensors bytes are absent")
    weight_bytes = model_bytes.pop("model.safetensors")
    if type(weight_bytes) is not bytes or not weight_bytes:
        raise IndependentModelReplayError("replay safetensors bytes are invalid")
    try:
    ...
    if not isinstance(state, dict) or not state:
        raise IndependentModelReplayError("replay decoded state dictionary is absent")
    return state
...
class RealModelReplayBackend:
    ...
    def __init__(
        ...
        *,
        ...
        if model_type != expected_model_type or model_type not in classes:
            raise IndependentModelReplayError(
                f"replay model_type differs from the frozen design: {model_type!r}"
            )
        ...
        try:
            state = _decode_replay_state_and_discard_input(
        ...
        if type(tied_word_embeddings) is not bool:
            raise IndependentModelReplayError(
                "replay model tie_word_embeddings is not an explicit boolean"
            )
        exact_state = _prepare_exact_state_for_replay(
            state,
        ...
        if "model.safetensors" in model_bytes:
            raise IndependentModelReplayError(
                "replay safetensors bytes survived before model construction"
            )
        model = model_class(config).float().cpu()
        try:
            model.load_state_dict(exact_state, strict=True, assign=False)
        except RuntimeError as error:
            raise IndependentModelReplayError(
                "replay model state differs after exact normalization"
            ) from error
        if tied_word_embeddings:
            model.tie_weights()
        ...
        if output is None or inputs is None:
            raise IndependentModelReplayError(
                "replay model input/output embeddings are absent"
            )
        pointers_equal = output.weight.data_ptr() == inputs.weight.data_ptr()
        if pointers_equal is not tied_word_embeddings:
            raise IndependentModelReplayError(
                "replay model output-head tying differs from configuration"
            )
        del exact_state
        del state
        model.eval()
        ...
        if (
            observed_vocab != expected_model_vocab_size
            or tokenizer_vocab != expected_tokenizer_vocab_size
        ):
            raise IndependentModelReplayError(
        ...
        self.vocab_size = expected_tokenizer_vocab_size
        self.model_vocab_size = expected_model_vocab_size
    ...
    def _continuation_logits(self, continuation_ids: list[int], layers: list[Any]) -> Any:
        ...
        if (
            ...
            or int(logits.shape[-1]) != self.model_vocab_size
    ...
    def evaluate(
        ...
        baseline_losses = functional.cross_entropy(
            baseline_logits.reshape(-1, self.model_vocab_size),
            target_tensor,
            reduction="none",
        ).float()
        candidate_losses = functional.cross_entropy(
            candidate_logits.reshape(-1, self.model_vocab_size),
            target_tensor,
            reduction="none",
        ).float()
    ...
    *,
    ...
    for corpus in corpora:
        ...
        for page_index, item in enumerate(selected):
            ...
            if not 1 <= entry["bytes"] <= MAX_SELECTED_CANONICAL_RECORD_BYTES:
                raise IndependentModelReplayError(
                    f"sealed selected corpus record exceeds independent byte bound: {path}"
                )
            values.append(
                {
                    "pageSelectionIndex": page_index,
...
def _consume_owned_selected_record(
    record_raw: bytes,
    consumer: Callable[[bytes], Any],
) -> Any:
    """Consume one selected record and release it before the next read."""

    try:
        return consumer(record_raw)
    finally:
        del record_raw
        gc.collect()
...
def run_independent_model_replay(
    *,
    ...
    } if isinstance(design_models, list) else set()
    if (
        ...
        or len(design_model_keys) != 6
        or not set(model_order).issubset(design_model_keys)
    ):
    ...
    for model_key in model_order:
        ...
        try:
            model_specification = next(
                item for item in design_models if item["key"] == model_key
            )
            expected_tokenizer_vocab_size = model_specification.get(
                "tokenizerVocabSize"
            )
            expected_model_type = ARCHITECTURE_MODEL_TYPES.get(
                model_specification.get("architecture")
            )
            if (
                type(expected_tokenizer_vocab_size) is not int
                or not 1 <= expected_tokenizer_vocab_size <= 2**32
                or expected_model_type is None
            ):
                raise IndependentModelReplayError(
                    f"frozen model architecture/vocabulary is invalid: {model_key}"
                )
            backend = RealModelReplayBackend(
                model_bytes,
                expected_model_type=expected_model_type,
                expected_model_vocab_size=job["model"]["vocabSize"],
                expected_tokenizer_vocab_size=expected_tokenizer_vocab_size,
                expected_layers=expected_layers,
            )
            if "model.safetensors" in model_bytes:
                raise IndependentModelReplayError(
                    "replay backend retained the safetensors byte buffer"
                )
        finally:
            model_bytes.clear()
            gc.collect()
            ...
            for corpus in selected_corpora:
                for page in job["pages"][corpus]:
                    ...
                    def consume_record(
                        ...
                        return page_summary, commitment

                    page_summary, record_commitment = _consume_owned_selected_record(
                        read_beneath(
                            private_root,
                            page["recordPath"],
                            maximum_bytes=MAX_SELECTED_CANONICAL_RECORD_BYTES,
