#!/usr/bin/env python3
"""Recompute the complete blind-v1 verdict from frozen raw evidence."""
...
from blind_v1.create_sbom import build_sbom  # noqa: E402
from blind_v1.git_source import GitSourceError, verify_copied_source  # noqa: E402
...
from blind_v1.independent_model_replay import (  # noqa: E402
...
def validate_frozen_design(design: Any) -> None:
    ...
    if design.get("suiteId") != "corelm-blind-crossmodel-v1":
    ...
    if (
        ...
        or beacon.get("pulseStatusCode") != 0
...
def validate_model_replay_summary(
    ...
    *,
    ...
    selected_model_keys = selection["modelExecutionOrder"]
    expected_containers = 32 * sum(
        design_models[key]["layers"] for key in selected_model_keys
    )
    if (
...
def verify_private_snapshot_manifest(
    ...
    reservation_receipts = [
        entry
        for entry in files
        if entry["role"] == "reservation-publication-receipt"
    ]
    if (
        len(reservation_receipts) != 1
        or reservation_receipts[0]["path"]
        != "publication/reservation-receipt.json"
        or reservation_receipts[0]["sha256"]
        != value["reservationPublicationReceiptSHA256"]
    ):
        raise EvidenceError("private execution-reservation receipt binding differs")
    reservation_asset_paths = {
        entry["path"]
        for entry in files
        if entry["role"] == "reservation-release-asset"
    }
    if reservation_asset_paths != {
        ...
        raise EvidenceError("private execution-reservation asset set differs")
    return value
...
def verify_runtime_asset_sbom_bindings(
    ...
    *,
    ...
    if (
        ...
        != "corelm-blind-crossmodel-v1-runtime-manifest-v1"
    ...
    if (
        ...
        != "corelm-blind-crossmodel-v1-asset-receipt-v1"
        ...
        or asset_receipt.get("fileCount") != 18
        or asset_receipt.get("totalBytes") != 3_438_516_676
        or asset_receipt.get("fullSafetensorsBytes") != 3_427_365_620
        or any(
...
def validate_selection(selection: Any, design: dict[str, Any], snapshot_digest: str):
    ...
    if selection.get("schemaVersion") != "corelm-blind-crossmodel-v1-selection-v1":
    ...
    *,
    ...
    for model_key in model_order:
        ...
        if (
            ...
            != "corelm-blind-crossmodel-v1-supervisor-receipt-v1"
        ...
        architecture_geometry = {
            ...
            "gpt-neox-mha": ("gpt_neox", "multi-head"),
            "gpt2-mha": ("gpt2", "multi-head"),
...
def verify_execution_reservation_publication_provenance(
    publication: Any,
    design: dict[str, Any],
    reservation_verification: dict[str, Any],
) -> dict[str, Any]:
    """Bind the public reservation package to its signed pre-pulse release."""

    release = design.get("reservationRelease")
    beacon = design.get("beacon")
    receipt = getattr(publication, "receipt", None)
    if not isinstance(release, dict) or not isinstance(beacon, dict) or receipt is None:
        raise EvidenceError("execution-reservation publication binding is incomplete")
    if (
        reservation_verification.get("status")
        != "VERIFIED_EXECUTION_RESERVATION_RELEASE_ASSETS"
        or reservation_verification.get("networkUsed") is not False
        or reservation_verification.get("modelInferenceUsed") is not False
        or reservation_verification.get("selectionDerived") is not False
    ):
        raise EvidenceError("execution-reservation package verification is incomplete")
    for field in (
        "reservationFileSHA256",
        "snapshotReceiptFileSHA256",
        "manifestFileSHA256",
    ):
        value = reservation_verification.get(field)
        if not isinstance(value, str) or re.fullmatch(r"[0-9a-f]{64}", value) is None:
            raise EvidenceError(
                f"execution-reservation package digest is invalid: {field}"
            )

    def parse_utc(value: Any, *, label: str, milliseconds: bool = False) -> datetime:
        pattern = "%Y-%m-%dT%H:%M:%S.%fZ" if milliseconds else "%Y-%m-%dT%H:%M:%SZ"
        try:
            parsed = datetime.strptime(value, pattern).replace(tzinfo=timezone.utc)
        except (TypeError, ValueError) as error:
            raise EvidenceError(
                f"execution-reservation {label} is not canonical UTC"
            ) from error
        return parsed

    not_before = parse_utc(
        release.get("publishNotBefore"), label="publishNotBefore"
    )
    deadline = parse_utc(
        release.get("publishNoLaterThan"), label="publishNoLaterThan"
    )
    target_pulse = parse_utc(
        beacon.get("targetTimestamp"),
        label="target pulse timestamp",
        milliseconds=True,
    )
    reserved = parse_utc(
        reservation_verification.get("reservedAt"), label="reservedAt"
    )
    published = parse_utc(receipt.published_at, label="publishedAt")
    attested = parse_utc(receipt.attested_at, label="attestedAt")
    if not (
        not_before < deadline <= target_pulse
        and not_before <= reserved <= published <= attested < deadline
    ):
        raise EvidenceError(
            "execution-reservation publication is outside its pre-pulse half-open window"
        )
    if receipt.kind != "reservation" or receipt.tag != release.get("tag"):
        raise EvidenceError("execution-reservation signed release identity differs")
    if receipt.receipt_sha256 != publication.receipt_sha256:
        raise EvidenceError("execution-reservation receipt digest replay differs")
    role_sha256 = dict(publication.role_sha256)
    if set(role_sha256) != {
        "execution-reservation",
        "snapshot-publication-receipt",
        "sha256-manifest",
    }:
        raise EvidenceError("execution-reservation signed asset roles differ")
    return {
        "status": "VERIFIED_PUBLIC_EXECUTION_RESERVATION_PROVENANCE",
        "releaseKind": "reservation",
        "releaseTag": receipt.tag,
        "receiptSHA256": publication.receipt_sha256,
        "sourceCommit": publication.source_commit,
        "sourceTree": publication.source_tree,
        "reservedAt": reservation_verification["reservedAt"],
        "publishedAt": receipt.published_at,
        "attestedAt": receipt.attested_at,
        "reservationFileSHA256": reservation_verification[
            "reservationFileSHA256"
        ],
        "snapshotReceiptFileSHA256": reservation_verification[
            "snapshotReceiptFileSHA256"
        ],
        "manifestFileSHA256": reservation_verification["manifestFileSHA256"],
        "roleSHA256": role_sha256,
        "prePulseHalfOpenWindowVerified": True,
...
def verify_publication_inputs(
    ...
    *,
    ...
    """Reverify design, snapshot, and reservation signed public releases."""

    key_path = private_root / "publication" / "signing-key.pub"
    design_raw = read_external_file(design_path, maximum_bytes=16 * 1024 * 1024)
    snapshot_raw = read_external_file(
        snapshot_path, maximum_bytes=16 * 1024 * 1024
    )
    reservation_receipt_raw = _private_committed_file(
        private_root,
        private_manifest,
        "publication/reservation-receipt.json",
        role="reservation-publication-receipt",
        maximum_bytes=64 * 1024 * 1024,
    )
    if (
        hashlib.sha256(reservation_receipt_raw).hexdigest()
        != private_manifest["reservationPublicationReceiptSHA256"]
    ):
        raise EvidenceError(
            "private execution-reservation receipt digest binding differs"
        )
    for relative in (
        "publication/reservation-assets/execution-reservation.json",
        "publication/reservation-assets/snapshot-publication-receipt.json",
        ...
        _private_committed_file(
            private_root,
            private_manifest,
            relative,
            role="reservation-release-asset",
            maximum_bytes=64 * 1024 * 1024,
        )
    try:
        ...
        reservation_publication = verify_publication(
            private_root / "publication" / "reservation-receipt.json",
            reservation_root,
            kind="reservation",
            tag=reservation_release["tag"],
            deadline=reservation_release["publishNoLaterThan"],
            signing_public_key_path=key_path,
            signing_key_fingerprint=reservation_release[
                "signingKeyFingerprint"
            ],
            signing_public_key_sha256=reservation_release[
                "signingPublicKeySHA256"
            ],
            expected_role_paths={
                "execution-reservation": reservation_root
                / "execution-reservation.json",
                "snapshot-publication-receipt": reservation_root
                / "snapshot-publication-receipt.json",
                "sha256-manifest": reservation_root / "sha256-manifest.json",
            },
            cryptographic_attestation_verifier=(
                cryptographic_attestation_verifier
            ),
    ...
    except (DesignReleaseError, ExecutionReservationError, PublicationError) as error:
    ...
    if (
        ...
        or reservation_verification.get("attemptId") != marker["attemptId"]
    ):
        raise EvidenceError(
            "attempt marker publication-receipt/attempt identity binding differs"
        )
    if (
        ...
        raise EvidenceError(
            "private snapshot execution-reservation receipt digest differs"
        )
    if (
        reservation_verification["snapshotReceiptFileSHA256"]
        != snapshot_publication.receipt_sha256
    ):
        raise EvidenceError(
            "execution-reservation package embeds another snapshot receipt"
        )
...
def verify_attempt_reservation(
    ...
    marker_upper = parse_utc_second(
        execution.get("markerNoLaterThan"), label="marker upper bound"
    )
    if not lower <= created < marker_upper:
        raise EvidenceError("attempt reservation is outside the frozen marker window")
...
def verify_external_attempt_time_anchor(
    ...
    if not lower <= observed < marker_upper:
        raise EvidenceError(
            "verified NIST HTTPS Date is outside the registered start window"
        )
...
def canonical_scientific_result(
    ...
    *,
    ...
    return {
        "schemaVersion": "corelm-blind-crossmodel-v1-result-v1",
...
def main() -> int:
    ...
    (
        ...
        reservation_publication_receipt_digest,
        design_ci_artifact_digests,
        execution_reservation_provenance,
    ...
    model_specs = {item["key"]: item for item in design["models"]}
    pool_tokenizer_vocabulary_sizes = {
        key: model_specs[key]["tokenizerVocabSize"] for key in model_specs
    }
    ...
    selected_tokenizer_vocabulary_sizes = {
        key: pool_tokenizer_vocabulary_sizes[key] for key in model_order
    }
    selected_model_vocabulary_sizes = {
        key: model_specs[key]["vocabSize"] for key in model_order
    }
    ...
    result.update(
        {
            ...
            "executionReservationProvenance": (
                execution_reservation_provenance
            ),
