#!/usr/bin/env python3
"""Read-only, no-inference preflight for the prospective blind-v1 suite."""
...
from blind_v1.reproducibility import verify_content_digest  # noqa: E402
...
def _absolute_without_resolving(path: Path) -> Path:
    ...
    # macOS exposes these two fixed system aliases as symlinks below
    # /private.  Accept only their exact, verified system targets; resolving
    # the complete caller path would also accept attacker-controlled links.
    if sys.platform == "darwin" and len(absolute.parts) >= 2:
        system_aliases = {
            "var": ("/var", "/private/var"),
            "tmp": ("/tmp", "/private/tmp"),
        }
        alias = system_aliases.get(absolute.parts[1])
        if (
            alias is not None
            and os.path.islink(alias[0])
            and os.path.realpath(alias[0]) == alias[1]
        ):
            return Path(alias[1], *absolute.parts[2:])
    return absolute
...
def verify_asset_receipt(
    ...
    *,
    ...
    if receipt.get("schemaVersion") != "corelm-blind-crossmodel-v1-asset-receipt-v1":
...
def result_boundary() -> dict[str, Any]:
    result_root = BLIND_V1_ROOT / "results"
    entries = sorted(path.name for path in result_root.iterdir())
    if entries != ["README.md"]:
        raise ValueError("blind-v1 result directory is not pristine")
...
def main() -> int:
    ...
    manifest = load_json_strict(BLIND_V1_ROOT / "model-assets.draft.json")
    ...
    asset_receipt = verify_asset_receipt(
        ...
        manifest_path=BLIND_V1_ROOT / "model-assets.draft.json",
        manifest=manifest,
        local_assets=assets,
    )
    ...
    result = {
        ...
        "designSHA256": sha256_bytes(
            (BLIND_V1_ROOT / "design-registration.draft.json").read_bytes()
        ),
        "codec": codec,
