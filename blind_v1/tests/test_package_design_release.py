...
from blind_v1.tests import test_freeze_manifest as freeze_fixture_module
...
class PackageDesignReleaseTests(unittest.TestCase):
    ...
    def _ci_members(
        ...
        *,
        ...
        design_raw = (subject.BLIND_V1_ROOT / "design-registration.draft.json").read_bytes()
        ...
        design_check = {
            ...
            "modelAssetManifestFileSHA256": sha256_bytes(
                (subject.BLIND_V1_ROOT / "model-assets.draft.json").read_bytes()
            ),
        ...
        log = (
            b"test_contract (blind_v1.tests.UnitContract.test_contract) ... ok\n"
            b"\n----------------------------------------------------------------------\n"
            b"Ran 1 test in 0.001s\n\nOK\n"
            b"ZERO-SKIP POLICY PASS: 1 tests, 0 skipped\n"
        )
        return {
            f"blind_v1-preflight-{suffix}.json": self._pretty(preflight),
            f"blind_v1-runtime-{suffix}.json": self._ci_runtime(
                role=role,
                system=system,
                machine=machine,
                platform_tag=platform_tag,
            ),
            f"blind_v1-zero-skip-{suffix}.log": log,
            f"blind_v1-design-check-{suffix}.json": self._pretty(design_check),
            f"blind_v1-release-attestation-known-answer-{suffix}.json": self._pretty(
    ...
    def setUp(self) -> None:
        ...
        self.asset_source_path = subject.BLIND_V1_ROOT / "model-assets.draft.json"
        ...
        key_raw = (
            subject.BLIND_V1_ROOT / "signing" / "corelm-blind-crossmodel-v1-signing.pub"
        ).read_bytes()
    ...
    def test_package_is_deterministic_exact_read_only_and_self_excluding(self) -> None:
        ...
        ci_schema = load_json_strict(
            subject.BLIND_V1_ROOT / "schemas" / "ci-artifact-verification.schema.json"
        )
    ...
    def test_ci_zip_digest_inventory_platform_workflow_and_zero_skip_fail_closed(self) -> None:
        ...
        preflight_name = "blind_v1-preflight-linux.json"
        ...
        runtime_name = "blind_v1-runtime-linux.json"
        ...
        macos_runtime_name = "blind_v1-runtime-macos.json"
        ...
        design_check_name = "blind_v1-design-check-linux.json"
        ...
        skip_members["blind_v1-zero-skip-linux.log"] = (
            b"Ran 1 test in 0.001s\n\nOK (skipped=1)\n"
            b"ZERO-SKIP POLICY PASS: 1 tests, 0 skipped\n"
        )
