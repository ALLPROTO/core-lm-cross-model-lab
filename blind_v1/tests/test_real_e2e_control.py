...
from blind_v1 import development_corpus as records
from blind_v1 import development_model_replay as independent
from blind_v1 import model_worker
from blind_v1 import run_real_e2e_control as control
from blind_v1 import runner as scientific_runner
from blind_v1.protocol import EXPECTED_DEVELOPMENT_CONTROLS, canonical_json_bytes
...
def _lab_source(
    *, commit: str = "a" * 40, tree: str = "b" * 40
    ...
    return {
        "repository": control.LAB_REPOSITORY,
        "commit": commit,
        "tree": tree,
        "worktreeClean": True,
    }
...
def _frozen_source() -> dict[str, object]:
    return {
        "tag": control.DEVELOPMENT_RELEASE_TAG,
        "tagObject": "c" * 40,
        "commit": "a" * 40,
        "tree": "b" * 40,
...
def _input_bindings() -> dict[str, object]:
    ...
    return {
        ...
        "modelAssetManifest": _digest_bytes(
            archival["development-model-assets.json"]
        ),
        "fullAssetReceipt": _digest_bytes(
            archival["development-model-assets.full-rehash.json"]
        ),
...
def _archival_inputs() -> dict[str, bytes]:
    return {
        "design-registration.draft.json": (
            control.BLIND_V1_ROOT / "design-registration.draft.json"
        ).read_bytes(),
        "development-model-assets.json": (
            control.BLIND_V1_ROOT / "development-model-assets.json"
        ).read_bytes(),
        "development-model-assets.full-rehash.json": (
            control.BLIND_V1_ROOT
            / "manifests"
            / "development-model-assets.full-rehash.json"
        ).read_bytes(),
        "development-corpus.draft.json": (
            control.BLIND_V1_ROOT / "development-corpus.draft.json"
        ).read_bytes(),
...
class RealE2EDevelopmentBoundaryTests(unittest.TestCase):
    ...
    def test_output_is_forbidden_under_scientific_results_and_repository(self) -> None:
        forbidden = control.BLIND_V1_ROOT / "results" / "development-e2e-forbidden"
        with self.assertRaisesRegex(
            control.DevelopmentControlError,
            "outside the repository and blind_v1/results",
        ):
    ...
    def test_scientific_worker_contract_remains_distinct_and_valid(self) -> None:
        design = json.loads(
            (control.BLIND_V1_ROOT / "design-registration.draft.json").read_bytes()
        )
        model_key = design["models"][0]["key"]
        model = next(item for item in design["models"] if item["key"] == model_key)
        model_assets = json.loads(
            (control.BLIND_V1_ROOT / "model-assets.draft.json").read_bytes()
        )
        files = []
        for filename, specification in model_assets["models"][model_key][
            "files"
        ].items():
            files.append(
                {
    ...
    def test_mocked_control_orchestration_never_loads_a_model(self) -> None:
        ...
        with tempfile.TemporaryDirectory(dir=TEMPORARY_PARENT) as temporary:
            ...
            self.assertFalse((control.BLIND_V1_ROOT / "results" / output.name).exists())
    ...
    def test_canonical_source_gate_derives_current_clean_head(self) -> None:
        expected = _lab_source(commit="d" * 40, tree="e" * 40)

        def git_output(_root: Path, arguments: list[str]) -> str:
            values = {
                ("remote", "get-url", "origin"): control.LAB_REPOSITORY,
                ("rev-parse", "--verify", "HEAD^{commit}"): expected["commit"],
                ("rev-parse", "--verify", "HEAD^{tree}"): expected["tree"],
            }
            return str(values[tuple(arguments)])

        with (
            mock.patch.object(control, "_verify_isolated_pycache_prefix"),
            mock.patch.object(control, "_git_output", side_effect=git_output),
            mock.patch.object(control, "_verify_clean_checkout") as clean,
            mock.patch.object(control, "_verify_head_files") as files,
        ):
            self.assertEqual(control.verify_canonical_execution_source(), expected)
        clean.assert_called_once_with(
            control.PROJECT_ROOT.resolve(strict=True),
            label="canonical lab",
            allowed_ignored_roots=control.LAB_ALLOWED_IGNORED_ROOTS,
        )
        files.assert_called_once()

    def test_post_release_source_is_derived_from_future_signed_tag(self) -> None:
        expected = _lab_source()
        tag_object = "c" * 40

        def git_output(_root: Path, arguments: list[str]) -> str:
            values = {
                ("remote", "get-url", "origin"): control.LAB_REPOSITORY,
                ("cat-file", "-t", control.DEVELOPMENT_RELEASE_TAG): "tag",
                ("rev-parse", control.DEVELOPMENT_RELEASE_TAG): tag_object,
                ("rev-list", "-n", "1", control.DEVELOPMENT_RELEASE_TAG): expected[
                    "commit"
                ],
                (
                    "rev-parse",
                    f"{control.DEVELOPMENT_RELEASE_TAG}^{{tree}}",
                ): expected["tree"],
                ("rev-parse", "HEAD"): expected["commit"],
                ("rev-parse", "HEAD^{tree}"): expected["tree"],
            }
            return str(values[tuple(arguments)])
        ...
        with (
            ...
            mock.patch.object(control, "_git_output", side_effect=git_output),
            mock.patch.object(control, "_verify_clean_checkout") as clean,
            mock.patch.object(control, "_verify_tag_signature") as signature,
            mock.patch.object(control, "_verify_head_files"),
        ):
            observed = control.verify_post_release_source()
        self.assertEqual(observed["tag"], control.DEVELOPMENT_RELEASE_TAG)
        self.assertEqual(observed["tagObject"], tag_object)
        self.assertEqual(observed["commit"], expected["commit"])
        self.assertEqual(observed["tree"], expected["tree"])
        self.assertEqual(observed["executionSource"], expected)
        clean.assert_called_once_with(
            control.PROJECT_ROOT.resolve(strict=True),
    ...
    def test_source_gate_must_match_runtime_and_report_binding(self) -> None:
        arguments = argparse.Namespace(
            asset_root=Path("assets"),
            dataset=Path("dataset"),
            codec_root=Path("codec"),
            runtime_manifest=Path("runtime"),
            output=Path("output"),
        )
        bindings = _input_bindings()
        bindings["labSource"] = _lab_source(commit="d" * 40, tree="e" * 40)
        with (
            mock.patch.object(
                control,
                "verify_canonical_execution_source",
            ...
            mock.patch.object(
                control,
                "_load_fixed_inputs",
                return_value=({}, {}, b"dataset", {}, bindings, {}),
            ),
            mock.patch.object(control, "verify_development_lifecycle") as lifecycle,
            self.assertRaisesRegex(
                control.DevelopmentControlError,
                "source gate differs from runtime/report lab binding",
            ),
        ):
            control._run_control(
                ...
                execution_id=(
                    ...
                    "20260808T000000Z-0123456789abcdef"
                ),
            )
    ...
    def test_lab_source_gate_allows_only_registered_ignored_input_roots(self) -> None:
        with tempfile.TemporaryDirectory(dir=TEMPORARY_PARENT) as temporary:
            root = Path(temporary)
            ...
            git("commit", "-q", "-m", "fixture")
            for relative in (
                "blind_v1/.assets/model/model.safetensors",
                "blind_v1/.runtime/bin/python",
                "blind_v1/.working/runtime-manifest.json",
            ):
                path = root / relative
                path.parent.mkdir(parents=True, exist_ok=True)
                path.write_bytes(b"registered external input\n")
            control._verify_clean_checkout(
                root,
                label="fixture",
                allowed_ignored_roots=control.LAB_ALLOWED_IGNORED_ROOTS,
            )
            ...
            with self.assertRaisesRegex(
                control.DevelopmentControlError,
                "forbidden ignored untracked paths",
            ):
                control._verify_clean_checkout(
                    root,
                    label="fixture",
    ...
    def test_public_regression_wrapper_requires_empty_isolated_pycache(self) -> None:
        wrapper = control.BLIND_V1_ROOT / "run_post_release_regression.py"
        ...
        self.assertLess(
            ...
            wrapper_text.index("from blind_v1.run_real_e2e_control import main"),
        )
        guide = (control.BLIND_V1_ROOT / "REPRODUCIBILITY.md").read_text(
            encoding="utf-8"
        )
        self.assertIn("PYTHONHASHSEED=0", guide)
        self.assertIn('"$RUNTIME_ROOT/bin/python" -P -s -B -X', guide)
        self.assertIn('pycache_prefix=$PYCACHE_ROOT', guide)
        self.assertIn("blind_v1/run_real_e2e_control.py", guide)
        self.assertEqual(guide.count("blind_v1/collect_snapshot.py"), 3)
        self.assertIn(
            "-I -B blind_v1/package_snapshot_release.py package", guide
        )
        self.assertIn("-I -B blind_v1/runner.py prepare", guide)
        self.assertIn("-I -B blind_v1/runner.py run-one-shot", guide)
        self.assertIn(
            "--confirm-scientific-one-shot corelm-blind-crossmodel-v1", guide
        )
        self.assertIn("PRIVATE_SNAPSHOT_SEALED", guide)
        self.assertNotIn("PRIVATE_SNAPSHOT_READY", guide)
