...
from blind_v1.release_receipt import ReleaseReceiptError, verify_release_receipt
...
class ExperimentCloseoutTests(unittest.TestCase):
    ...
    def observation(
        ...
        *,
        ...
        created_at: str = "2026-08-22T18:00:01Z",
    ) -> bytes:
        return seal(
            {
    ...
    def evidence_fixture(
        ...
        *,
        ...
        replacements = {
            ...
            "SERVER_DATE": "2026-08-26T18:01:00Z",
            "CAPTURED_AT": "2026-08-26T18:02:00Z",
            "RECEIPT_CREATED": "2026-08-26T18:03:00Z",
        }
    ...
    def test_no_attempt_is_public_closeout_not_attempt_outcome(self) -> None:
        ...
        self.assertEqual(
            closeout["publicationBindings"]["reservedAttemptId"],
    ...
    def test_no_attempt_requires_post_deadline_limited_audited_observation(self) -> None:
        before = self.observation(
            observed_at="2026-08-22T17:59:59Z",
        ...
        with self.assertRaisesRegex(ExperimentCloseoutError, "predates"):
            create_no_attempt_expired(
                ...
                classified_at="2026-08-23T18:00:01Z",
            )
    ...
    def test_no_attempt_verifier_requires_exact_observation_and_bindings(self) -> None:
        ...
        wrong_bindings = PublicationBindings(
            reserved_attempt_id=self.bindings.reserved_attempt_id,
    ...
    def test_on_time_receipt_cannot_be_mislabeled_late(self) -> None:
        asset_root, receipt, raw = self.evidence_fixture(
            "on-time-assets",
            published_at="2026-08-26T17:59:59Z",
        )
    ...
    def test_late_receipt_signature_assets_deadline_and_time_fail_closed(self) -> None:
        ...
        wrong_deadline["release"]["deadline"] = "2026-08-10T18:00:00Z"
        ...
        with self.assertRaisesRegex(
            ...
            create_late_publication_invalid(
                ...
                expected_key_fingerprint=self.fingerprint,
                expected_public_key_sha256=self.public_key_sha256,
                classified_at="2026-08-26T17:59:59Z",
                cryptographic_attestation_verifier=FIXTURE_CRYPTOGRAPHIC_VERIFIER,
            )
    ...
    def test_publication_bindings_hash_exact_bytes(self) -> None:
        ...
        execution_reservation = canonical_line(
            {
                "schemaVersion": "corelm-blind-crossmodel-v1-execution-reservation-v1",
                "suiteId": SUITE_ID,
        ...
        bindings = PublicationBindings.from_exact_bytes(
            ...
            reservation_publication_receipt=b"reservation receipt\n",
            execution_reservation=execution_reservation,
            reservation_release_manifest=b"reservation manifest\n",
        ...
        self.assertEqual(
            bindings.reserved_attempt_id,
            "20260821T180000Z-0123456789abcdef",
        )
        wrong_reservation = json.loads(execution_reservation)
        wrong_reservation["attemptId"] = "20260926T180000Z-0123456789abcdef"
        with self.assertRaisesRegex(
            ExperimentCloseoutError, "execution reservation identity"
        ):
            PublicationBindings.from_exact_bytes(
                ...
                snapshot_publication_receipt=b"snapshot receipt\n",
                reservation_publication_receipt=b"reservation receipt\n",
                execution_reservation=canonical_line(wrong_reservation),
                reservation_release_manifest=b"reservation manifest\n",
            )
        ...
        with self.assertRaisesRegex(ExperimentCloseoutError, "duplicate JSON key"):
            PublicationBindings.from_exact_bytes(
                design_registration=(
                    b'{"schemaVersion":"corelm-blind-crossmodel-v1-design-v1",'
                ...
                snapshot_publication_receipt=b"snapshot receipt\n",
                reservation_publication_receipt=b"reservation receipt\n",
                execution_reservation=b"execution reservation\n",
                reservation_release_manifest=b"reservation manifest\n",
            )
        ...
        with self.assertRaisesRegex(ExperimentCloseoutError, "reserved attempt"):
            PublicationBindings(
                reserved_attempt_id="20260926T180000Z-0123456789abcdef",
                design_registration_sha256="1" * 64,
                design_publication_receipt_sha256="2" * 64,
                snapshot_registration_sha256="3" * 64,
                snapshot_publication_receipt_sha256="4" * 64,
    ...
    def test_closeout_classification_must_precede_its_release_deadline(self) -> None:
        ...
        with self.assertRaisesRegex(ExperimentCloseoutError, "release deadline"):
            create_no_attempt_expired(
                publication_bindings=self.bindings,
                empty_result_root_observation_raw=observation,
                classified_at="2026-08-30T18:00:00Z",
            )
