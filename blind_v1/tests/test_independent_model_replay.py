...
from blind_v1 import independent_model_replay as subject
...
class IndependentModelReplayTests(unittest.TestCase):
    def test_selected_record_is_released_in_finally_before_next_read(self) -> None:
        class TrackedRecord:
            pass
        ...
        with mock.patch.object(
            subject.gc, "collect", wraps=original_collect
        ) as collect:
            result = subject._consume_owned_selected_record(
                new_record(), lambda record: ("consumed", id(record))
            )
        self.assertEqual(result[0], "consumed")
        self.assertEqual(collect.call_count, 1)
        self.assertIsNone(references[-1]())

        def fail_replay(_record: TrackedRecord) -> None:
            raise RuntimeError("fixture replay failure")

        with mock.patch.object(
            subject.gc, "collect", wraps=original_collect
        ) as collect:
            with self.assertRaisesRegex(RuntimeError, "fixture replay failure"):
                subject._consume_owned_selected_record(
                    new_record(),
                    fail_replay,
                )
        self.assertEqual(collect.call_count, 1)
        ...
        runner_source = inspect.getsource(subject.run_independent_model_replay)
        helper_source = inspect.getsource(subject._consume_owned_selected_record)
        self.assertNotIn("record_raw = read_beneath", runner_source)
        self.assertLess(
            runner_source.index("page_summary, record_commitment ="),
    ...
    def test_selected_record_exact_eight_mib_boundary_is_independent(self) -> None:
        self.assertEqual(subject.MAX_SELECTED_CANONICAL_RECORD_BYTES, 8_388_608)
        model_key = "fixture-boundary-model"
        corpora = ["corpus-a.example", "corpus-b.example"]
        selection = {
            "selectedCorpora": corpora,
            "selectedPages": {
                corpus: [
                    {"revid": corpus_index * 1000 + page_index + 1}
                    for page_index in range(subject.PAGES_PER_CORPUS)
                ]
        ...
        entries = {
            f"models/{model_key}/config.json": {
                "path": f"models/{model_key}/config.json",
                "bytes": 1,
                "sha256": "b" * 64,
                "role": "model-asset",
            },
            f"models/{model_key}/model.safetensors": {
                "path": f"models/{model_key}/model.safetensors",
                "bytes": 1,
                "sha256": weight_sha256,
                "role": "model-asset",
            },
            f"models/{model_key}/tokenizer.json": {
                "path": f"models/{model_key}/tokenizer.json",
                "bytes": 1,
                "sha256": "c" * 64,
                "role": "model-asset",
            },
        ...
        for corpus in corpora:
            for page in selection["selectedPages"][corpus]:
                path = f"records/{corpus}/{page['revid']}.bin"
                entries[path] = {
                    "path": path,
                    "bytes": subject.MAX_SELECTED_CANONICAL_RECORD_BYTES,
                    "sha256": "d" * 64,
                    "role": "eligible-corpus-record",
                }

        job = subject._expected_worker_job(
            model_key=model_key,
            design=design,
            selection=selection,
        ...
        self.assertEqual(
            job["pages"][corpora[0]][0]["recordBytes"],
            subject.MAX_SELECTED_CANONICAL_RECORD_BYTES,
        )

        oversized = copy.deepcopy(entries)
        first_path = job["pages"][corpora[0]][0]["recordPath"]
        oversized[first_path]["bytes"] = (
            subject.MAX_SELECTED_CANONICAL_RECORD_BYTES + 1
        )
        with self.assertRaisesRegex(
            subject.IndependentModelReplayError,
            "exceeds independent byte bound",
        ):
            subject._expected_worker_job(
                model_key=model_key,
                design=design,
                selection=selection,
                marker={"suiteId": SUITE, "attemptId": ATTEMPT},
                private_entries=oversized,
            )
