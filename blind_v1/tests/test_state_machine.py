...
from blind_v1.state_machine import (
    ...
    _historical_create_terminal_outcome as create_terminal_outcome,
    load_attempt_marker,
...
class StateMachineTests(unittest.TestCase):
    ...
    def test_marker_rejects_nonpublic_attempt_prefix_before_consumption(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            with self.assertRaisesRegex(StateMachineError, "canonical form"):
    ...
    def test_terminal_outcome_requires_marker_and_is_exclusive(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            outcome = create_terminal_outcome(
                ...
                completed_at="2026-08-21T18:01:00Z",
            )
    ...
    def test_marker_rejects_noncanonical_suite_and_attempt_identity(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            arguments["suite_id"] = "corelm-blind-crossmodel-v1"
            ...
            arguments["created_at"] = "2026-08-21T17:59:59Z"
            with self.assertRaisesRegex(StateMachineError, "marker window"):
                create_attempt_marker(**arguments)
            arguments["created_at"] = "2026-08-21T18:15:00Z"
            with self.assertRaisesRegex(StateMachineError, "marker window"):
                create_attempt_marker(**arguments)
            arguments["created_at"] = "2026-08-21T18:14:59Z"
            marker = create_attempt_marker(**arguments)
            self.assertEqual(marker["createdAt"], "2026-08-21T18:14:59Z")
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(StateMachineError, "precedes"):
                create_terminal_outcome(
                    ...
                    completed_at="2026-08-21T17:59:59Z",
                )
    ...
    def test_gate_outcome_must_be_durable_strictly_before_hard_deadline(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            outcome = create_terminal_outcome(
                ...
                completed_at="2026-08-22T18:00:01Z",
            )
    ...
    def test_result_root_requires_current_owner_and_exact_0700_mode(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary) / "unsafe-result"
            root.mkdir(mode=0o700)
            root.chmod(0o777)
            with self.assertRaisesRegex(StateMachineError, "mode 0700"):
                self.marker(root)
            self.assertEqual(list(root.iterdir()), [])

            root.chmod(0o700)
            with mock.patch(
                "blind_v1.state_machine.os.geteuid",
                return_value=os.geteuid() + 1,
            ):
                with self.assertRaisesRegex(StateMachineError, "current user"):
                    self.marker(root)
            self.assertEqual(list(root.iterdir()), [])

    def test_result_root_rejects_extended_acl_before_writing_state(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary) / "acl-result"
            root.mkdir(mode=0o700)
            with mock.patch(
                "blind_v1.state_machine._macos_extended_acl_present",
                return_value=True,
            ):
                with self.assertRaisesRegex(StateMachineError, "extended ACL"):
                    self.marker(root)
            self.assertEqual(list(root.iterdir()), [])

    def test_result_root_acl_inspection_failure_is_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary) / "acl-result"
            root.mkdir(mode=0o700)
            with mock.patch(
                "blind_v1.state_machine._macos_extended_acl_present",
                side_effect=StateMachineError("ACL inspection fixture failed"),
            ):
                with self.assertRaisesRegex(StateMachineError, "inspection fixture"):
                    self.marker(root)
            self.assertEqual(list(root.iterdir()), [])

    def test_real_macos_extended_acl_cannot_override_0700_gate(self) -> None:
    ...
    def test_every_marker_publication_crash_is_consumed_and_forensic(self) -> None:
        ...
        for stage in stages:
            with self.subTest(stage=stage), tempfile.TemporaryDirectory() as temporary:
                ...
                with mock.patch("blind_v1.state_machine._fault_injection", side_effect=crash):
                ...
                with mock.patch("blind_v1.state_machine._fault_injection", side_effect=crash):
                    with self.assertRaisesRegex(RuntimeError, "simulated process crash"):
                        create_terminal_outcome(
                            ...
                            completed_at="2026-08-22T18:01:00Z",
                        )
                self.assertTrue(
