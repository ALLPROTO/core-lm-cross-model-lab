...
import blind_v1.package_evidence_release as subject

from blind_v1.package_evidence_release import (
    ARTIFACT_PATHS,
...
class EvidenceReleasePackageTests(unittest.TestCase):
    ...
    def build_sources(self, state: str | None) -> dict[str, Path]:
        ...
        trust_certificate_raw = b"nist-leaf-certificate"
        trust_cert.write_bytes(trust_certificate_raw)
        certificate_id = hashlib.sha512(trust_certificate_raw).hexdigest()
        trust_raw = canonical_line(
            {
                ...
                "trustPolicy": {
                    ...
                    "revocationResidualRisk": (
                        ...
                        "can still be accepted; no contemporaneous OCSP or CRL status is checked or claimed."
                    ),
                ...
                "certificates": {
                    certificate_id: {
                        ...
                        "pem": {
                            ...
                            "bytes": len(trust_certificate_raw),
                            "sha256": sha256_bytes(trust_certificate_raw),
                        },
                        "chain": [
                            {
                                "relativePath": "certificates/leaf.der",
        ...
        reservation_receipt_raw = canonical_line(
            {"kind": "reservation", "fixture": True}
        )
        ...
        reservation_receipt_path = self.write(
            "bindings-source/reservation-publication-receipt.json",
        ...
        github_gate_raw = canonical_line(
            {
                "schemaVersion": (
                    "corelm-blind-crossmodel-v1-github-gate-receipt-v1"
                ),
                "fixture": True,
            }
        )
        ...
        freeze_raw = canonical_line(
            with_digest(
                {
                    "schemaVersion": (
                        "corelm-blind-crossmodel-v1-freeze-manifest-v1"
                    )
                }
            )
        )
        ...
        snapshot_raw = canonical_line(
            {
                "schemaVersion": "corelm-blind-crossmodel-v1-snapshot-v1",
                "suiteId": SUITE_ID,
        ...
        reservation_release_assets = self.root / "reservation-release-assets"
        reservation_release_assets.mkdir()
        execution_reservation = {
            ...
            "reservedAt": "2026-08-21T17:00:00Z",
            "targetPulseTimestamp": "2026-08-21T18:00:00.000Z",
            "targetPulseEndpoint": (
            ...
            "designRegistration": {
                ...
                "sha256": sha256_bytes(design_raw),
            },
            "snapshotRegistration": {
                "bytes": len(snapshot_raw),
                "sha256": sha256_bytes(snapshot_raw),
            },
        ...
        execution_reservation["attemptId"] = subject._derive_public_attempt_id(
            execution_reservation
        )
        attempt_id = execution_reservation["attemptId"]
        execution_reservation["reservationContentSHA256"] = sha256_bytes(
            canonical_json_bytes(execution_reservation)
        )
        (reservation_release_assets / "execution-reservation.json").write_bytes(
            canonical_line(execution_reservation)
        )
        (
            reservation_release_assets / "snapshot-publication-receipt.json"
        ).write_bytes(snapshot_receipt_raw)
        (reservation_release_assets / "sha256-manifest.json").write_bytes(
            canonical_line({"fixture": True})
        )
        ...
        private_raw = canonical_line(
            {
                ...
                "reservationPublicationReceiptSHA256": sha256_bytes(
                    reservation_receipt_raw
                ),
        ...
        marker = {
            "schemaVersion": "corelm-blind-crossmodel-v1-attempt-v1",
            "status": "STARTED",
            "suiteId": SUITE_ID,
        ...
        if state in {"PASS", "FAIL_GATES"}:
            ...
            replay_summary = {
                "schemaVersion": (
                    "corelm-blind-crossmodel-v1-independent-model-replay-v1"
                ),
                "suiteId": SUITE_ID,
                "attemptId": attempt_id,
                "replayComplete": True,
            ...
            report_raw = canonical_line(
                {
                    "schemaVersion": "corelm-blind-crossmodel-v1-independent-verification-v1",
                    "suiteId": SUITE_ID,
                    "attemptId": attempt_id,
                    "verdict": state,
            ...
            manifest_paths = [
                ...
                "page-token-evidence.jsonl",
                "workers/pythia-160m/page-token-evidence.jsonl",
                "workers/smollm-360m/page-token-evidence.jsonl",
                "workers/gpt2-124m/page-token-evidence.jsonl",
            ]
            evidence_entries = []
            ...
            evidence_entries.sort(key=lambda item: item["path"])
            evidence_manifest_raw = canonical_line(
                {
                    "schemaVersion": "corelm-blind-crossmodel-v1-evidence-manifest-v1",
            ...
            outcome = {
                "schemaVersion": "corelm-blind-crossmodel-v1-outcome-v1",
                "suiteId": SUITE_ID,
                "attemptId": attempt_id,
                "terminalState": state,
                "completedAt": "2026-08-21T19:00:00Z",
        ...
        return {
            ...
            "reservation_publication_receipt": reservation_receipt_path,
            ...
            "reservation_release_assets": reservation_release_assets,
    ...
    def test_marker_at_fifteen_minute_upper_bound_is_rejected(self) -> None:
        sources = self.build_sources("PASS")
        marker_path = sources["attempt_root"] / "attempt-marker.json"
        marker = json.loads(marker_path.read_bytes())
        marker["createdAt"] = "2026-08-21T18:15:00Z"
        marker.pop("markerContentSHA256")
        marker["markerContentSHA256"] = sha256_bytes(
            canonical_json_bytes(marker)
        )
        with self.assertRaisesRegex(EvidenceReleaseError, "marker window"):
            subject._validate_marker(canonical_line(marker))
