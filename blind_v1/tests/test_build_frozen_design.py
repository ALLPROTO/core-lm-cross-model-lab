...
class FrozenDesignBuilderTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        (self.lab_root / "blind_v1").mkdir(parents=True)
        self.draft_raw = (subject.BLIND_V1_ROOT / "design-registration.draft.json").read_bytes()
    ...
    def _arguments(self, **overrides: object) -> dict[str, object]:
        arguments: dict[str, object] = {
            ...
            "signing_public_key_path": subject.BLIND_V1_ROOT
            / "signing/corelm-blind-crossmodel-v1-signing.pub",
    ...
    def test_builds_canonical_validated_external_asset_without_overwrite(self) -> None:
        ...
        self.assertEqual(
            value["beacon"]["trustBundleStatus"],
            "FROZEN_OFFLINE_TRUST_BUNDLE",
        )
        self.assertEqual(
            value["beacon"]["candidateOfflineTrustBundleSHA256"],
            subject.NIST_CANDIDATE_TRUST_BUNDLE_SHA256,
        )
        self.assertEqual(
            value["beacon"]["frozenOfflineTrustBundleSHA256"],
            subject.NIST_FROZEN_TRUST_BUNDLE_SHA256,
        )
        self.assertEqual(
            value["beacon"]["offlineTrustBundleSHA256"],
            subject.NIST_FROZEN_TRUST_BUNDLE_SHA256,
        )
        self.assertEqual(
        ...
        trust_discharge = report["artifactVerification"]["nistTrustDischarge"]
        self.assertEqual(
            trust_discharge["status"],
            "VERIFIED_FROZEN_NIST_TRUST_DISCHARGE",
        )
        self.assertIs(trust_discharge["revocationChecked"], False)
        self.assertIs(trust_discharge["certificateChainVerified"], True)
        self.assertIs(trust_discharge["statusOnlyPromotionVerified"], True)
    ...
    def test_late_manifest_is_rejected_before_output(self) -> None:
        ...
        manifest["createdAt"] = "2026-08-09T00:00:01Z"
    ...
    def test_non_lifecycle_mutation_is_rejected(self) -> None:
        ...
        frozen = subject.construct_frozen_design(
            ...
            manifest,
            self.manifest_raw,
            trust_manifest_path=self.helper.trust_path,
        )
    ...
    def test_construct_cannot_erase_trust_blocker_without_frozen_evidence(self) -> None:
        draft = load_json_strict(subject.BLIND_V1_ROOT / "design-registration.draft.json")
        manifest = load_json_strict(self.helper.manifest_path)
        candidate = load_json_strict(self.helper.trust_path)
        candidate["status"] = "CANDIDATE_OFFLINE_TRUST_BUNDLE"
        candidate_path = self.helper.root / "candidate-trust.json"
        candidate_raw = canonical_json_bytes(candidate)
        candidate_path.write_bytes(candidate_raw)
        manifest["artifacts"]["offlineTrustBundleSHA256"] = sha256_bytes(
            candidate_raw
        )
        with self.assertRaisesRegex(
            ...
            subject.construct_frozen_design(
                ...
                manifest,
                self.manifest_raw,
                trust_manifest_path=candidate_path,
            )

        no_blocker = copy.deepcopy(draft)
        no_blocker["freezeBlockers"] = no_blocker["freezeBlockers"][1:]
        with self.assertRaisesRegex(
