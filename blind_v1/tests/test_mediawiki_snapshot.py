...
from blind_v1 import mediawiki_snapshot as subject


SERVER_DATE = "Wed, 19 Aug 2026 06:00:01 GMT"
...
class StatelessFixtureTokenizer:
    vocab_size = 1024

    def encode(self, text: str, *, add_special_tokens: bool) -> list[int]:
        return [index % self.vocab_size for index in range(512)]
...
class MediaWikiSnapshotTests(unittest.TestCase):
    ...
    @staticmethod
    def clock() -> datetime:
        return datetime(2026, 8, 19, 6, 0, 2, tzinfo=timezone.utc)
    ...
    def test_canonical_record_byte_bound_is_exact_and_independently_replayed(
        self,
    ) -> None:
        project = "en.wikipedia.org"
        for extra_byte in (0, 1):
            with self.subTest(extra_byte=extra_byte):
                case_root = self.root / f"record-bound-{extra_byte}"
                case_root.mkdir()
                value = change(1, extra_byte)
                value["title"] = f"Canonical bound case {extra_byte}"
                template = {
                    "project": project,
                    "pageid": int(value["pageid"]),
                    "revid": int(value["revid"]),
                    "userid": int(value["userid"]),
                    "timestamp": str(value["timestamp"]),
                    "username": str(value["user"]),
                    "title": str(value["title"]),
                    "content": "x",
                }
                fixed_bytes_without_content = (
                    len(subject.serialize_record(**template)) - 1
                )
                target_bytes = (
                    subject.MAX_ELIGIBLE_CANONICAL_RECORD_BYTES + extra_byte
                )
                self.assertLess(fixed_bytes_without_content, target_bytes)
                value["_content"] = "x" * (
                    target_bytes - fixed_bytes_without_content
                )
                value["sha1"] = hashlib.sha1(
                    str(value["_content"]).encode("utf-8")
                ).hexdigest()
                tokenizers = {
                ...
                item = subject.fetch_and_inventory_revision(
                    ...
                    root=case_root,
                    transport=SmartTransport({project: [value]}),
                    tokenizers=tokenizers,
                )
                subject._verify_inventory_item(
                    case_root,
                    project,
                    public_change(value),
                    item,
                ...
                if extra_byte == 0:
                    ...
                    self.assertEqual(item["record"]["bytes"], target_bytes)
                    self.assertEqual(item["ineligibilityReasons"], [])
                else:
                    self.assertFalse(item["eligible"])
                    self.assertNotIn("record", item)
                    self.assertEqual(
                        item["ineligibilityReasons"],
                        [subject.CANONICAL_RECORD_SIZE_INELIGIBILITY_REASON],
                    )
                    forged = copy.deepcopy(item)
                    forged["eligible"] = True
                    with self.assertRaisesRegex(
                        subject.SnapshotError, "replayed bounds"
                    ):
                        subject._verify_inventory_item(
                            case_root,
                            project,
    ...
    def test_crawl_fails_on_old_date_duplicate_and_continuation_cycle(self) -> None:
        ...
        def old_date(uri: str) -> subject.ArchivedHTTPResponse:
            return response(
                ...
                date="Tue, 18 Aug 2026 05:59:59 GMT",
            )
