...
from blind_v1.git_source import (
    GitSourceFile,
    GitSourceSeal,
...
from blind_v1.release_receipt import REQUIRED_ASSET_ROLES, ReleaseReceiptError
from blind_v1.release_attestation_crypto import expected_known_answer_result
from blind_v1.reproducibility import canonical_json_bytes, with_content_digest
from blind_v1.tests.test_release_receipt import _fixture_crypto_record
from blind_v1.zenodo_archive import (
    EVIDENCE_BOUNDARY,
    MANIFEST_FILE_NAME,
    HTTPSCapture,
    ZenodoArchiveError,
...
LAB_FILES = {
    ...
    "blind_v1/verifier.py": (0o755, b"#!/usr/bin/env python3\nprint('verified')\n"),
}
...
class ZenodoFixture:
    def __init__(self, root: Path) -> None:
        ...
        def ci_zip(system: str, machine: str, suffix: str) -> bytes:
            preflight = {
                "schemaVersion": "corelm-blind-crossmodel-v1-preflight-v1",
            ...
            return _zip_bytes(
                {
                    f"blind_v1-preflight-{suffix}.json": canonical_json_bytes(preflight) + b"\n",
                    f"blind_v1-runtime-{suffix}.json": runtime(system, machine),
                    f"blind_v1-zero-skip-{suffix}.log": (
                        b"Ran 42 tests in 1.0s\nOK\n"
                        b"ZERO-SKIP POLICY PASS: 42 tests, 0 skipped\n"
                    ),
                    f"blind_v1-design-check-{suffix}.json": canonical_json_bytes(check) + b"\n",
                    f"blind_v1-release-attestation-known-answer-{suffix}.json": (
                        json.dumps(
        ...
        linux_artifact_name = f"author-blind_v1-linux-development-{RUN_ID}-1"
        macos_artifact_name = f"author-blind_v1-macos-development-{RUN_ID}-1"
        ...
        development_tag = "corelm-blind-crossmodel-v1-development-control"
        ...
        design = {
            ...
            "freezeBlockers": [],
            "models": json.loads(
                (PROJECT_ROOT / "blind_v1/design-registration.draft.json").read_text(
                    encoding="utf-8"
                )
            )["models"],
            "modelCardEvidence": json.loads(
                (
        ...
        blind_model_card_manifest = json.loads(
            (
                PROJECT_ROOT / "LICENSES/blind-v1-model-card-evidence.json"
            ).read_text(encoding="utf-8")
        )
        license_members = {
            ...
            "blind-v1-model-card-evidence.json": (
                0o644,
                (
                    PROJECT_ROOT / "LICENSES/blind-v1-model-card-evidence.json"
                ).read_bytes(),
        ...
        for card in blind_model_card_manifest["cards"]:
            license_members[card["archivedPath"]] = (
                0o644,
                (PROJECT_ROOT / "LICENSES" / card["archivedPath"]).read_bytes(),
            )
        licenses_tar = _tar_bytes(license_members)
...
class ZenodoArchiveTests(unittest.TestCase):
    ...
    def test_closeout_release_attestation_must_precede_its_own_deadline(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch(
                "blind_v1.zenodo_archive.verify_attestation_record",
    ...
    def test_manifest_semantically_verifies_citation_source_and_ci_bytes(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (fixture.deposit / "linux-ci-artifact.zip").write_bytes(
                _zip_bytes({"blind_v1-zero-skip-linux.log": b"fabricated PASS\n"})
            )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            known_answer_name = "blind_v1-release-attestation-known-answer-linux.json"
            ...
            runtime = json.loads(members["blind_v1-runtime-linux.json"])
            with self.assertRaisesRegex(
    ...
    def test_manifest_rejects_model_card_evidence_not_bound_to_design(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            fixture = ZenodoFixture(Path(temporary))
            archive_path = fixture.deposit / "LICENSES.tar"
            members: dict[str, tuple[int, bytes]] = {}
            with tarfile.open(fileobj=io.BytesIO(archive_path.read_bytes()), mode="r:") as archive:
                for member in archive.getmembers():
                    stream = archive.extractfile(member)
                    self.assertIsNotNone(stream)
                    members[member.name] = (member.mode, stream.read())  # type: ignore[union-attr]
            manifest = json.loads(
                members["blind-v1-model-card-evidence.json"][1]
            )
            manifest["cards"][0]["declaredLicense"] = "mit"
            members["blind-v1-model-card-evidence.json"] = (
                0o644,
                canonical_json_bytes(manifest) + b"\n",
            )
            archive_path.write_bytes(_tar_bytes(members))
            with self.assertRaisesRegex(
                ZenodoArchiveError, "model-card evidence differs"
            ):
                build_deposit_manifest(
                    fixture.deposit, fixture.plan, **fixture.verification_kwargs
                )
