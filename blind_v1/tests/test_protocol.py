...
if np is not None:
    from blind_v1.cache_adapter import (
        build_dynamic_cache,
        flatten_kv_numpy,
...
class ProtocolTests(unittest.TestCase):
    ...
    def test_rejection_sampling_increments_counter(self) -> None:
        ...
        with patch(
            "blind_v1.protocol.hashlib.sha512",
    ...
    def test_ledger_requires_canonical_order(self) -> None:
        records = [
            {"timestamp": "2026-08-10T00:00:02Z", "pageid": 2, "revid": 2},
            {"timestamp": "2026-08-10T00:00:01Z", "pageid": 1, "revid": 1},
        ] * 8
        with self.assertRaises(ValueError):
    ...
    def test_ledger_rejects_duplicate_revision_or_page(self) -> None:
        vector = json.loads(
            (BLIND_V1_ROOT / "test-vectors" / "selection-v1.json").read_text()
        )
    ...
    def test_normative_ledger_size_and_time_window_are_enforced(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "at least 64 records"):
            validate_ledger(
                ...
                records,
                minimum_records=64,
                timestamp_start=datetime(2026, 8, 10, tzinfo=timezone.utc),
                timestamp_end=datetime(2026, 8, 18, tzinfo=timezone.utc),
            )
        expanded = [
            {
                "timestamp": f"2026-08-10T00:{index // 60:02d}:{index % 60:02d}Z",
                "pageid": index + 1,
                "revid": index + 1,
            }
            for index in range(64)
        ]
        expanded[-1]["timestamp"] = "2026-08-18T00:00:00Z"
        with self.assertRaisesRegex(ValueError, "corpus interval"):
            validate_ledger(
                "de.wikipedia.org",
                expanded,
                minimum_records=64,
    ...
    def test_known_answer_selection(self) -> None:
        ...
        self.assertEqual(len(selection["draws"]), 37)
        self.assertEqual(
    ...
    def test_normative_snapshot_binds_caller_projects_and_models(self) -> None:
        ...
        before_second_crawl["createdAt"] = "2026-08-19T05:59:59Z"
        with self.assertRaisesRegex(
            ValueError, "before the second registered crawl"
        ):
            validate_snapshot_registration(before_second_crawl, allow_fixture=False)
        last_valid_second = copy.deepcopy(snapshot)
        last_valid_second["createdAt"] = "2026-08-20T17:59:59Z"
        validate_snapshot_registration(last_valid_second, allow_fixture=False)
        for timestamp in ("2026-08-20T18:00:00Z", "2026-08-20T18:00:01Z"):
            at_or_after_release_deadline = copy.deepcopy(snapshot)
            at_or_after_release_deadline["createdAt"] = timestamp
            with self.subTest(timestamp=timestamp), self.assertRaisesRegex(
                ValueError, "at or after its registered release deadline"
            ):
                validate_snapshot_registration(
                    at_or_after_release_deadline, allow_fixture=False
                )
    ...
    def test_normative_snapshot_rejects_old_self_referential_release_form(self) -> None:
        ...
        old_snapshot = {
            ...
            "designRelease": release(
                "corelm-blind-crossmodel-v1-design", "2026-08-08T12:00:00Z"
            ),
            "snapshotRelease": release(
                "corelm-blind-crossmodel-v1-snapshot", "2026-08-19T07:00:00Z"
            ),
            "projects": [
                "de.wikipedia.org",
                "en.wikipedia.org",
    ...
    def test_freeze_status_cannot_be_enabled_by_deleting_blocker_strings(self) -> None:
        registration = json.loads(
            (BLIND_V1_ROOT / "design-registration.draft.json").read_text()
        )
    ...
    def test_design_remains_explicitly_not_freezable(self) -> None:
        ...
        self.assertTrue(
            any(
                "NIST Beacon signing certificate chain" in blocker
                and "time-valid at the target" in blocker
                and "not yet frozen" in blocker
                for blocker in blockers
            )
        )
    ...
    def test_frozen_design_lifecycle_reuses_every_normative_body_check(self) -> None:
        ...
        frozen["beacon"].update(
            ...
            offlineTrustBundleSHA256=frozen["beacon"][
        ...
        with self.assertRaisesRegex(ValueError, "trust lifecycle"):
            validate_frozen_design_registration(mutated)
        mutated = copy.deepcopy(frozen)
        mutated["beacon"]["frozenOfflineTrustBundleSHA256"] = "0" * 64
        with self.assertRaisesRegex(ValueError, "trust lifecycle"):
            validate_frozen_design_registration(mutated)
        mutated = copy.deepcopy(frozen)
    ...
    def test_design_validator_binds_normative_fields(self) -> None:
        ...
        mutations = (
            ...
            ("execution", "workerRSSMeasurement", "hard RLIMIT"),
            (
                "execution",
                "outerSupervisorIdentityClaim",
                "authenticated-parent-implementation",
            ),
            (
                "execution",
                "weightLoadOrder",
                "construct-fp32-model-before-destroying-weight-bytes",
            ),
            ("execution", "maximumSimultaneousWeightPayloadCopies", 3),
            (
                "execution",
                "staticWorstCaseWeightStorageOverlapBytes",
                4_341_886_040,
            ),
            (
                "execution",
                "maximumSelectedCorpusRecordsRetainedPerProducer",
                2,
            ),
            ("beacon", "targetUnixMilliseconds", 0),
            ("beacon", "pulseStatusCode", 1),
            ("beacon", "trustBundleStatus", "FROZEN_OFFLINE_TRUST_BUNDLE"),
            ("beacon", "frozenOfflineTrustBundleSHA256", "0" * 64),
            ("beacon", "revocationChecked", True),
            ...
            ("futureCorpus", "maximumCanonicalRecordBytes", 8_388_609),
        )
    ...
    def test_model_asset_manifest_matches_design(self) -> None:
        ...
        manifest = json.loads((BLIND_V1_ROOT / "model-assets.draft.json").read_text())
        summary = validate_model_asset_manifest(manifest, registration)
        self.assertEqual(summary["models"], 6)
        self.assertEqual(summary["runtimeFiles"], 18)
        self.assertEqual(summary["smallFilesPinnedByDigest"], 12)
        self.assertFalse(summary["smallFilesContentHashedLocally"])
...
class CacheAdapterTests(unittest.TestCase):
    ...
    def test_registered_geometries(self) -> None:
        expected = {
            "pythia-160m": (12, 12, 64, 1536),
            "pythia-70m": (6, 8, 64, 1024),
            "smollm-135m": (30, 3, 64, 384),
            "smollm-360m": (32, 5, 64, 640),
            "gpt2-124m": (12, 12, 64, 1536),
            "distilgpt2-82m": (6, 12, 64, 1536),
    ...
    def test_transformers_dynamic_cache_accepts_every_layer_geometry(self) -> None:
        ...
        for mapping in self.CONFIGS.values():
            config = classes[mapping["model_type"]].from_dict(mapping)
