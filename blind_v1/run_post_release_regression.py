#!/usr/bin/env python3
"""Run the real-data BLIND_V1 engine only as a post-release development regression.

This entrypoint is deliberately incompatible with the canonical development
control and the scientific attempt. It remains unavailable until the Blind V1
development-control tag exists, then verifies that signed tag and permits a
later non-scientific regression only from its exact commit/tree. It emits only
...
def _verify_clean_tracked_source() -> None:
    ...
    if ignored:
        if not ignored.endswith(b"\x00"):
            _fail("pre-import ignored untracked path inventory is invalid")
        ...
        for raw_path in ignored[:-1].split(b"\x00"):
            ...
            matching_roots = tuple(
                allowed
                for allowed in LAB_ALLOWED_IGNORED_ROOTS
            ...
            if (
                ...
                _fail("pre-import ignored untracked path is forbidden")
            observed_roots.add(matching_roots[0])
        for allowed in observed_roots:
            try:
                metadata = (PROJECT_ROOT / allowed.as_posix()).lstat()
            except OSError:
                _fail("pre-import ignored untracked root is unavailable")
            if not stat.S_ISDIR(metadata.st_mode) or stat.S_ISLNK(metadata.st_mode):
                _fail("pre-import ignored untracked root is not a real directory")
...
def _preimport_source_gate() -> None:
    ...
    if _git_text(("cat-file", "-t", DEVELOPMENT_RELEASE_TAG)) != "tag":
        _fail("pre-import frozen source identity differs")
    tag_object = _git_text(("rev-parse", DEVELOPMENT_RELEASE_TAG))
    frozen_commit = _git_text(
        ("rev-list", "-n", "1", DEVELOPMENT_RELEASE_TAG)
    )
    frozen_tree = _git_text(
        ("rev-parse", f"{DEVELOPMENT_RELEASE_TAG}^{{tree}}")
    )
    if any(
        len(value) != 40 or any(character not in "0123456789abcdef" for character in value)
        for value in (tag_object, frozen_commit, frozen_tree)
    ):
        _fail("pre-import frozen source identity differs")
    _verify_signature(DEVELOPMENT_RELEASE_TAG, allowed_signers)
    if (
        frozen_commit != _git_text(("rev-parse", "HEAD"))
        or frozen_tree != _git_text(("rev-parse", "HEAD^{tree}"))
    ):
        _fail("pre-import regression is not on the exact frozen source")
...
from blind_v1.run_real_e2e_control import main  # noqa: E402
