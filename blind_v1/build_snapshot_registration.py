#!/usr/bin/env python3
"""Build the pre-publication blind-v1 snapshot registration offline.

The builder accepts only already-existing commitments.  It performs no
transport, model-weight import, inference, selection, or attempt-state
mutation.  It constructs tokenizers only from already verified owned bytes.
The resulting file is canonical JSON plus one LF and is created exclusively;
...
from blind_v1.mediawiki_snapshot import verify_corpus_snapshot  # noqa: E402
from blind_v1.collect_snapshot import (  # noqa: E402
...
from blind_v1.publication import PublicationError, verify_publication  # noqa: E402
...
from blind_v1.reproducibility import verify_content_digest, write_new_bytes  # noqa: E402
...
def _validate_asset_receipt(
    ...
    *,
    ...
    if not isinstance(models, dict) or set(models) != set(source_models):
    ...
    for model_key, source in source_models.items():
        ...
        if not isinstance(files, dict) or set(files) != set(source["files"]):
